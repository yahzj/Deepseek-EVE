 ...
 *
 * 口径（与引擎同源，不另存一份）：
 * - 威胁：`wormholeLayerThreat(depth)`（层 1 = 45、每层 **×1.12** —— 2026-09-23 船长令由 ×1.10 上调；
 *   沿革：×1.16 →（2026-09-15 船长令拆墙）→ ×1.10 →（2026-09-23 船长令）→ ×1.12）；
 ...
 * `grid.exitKnown === false` 时**不许偷看** `grid.exit`（入口被标出来才置 true），没标出来就继续探索。
 * ⚠ **标出来的两条途径**（2026-09-16 甲案起）：① 到达漂浮信标；② **扫描扫到出口格本身**
 * （船长：「玩家扫描无法直接扫出下一层入口」⇒ 扫到就标）。本政策调的就是真 `wormholeGridScan`
 * ⇒ 这条自动跟随，**不需要**在这里自己模拟"扫没扫到出口格"。
 *
...
function runRunsMode(): void {
  ...
  console.log(
    '\n读法：① **主读数 = 「稀有残骸/层」**（船长 2026-09-13：「进虫洞的目的是获得专属装备（稀有残骸，虫洞专属），' +
      ...
      '⑥ 政策不偷看 `grid.exit`，出口只由**信标或用扫描扫到出口格**给出（2026-09-16 甲案）⇒ 读数里包含"找入口"的回合成本；' +
...
function main(): void {
  ...
  console.log(
    `\n① 收益曲线（解析口径：单堆收益系数 ÷ 威胁）逐层${rising ? '**严格上升** ✓' : '**未严格上升** ✗'}` +
      `（口径 = 收益每层 ×1.2、威胁每层 ×1.12；船长 2026-09-13「深层收益应该比难度曲线要更高」；` +
      `威胁增幅沿革：×1.16 →（2026-09-15）→ ×1.10 →（2026-09-23 船长令）→ ×1.12）`,
  )
  console.log(
