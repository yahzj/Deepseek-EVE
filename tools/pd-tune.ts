 ...
 * **每 pdJudgementMs(500ms) 独立判定一次**：随机挑一架**正在攻击的放飞无人机**（哨戒机**要在射程
 * PD_SENTRY_RANGE_M(2,500m) 内才进候选池**，2026-09-12 船长口径）
 * → `pdAcc − 机型闪避` 掷命中 → 命中按 `pdDmg` 走机型三层抗性；血量打空即击落（战后永久损失）。
 * 近防炮不看距离、也不参与敌舰对玩家的常规攻击。
 * ⚠ **反击令牌为敌方全队共用**（非逐舰）：我方无人机打到**任意一艘**敌舰 ⇒ 当场所有冷却已就绪的敌点防舰
 * **各还手一次**——船长 2026-09-16 复核定论「点防没问题」= 现状为准（与我方侧逐舰令牌故意不对称，勿改）。
 *
