 ...
 *
 * **2026-09-24 二次裁定（船长）**：「伤害类型的颜色怎么改动了？**能恢复到最初的颜色吗（动能黄色，高爆红色，能量蓝色）**」
 * ⇒ 三系正字为 **动能 = 黄 · 爆破 = 红 · 能量 = 蓝**。合并那一步取的是**弹药徽标族**的橙（`255 160 77`），
 * 而"红"原本只活在旧的**层位色**那套（`.app-d-explosive` = `#d34a4a`）⇒ 红被改没了。现爆炸档已按逐主题改回红。
 *
 * 判据（报错即红）：
 *   ① `.app-d-<系>` 与 `.app-a-<系>` 必须解析到**同一个 `--wui-*` token**（三系各一条）；
 *   ② 每个主题里三系底色**两两不同**，且**动能不比能量更蓝**（动能 r ≥ b、能量 b > r）——
 *      这正是"错位"的形状（错位时动能=蓝、能量=黄，②会立刻红）；
 *   ③ **爆炸必须是红**：r 通道显著高于 g/b（`r − max(g,b) ≥ 60`）——防"橙又被写回来"。
 *      ⚠ 色盲主题（`cb`）的爆炸色允许与 warm 同值复用（该主题的三系仍两两可分）。
 *
...
for (const theme of themeNames) {
  ...
  if (k && e && p) {
    ...
    // ③ 爆炸 = **红**（2026-09-24 船长令「恢复到最初的颜色」）：r 显著高于 g/b。
    //    阈值 60 把"橙"（橙档 255 160 77 ⇒ 差 95；warm 的 245 152 71 ⇒ 差 93）与"红"分开，
    //    同时容得下各主题为可读性调过的红（contrast 的 255 232 230 ⇒ 差 23 ⇒ **会红**，故该主题用低饱和红）。
    const redEdge = e[0] - Math.max(e[1], e[2])
    check(redEdge >= 60, `主题 ${theme ?? 'base'}：爆破应偏红（爆破 ${e.join()}，r−max(g,b)=${redEdge} < 60）`)
  }
}
...
console.log('')
console.log('✅ 伤害类型配色契约通过：两族共用同一套 token · 逐主题三系可分 · 动能黄 / 爆破红 / 能量蓝。')
