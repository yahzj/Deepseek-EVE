...
const READ = `(() => {
  ...
  const laneBox = laneEl
    ...
    : null
  const fit = document.querySelector('.app-battle-fit')
  return {
    旋转模式: rot,
    ...
    车道: laneBox,
    部件: ['.app-battle-screen-top', '.app-bts-stage', '.app-bts-ruler', '.app-bts-lane', '.app-bts-dock', '.app-bts-legends', '.app-bts-reloads', '.app-bts-ops'].map(one),
  }
})()`
...
function check(tag: string, r: Reading, mobile: boolean): string[] {
  ...
  if (!r.等比缩放) bad.push(`[${tag}] 没有 .app-battle-fit 包裹层（方案甲没生效？）`)
  const stage = part(r, '.app-bts-stage')
  const lane = part(r, '.app-bts-lane')
  if (mobile) {
    if (stage && stage.纵向溢出 > 0) bad.push(`[${tag}] 舞台裁内容 ${stage.纵向溢出}px`)
    ...
    if (r.等比缩放 && Number(r.等比缩放.k) > 1) bad.push(`[${tag}] 缩放系数 ${r.等比缩放.k} > 1（只缩不放的口径被破坏）`)
  } else {
    if (r.等比缩放 && Number(r.等比缩放.k) !== 1) bad.push(`[${tag}] 桌面档 k=${r.等比缩放.k} ≠ 1（桌面应逐像素不变）`)
    if (r.等比缩放 && r.等比缩放.变换 !== 'none') bad.push(`[${tag}] 桌面档带了 transform：${r.等比缩放.变换}`)
    if (r.等比缩放 && r.等比缩放.整屏隐藏溢出) bad.push(`[${tag}] 桌面档整屏被隐藏溢出（不该发生）`)
    if (stage && stage.纵向溢出 > 0) bad.push(`[${tag}] 桌面舞台裁内容 ${stage.纵向溢出}px`)
  }
  return bad
}
...
async function boot(cdp: Cdp, layout: 'classic' | 'modern', save: string): Promise<void> {
  ...
  for (let i = 0; i < 80; i++) {
    if (await cdp.evalJS<boolean>(`document.readyState === 'complete' && !!window.localStorage`)) break
    await sleep(120)
  }
  await cdp.evalJS(
  ...
  await cdp.evalJS<string>(`(() => {
    const box = document.querySelector('.app-modal-mask .app-modal, .app-modal')
    if (!box) return 'none'
    const btns = [...box.querySelectorAll('button')]
    const pick = btns.find((b) => /关闭|确定|知道了|开始|继续/.test(b.innerText || '')) ?? btns[btns.length - 1]
    if (pick) pick.click()
    return 'closed'
  })()`)
  await sleep(500)
...
async function main(): Promise<void> {
  ...
  if (problems.length > 0) {
    ...
    console.log('\n✅ 手机档：舞台/车道均不裁内容、车道要紧件不越界；桌面档：等比缩放不介入（k=1）。')
  }
}
