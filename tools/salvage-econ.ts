...
function main(): void {
  ...
  console.log('    与「AI 核心启用上限技能」双闸约束（2026-09-08 起：同时启用总数受技能上限，核心买再多也开不出超过上限的台数）；若长线验证核心印钞，旋钮候选=回调 Y 或收紧上限技能成长（未定）。')
  ...
  console.log(`校准注：锚 X 的口径含「采矿技术 5」（模拟策略统一前置）；回收链两行（无技能/满技能）为同链成长差；若采矿侧再把护卫舰操作/地质学等点满，X 同步抬升 ≈ ×1.8（橄榄级口径），两链满级比例仍落在 ×1~×1.3 上下——目测以本表为主。2026-09-06 锚 82k（×1.65）。`)
