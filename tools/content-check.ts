...
//   → 物品总数 101→**102**
check(itemDefs.length === 102, `物品总数应为 102（2026-09-25 起：+损管修理组件 repairkit-dc），实际 ${itemDefs.length}`)
 ...
 * 因此下面**不再有**「键数 = 市场卡数」与「多余键」两条（它们的前提已不存在），改为按两类键分别核对。 */
{
   ...
   * exotic拓展到3~5**」；**2026-09-20 船长再改一条见下**）。
   *
   * 现行三条带：
   * - `common`（常驻）⇒ ⚠ **不约束档位**（2026-09-20 船长：「**「common 渠道档必须=1」的契约废除，
   *   改为『common 渠道稀有度不影响交易』**」）——常驻商品可以带任意 1~5 档，那只是**描述性标签**，
   *   因为**交易结果与它无关**（实证：`market.ts` 的 `rareTierWeight` 只在 rare/exotic 分支被调用，
   *   常驻那条分支从不用它）⇒ 拿档位去卡常驻商品是**把不参与运算的字段当约束**，属过度约束；
 ...
 */
{
   ...
   * 奢侈品（`luxury`）作为「货物」的**二级子分类**（不是一级类型）。 */
  {
    ...
    for (const label of ['基础零件', '高级零件', '零件蓝图']) {
      check(subSrc.includes(label), `市场类型契约：${subPath} 的子分类里没有「${label}」一档（船长 2026-09-20 零件体系定）`)
    }
    check(
   ...
   *    速度都快**」——数量弥补所以要 3 艘杂鱼，头目强大所以头目独占 60% 血与 60% 名义火力。 */
  {
    ...
    for (const ship of FOE_SHIPS) {
      ...
      if (ship.family === 'C') {
        ...
        // **2026-09-24 船长令「设计每个种族的 T5 旗舰」⇒ 开一条 T5 旗舰位**）：
        // ①允许 1~4 档（**T4 = "巨兽"专档**）+ **5 旗舰（只允许旗舰白名单那一条）**；
        //   ⚠ 旧条文原写"**不配 5 旗舰**（生物再大也是'巨兽'而非'旗舰'编队）"——随船长本轮令**改写**：
        //   本族从此有且仅有一条 T5（周末入侵的族旗舰），**不是**放开给杂鱼；
        // ②**T4 必须登记为"巨兽"用途**（`ALIEN_BEAST_SHIP_IDS` 白名单）——目的是**防日后随手给杂鱼挂 T4**
        //   （把战列档当普通量产物用，等于把"巨兽"的意义抹平）。白名单是"显式登记"的代码化表达。
        check(
          t <= 4 || ALIEN_FLAGSHIP_SHIP_IDS.includes(ship.id),
          `舰级契约：异形族舰级「${ship.name}」（${ship.id}）登记了 ${HULL_CLASS_NAME[t as 1] ?? "未知档"}（T${t}）档——` +
            `异形族允许 1 护卫舰 ~ 4 战列舰（船长裁定③「**允许 T4 战列舰**（生物巨兽）」）＋ **5 旗舰仅限旗舰白名单那一条**` +
            `（船长 2026-09-24「设计每个种族的 T5 旗舰」⇒ \`ALIEN_FLAGSHIP_SHIP_IDS\`）；杂鱼不得挂 T5`,
        )
        if (t === 4) {
          check(
      ...
      if (ship.family === 'G') {
        ...
        //   **5 旗舰仅限旗舰白名单那一条**（旧条文"不配 5 旗舰（旗舰留给 E 族巨构）"随本轮令**改写**）；
        ...
        check(
          t <= 4 || G_FLAGSHIP_SHIP_IDS.includes(ship.id),
          `舰级契约：鱿烬亡军舰级「${ship.name}」（${ship.id}）登记了 ${HULL_CLASS_NAME[t as 1] ?? "未知档"}（T${t}）档——` +
            `G 族按 5 档体系登记、本次铺 **1 护卫舰 ~ 4 战列舰**（船长「G组分5档。从护卫舰到战列舰」）` +
            `＋ **5 旗舰仅限旗舰白名单那一条**（船长 2026-09-24「设计每个种族的 T5 旗舰」⇒ \`G_FLAGSHIP_SHIP_IDS\`）`,
        )
        if (t === 4) {
          check(
    ...
    console.log(
      `· 舰级契约：${shipCards} 张舰级路径卡（${slotTotal} 条编成，其中混编 ${mixed} 张）引用有效、族与卡面口径一致；` +
        ...
        `异形族（C）允许 T4（须登记为"巨兽"用途：${ALIEN_BEAST_SHIP_IDS.join(" / ")}）` +
        `＋ **T5 旗舰仅限白名单**（${ALIEN_FLAGSHIP_SHIP_IDS.length > 0 ? ALIEN_FLAGSHIP_SHIP_IDS.join(" / ") : "暂空"}）；` +
        `鱿烬亡军（G）登记 1~4 档（**T4 战列舰 = 那条壳体**（船长「战列舰先建壳体」；**2026-09-15 已启用**并挂蜂群机 ×3）` +
        `＋ **T5 旗舰仅限白名单**（${G_FLAGSHIP_SHIP_IDS.length > 0 ? G_FLAGSHIP_SHIP_IDS.join(" / ") : "暂空"}）；` +
...
const STALE_COPY_TERMS: ReadonlyArray<readonly [RegExp, string]> = [
   ...
   * （「陆龟级重装艇 / 玳瑁级重装巡舰 / 玄武级重装战列舰」——船长选甲案，单舰名与蓝图描述保持原样；
   * 玄武级尾字 2026-09-21 由「旗舰」改「战列舰」，见上面 `sh-xuanwu` 那行）。
   */
  [/重装舰/, '旧类别名（2026-09-16 起：装甲舰）——单舰名「重装艇/重装巡舰/重装战列舰」不受此条约束'],
  /**
   ...
   为什么钉：这条判据一旦漂回"只看 role"，船长点名的牛鲨级与 E 族专属舰就会从「装甲舰」里掉出去。 */
{
   ...
   *    例外 = **子分类给的**（构件鱼雷舰 +0.2）；**⚠ 2026-09-24 船长裁决：「鱼雷舰排除出装甲舰不得带
   *    档位单发加成的范围内。」** ⇒ `subClass === '鱼雷舰'` 的装甲舰改按**武装舰口径**给
   *    （档位阶梯 ＋ 子分类 0.2；现存唯一一艘 = 构件鱼雷舰 T1 ⇒ 0.15 ＋ 0.2 = 0.35）；
   ...
   */
  {
    ...
    for (const s of SHIPS) {
      ...
      if (cat === 'armed') {
        ...
        /**
         * 档位单发加成：装甲舰**不得有**，只留子分类给的那笔。
         * ⚠ **2026-09-24 船长裁决**：「**鱼雷舰排除出装甲舰不得带档位单发加成的范围内。**」
         * ⇒ 鱼雷舰按**武装舰口径**给（档位阶梯 ＋ 子分类 0.2）；现存唯一一艘 = 构件鱼雷舰
         * （T1 ⇒ 0.15 ＋ 0.2 = 0.35）。其余装甲舰照旧只允许子分类那笔。
         */
        const torpedo = s.subClass === '鱼雷舰'
        const allowed = torpedo ? (ARMED_DMG[s.tier] ?? 0) + subExtra : subExtra
        const got = s.powerBonus ?? 0
        if (Math.abs(got - allowed) > 1e-9)
          bad.push(
            `${s.name}（${s.id}）属装甲舰 ⇒ 单发加成只允许 ${
              torpedo ? `鱼雷舰口径 ${allowed}（档位 ＋ 子分类）` : `子分类给的 ${allowed}`
            }（现 ${s.powerBonus ?? '无'}）`,
          )
      }
    ...
    console.log(
      `· 加成与抗性新口径（船长 2026-09-17）：武装舰 ${armedN} 艘按档位给单发/无人机伤害（15/20/25/35/50 ±5；` +
        ...
        `鱼雷舰已按船长 2026-09-24 裁决**豁免**"装甲舰不带档位加成"（走武装舰口径）· ` +
