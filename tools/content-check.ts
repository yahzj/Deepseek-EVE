 ...
 * - 精炼配平（2026-09-08 船长定稿护栏）：50% 收率（无技能）时按批量 floor 结算的
...
// 精炼配平护栏（2026-09-08 船长定稿）：50% 收率 floor 结算产值/耗料价值 ∈ [0.98, 1.08]
// （配方按"零技能 ≈ 持平"配平；perOre 取整允许微正，见 items.ts 头注）
for (const item of itemDefs) {
  ...
  for (const row of item.refine) {
    ...
    if (!mineral) continue
    outValue += Math.floor(batch * row.perOre * 0.5) * (mineral.baseSellPriceIsk ?? 0)
  }
  const ratio = outValue / (batch * item.baseSellPriceIsk!)
  if (ratio < 0.98 || ratio > 1.08) {
    errors.push(
      `${item.id}（${item.name}）精炼配平失衡：50% 收率每批产值/耗料 = ${(ratio * 100).toFixed(1)}%（护栏 98%~108%，目标≈持平）`,
    )
  }
}
