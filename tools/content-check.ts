...
const scoutDmg =
  drones.find((d) => d.id === DRONE_ROLE_ANCHORS.scout)?.dmg ?? 0;
...
check(
  ammos.length === 6,
  `弹药应为 6 种（每族基础弹 + MK2），实际 ${ammos.length}`,
);
check(
  drones.length === 5,
  `无人机应为 5 种（四型制式锚点 + 专属强化型），实际 ${drones.length}`,
);

/* ── 市场目录 ── */
const goodKeys = new Set<string>();
const itemGoods = new Map<
  string,
  { rarity: string; playerSellable: boolean }
>();
for (const g of MARKET_GOODS) {
  ...
  switch (g.kind) {
    ...
    case "module":
      check(
        MODULES.some((m) => m.id === g.refId),
        `市场卡 ${g.key} → 装备 ${g.refId} 不存在`,
      );
      break;
    case "ship":
      check(
        SHIPS.some((s) => s.id === g.refId),
        `市场卡 ${g.key} → 舰船 ${g.refId} 不存在`,
      );
      break;
    case "blueprint":
      check(
        BLUEPRINTS.some((b) => b.id === g.refId) ||
          SHIP_BLUEPRINTS.some((b) => b.id === g.refId),
        `市场卡 ${g.key} → 蓝图 ${g.refId} 不存在`,
      );
      break;
    case "aicore":
      check(
        ["basic", "gamma", "beta", "alpha"].includes(g.refId),
        `市场卡 ${g.key} → 核心 ${g.refId} 非法`,
      );
      break;
    default:
  ...
  if (g.kind !== "item") {
    check(
      g.standingReq === undefined ||
        (Number.isInteger(g.standingReq) && g.standingReq >= 0),
      `市场卡 ${g.key} standingReq 非法`,
    );
  }
  if (g.poolTarget !== undefined) {
    check(
      (g.poolTarget ?? 0) > 0 && (g.supplyFlow ?? 0) > 0,
      `池商品 ${g.key} 缺少 poolTarget/supplyFlow`,
    );
  }
}
check(
  goodKeys.size === MARKET_GOODS.length,
  `市场卡键重复（${MARKET_GOODS.length - goodKeys.size} 处）`,
);
...
/* ── 数字稀有度表（2026-09-09 船长拍板：稀有度入物品本体 RARITY_TIER，市场调用）── */
{
  ...
  check(
    tableKeys.size === MARKET_GOODS.length,
    `稀有度表键数 ${tableKeys.size} ≠ 市场卡数 ${MARKET_GOODS.length}`,
  );
  for (const ref of refSet) {
    if (!tableKeys.has(ref))
      errors.push(`市场卡 ${ref} 缺稀有度表项（RARITY_TIER）`);
  }
  ...
  for (const g of MARKET_GOODS) {
    ...
    if (g.rarity === "common")
      check(v === 1, `稀有度表 ${g.refId}：common 渠道应为 1，实际 ${v}`);
    else if (g.rarity === "rare")
      check(
        v === 2 || v === 3,
        `稀有度表 ${g.refId}：rare 渠道应为 2/3，实际 ${v}`,
      );
    else
      check(
        v === 3 || v === 4,
        `稀有度表 ${g.refId}：exotic 渠道应为 3/4，实际 ${v}`,
      );
  }
}
...
for (const item of itemDefs) {
  ...
  if (item.exclusive === true) {
    check(
      !good,
      `专属物品 ${item.id} 不得上市场（专属型号只能从窝点高级箱产出）`,
    );
    continue;
  }
  if (!good) {
    errors.push(
      `物品 ${item.id}（${item.name}）没有市场卡——将无法买卖（死物品）`,
    );
  } else if (good.rarity !== "common") {
    warn.push(
      `物品 ${item.id} 的市场卡非常驻（${good.rarity}），玩家产出将无法稳定卖出`,
    );
  }
}
...
for (const b of BELTS) {
  ...
  check(
    !!item && MINEABLE_KINDS.has(item.kind),
    `采集点 ${b.id} 产物 ${b.oreId} 不存在或不可采集`,
  );
  check(
    b.standingReq === undefined ||
      (Number.isInteger(b.standingReq) && b.standingReq >= 0),
    `采集点 ${b.id} 声望门槛非法`,
  );
}
...
for (const item of itemDefs) {
  ...
  if (hasRefine) {
    check(
      MINEABLE_KINDS.has(item.kind),
      `${item.id} 带了精炼配方但不是可采集资源（应无配方）`,
    );
    for (const row of item.refine) {
      const target = items.get(row.mineralId);
      check(
        !!target && target.kind === "mineral",
        `${item.id} 配方产物 ${row.mineralId} 不存在或不是矿物`,
      );
      check(
        row.perOre > 0 && Number.isFinite(row.perOre),
        `${item.id} 配方系数非法：${row.perOre}`,
      );
    }
  } else {
    check(
...
for (const g of MARKET_GOODS) {
  if (g.kind === "module" && !moduleIds.has(g.refId))
    errors.push(`市场卡 ${g.key} 装备缺失`);
}
...
for (const bp of BLUEPRINTS) {
  ...
  if (bp.moduleId !== undefined) {
    check(
      moduleIdSet.has(bp.moduleId),
      `蓝图 ${bp.id} → 装备 ${bp.moduleId} 不存在`,
    );
  } else if (bp.itemId !== undefined) {
    const out = items.get(bp.itemId);
    check(!!out, `蓝图 ${bp.id} → 产物物品 ${bp.itemId} 不存在`);
    check(
      bp.outputUnits !== undefined &&
        Number.isInteger(bp.outputUnits) &&
        bp.outputUnits > 0,
      `蓝图 ${bp.id} 产物数量非法`,
    );
  } else {
  ...
  for (const need of bp.materials) {
    ...
    check(
      !!mat && mat.kind === "mineral",
      `蓝图 ${bp.id} 材料 ${need.itemId} 不存在或不是矿物`,
    );
    check(
      need.count > 0 && Number.isInteger(need.count),
      `蓝图 ${bp.id} 材料数量非法`,
    );
  }
  check(
...
for (const sbp of SHIP_BLUEPRINTS) {
  ...
  bpIds.add(sbp.id);
  check(
    shipIdSet.has(sbp.shipId),
    `舰船蓝图 ${sbp.id} → 舰船 ${sbp.shipId} 不存在`,
  );
  for (const need of sbp.materials) {
    const mat = items.get(need.itemId);
    check(
      !!mat && mat.kind === "mineral",
      `舰船蓝图 ${sbp.id} 材料 ${need.itemId} 不存在或不是矿物`,
    );
  }
  check(
 ...
 * 属命名习惯，只提示不拦。 */
{
  ...
  const goodsByBp = new Map(
    MARKET_GOODS.filter((g) => g.kind === "blueprint").map((g) => [
      g.refId!,
      g,
    ]),
  );
  ...
  for (const bp of [...BLUEPRINTS, ...SHIP_BLUEPRINTS]) {
    ...
    const ship = (bp as { shipId?: string }).shipId
      ? shipById.get((bp as { shipId?: string }).shipId!)
      : undefined;
    ...
    for (const m of d.matchAll(/「([^」]+)」/g)) {
      ...
      check(
        m[1] === pName,
        `蓝图说明契约：${bp.id} 说明写「${m[1]}」，实际产物名「${pName}」`,
      );
    }
    // ② 每批产出（N 发/个/枚 … 批）
    for (const m of d.matchAll(/(\d+)\s*[发个枚]/g)) {
      ...
      check(
        Number(m[1]) === out,
        `蓝图说明契约：${bp.id} 说明写每批 ${m[1]}，实际 outputUnits = ${out}`,
      );
    }
    // ③ 射程（远程/射程 N km）
    ...
    for (const m of d.matchAll(/基础\s*(\d+)\s*HP/gi)) {
      ...
      claims += 1;
      check(
        Number(m[1]) === item.repairRestore,
        `蓝图说明契约：${bp.id} 说明写基础 ${m[1]} HP，实际 repairRestore = ${item.repairRestore}`,
      );
    }
    // ⑤ 弹药克制声明
    if (item?.damageType) {
      for (const [layer, word] of [
        ["shield", "护盾"],
        ["armor", "装甲"],
      ] as const) {
        for (const m of d.matchAll(
          new RegExp(`对${word}\\s*×\\s*([\\d.]+)`, "g"),
        )) {
          claims += 1;
    ...
    if (ship) {
      for (const m of d.matchAll(/货舱\s*([\d,]+)\s*m³/g)) {
        claims += 1;
        check(
          num(m[1]!) === ship.cargoM3,
          `蓝图说明契约：${bp.id} 说明写货舱 ${m[1]} m³，实际 cargoM3 = ${ship.cargoM3}`,
        );
      }
      for (const m of d.matchAll(
        /循环\s*([\d.]+)\s*秒|([\d.]+)\s*秒\s*循环/g,
      )) {
        claims += 1;
        check(
    ...
    const oreFeeds = (oreId: string): boolean =>
      (items.get(oreId)?.refine ?? []).some((r) => mats.has(r.mineralId));
    ...
    for (const m of d.matchAll(/CPU\s*(\d+)|(\d+)\s*点\s*CPU/g)) {
      ...
      check(
        Number(m[1] ?? m[2]) === real,
        `蓝图说明契约：${bp.id} 说明写 CPU ${m[1] ?? m[2]}，实际 = ${real}`,
      );
    }
    // ⑩ 蓝图名 vs 产物名：命名习惯差异只提示
    const base = bp.name.replace(/图纸$|蓝图$/, "").trim();
    if (!pName.includes(base.replace(/[（(].*?[)）]/g, "").trim()))
      nameHints += 1;
  }
  console.log(
    `· 蓝图说明契约：核对 ${claims} 条数值声明（${BLUEPRINTS.length + SHIP_BLUEPRINTS.length} 张蓝图）；蓝图名与产物名不同写法 ${nameHints} 张（命名习惯，不拦）`,
  );
}
 ...
 * 解释不了的声明一律报错：既防"改了数值忘改说明"，也防"新写说明抄错数"。 */
{
  ...
  const claimsOf = (d: string): Claim[] => {
    const out: Claim[] = [];
    for (const m of d.matchAll(/[+＋−-]?\s*([\d.]+)\s*%/g))
      out.push({ raw: m[0].trim(), value: numVal(m[1]!), kind: "pct" });
    for (const m of d.matchAll(/×\s*([\d.]+)/g))
      out.push({ raw: m[0], value: numVal(m[1]!), kind: "mul" });
    for (const m of d.matchAll(/([\d.]+)\s*km/g))
      out.push({ raw: m[0], value: numVal(m[1]!), kind: "km" });
    for (const m of d.matchAll(/([\d.]+)\s*m³/g))
      out.push({ raw: m[0], value: numVal(m[1]!), kind: "m3" });
    for (const m of d.matchAll(/([\d.]+)\s*m(?![³a-zA-Z])/g))
      out.push({ raw: m[0], value: numVal(m[1]!), kind: "m" });
    for (const m of d.matchAll(/([\d.]+)\s*秒/g))
      out.push({ raw: m[0], value: numVal(m[1]!), kind: "sec" });
    for (const m of d.matchAll(/([\d.]+)\s*点/g))
      out.push({ raw: m[0], value: numVal(m[1]!), kind: "pt" });
    for (const m of d.matchAll(/(?:CPU|处理器)[^0-9]{0,6}([\d.]+)/g))
      out.push({ raw: m[0], value: numVal(m[1]!), kind: "cpu" });
    for (const m of d.matchAll(/([\d.]+)\s*(?:点\s*)?CPU/g))
      out.push({ raw: m[0], value: numVal(m[1]!), kind: "cpu" });
    return out;
  };
  const flatNums = (
  ...
  const droneVolumes = drones
    .map((d) => d.unitM3)
    .filter((v): v is number => typeof v === "number");
  const droneRanges = drones
    .map((d) => d.maxRangeM)
    .filter((v): v is number => typeof v === "number");
  ...
  const scan = (
    ...
    const crossDiff = refs.flatMap((o) =>
      flatNums(o).flatMap(([, v]) => self.map(([, sv]) => Math.abs(v - sv))),
    );
    for (const c of claims) {
      ...
      const hit = [...cand].some(
        (x) =>
          Math.abs(x - c.value) < 0.051 ||
          Math.abs(Math.round(x) - c.value) < 0.51,
      );
      if (!hit) {
        unexplained += 1;
        check(
          false,
          `产物说明契约：${kindLabel} ${def.id} 说明里的「${c.raw}」无法由真实数值解释——说明与实际属性对不上，或说明写了凭空的数`,
        );
      }
    }
    ...
    const mod = def as {
      repairKit?: string;
      damageTypeBonusPct?: Record<string, number>;
    } & Record<string, unknown>;
    if (mod.repairKit) {
      const kitName = items.get(mod.repairKit)?.name ?? mod.repairKit;
      check(
        d.includes(kitName),
        `产物说明契约：装备 ${def.id} 说明没点到实际消耗的修理组件「${kitName}」（接线 ${mod.repairKit}）`,
      );
    }
    ...
    for (const f of mapFields) {
      const keys = Object.keys(
        (mod[f] as Record<string, number> | undefined) ?? {},
      );
      ...
      const mentioned = new Set(
        typeWords.filter(([w]) => headline.includes(w)).map(([, t]) => t),
      );
      if (mentioned.size === 1) {
  ...
  console.log(
    `· 产物说明契约：核对 ${total} 条数值声明（装备 ${MODULES.length} 件 + 物品 ${ITEMS.length} 种），其中舱位换算 ${countHints} 处；无法解释 ${unexplained} 条`,
  );
}
 ...
 * 另加**反向断言**：说明里带 ⟦⟧ 数值的技能必须能在上述三处之一登记，防新技能悄悄写一组没人负责的数。 */
{
  ...
  const collect = (o: unknown, path: string): void => {
    ...
    for (const [k, v] of Object.entries(obj)) {
      if (typeof v === "string" && isSkillKey(k))
        ids.push({ id: v, prefix: prefixOf(k) });
      else if (typeof v === "number" && /per\s*level/i.test(k))
        pers.push({ per: v, prefix: prefixOf(k) });
      else if (Array.isArray(v) && isSkillKey(k)) {
        for (const x of v)
          if (typeof x === "string") ids.push({ id: x, prefix: prefixOf(k) });
      } else if (v && typeof v === "object" && isSkillKey(k)) {
        for (const [k2, v2] of Object.entries(v as Record<string, unknown>)) {
          // industrySkillSlots: { '技能 id': 每级工位数 }——记录本身就是"技能 → 每级值"的成对表
          if (typeof v2 === "number")
            pairs.push({ skill: k2, per: v2, from: `${path}.${k}` });
          // familySkillIds: { 武器族: '技能 id' }——与同层 familySkillPerLevel 配对
          else if (typeof v2 === "string" && !isSkillKey(k2))
            ids.push({ id: v2, prefix: prefixOf(k) });
        }
      }
    }
    for (const i of ids)
  ...
  pairs.push(
    {
      skill: "drone-warfare",
      per: DRONE_SKILL.warfarePerLevel,
      from: "combat.DRONE_SKILL.warfarePerLevel",
    },
    {
      skill: "drone-strike",
      per: DRONE_SKILL.strikePerLevel,
      from: "combat.DRONE_SKILL.strikePerLevel",
    },
    {
      skill: "drone-durability",
      per: DRONE_SKILL.durabilityPerLevel,
      from: "combat.DRONE_SKILL.durabilityPerLevel",
    },
    {
      skill: "drone-reinforce",
      per: DRONE_SKILL.reinforcePerLevel,
      from: "combat.DRONE_SKILL.reinforcePerLevel",
    },
    {
      skill: "drone-evasion",
      per: DRONE_SKILL.evasionPerLevel,
      from: "combat.DRONE_SKILL.evasionPerLevel",
    },
    {
      skill: "drone-recovery",
      per: DRONE_SKILL.recoveryPerLevel,
      from: "combat.DRONE_SKILL.recoveryPerLevel",
    },
  );
  ...
  const INLINE: Array<{
  ...
  }> = [
    {
      ...
      per: 0.02,
      call: "travel.ts travelTimeFactor",
    },
    { skill: "mining-frigate", per: 0.03, call: "balance.mining.timePerLevel" },
    {
      skill: "industrial-ops",
      per: 0.04,
      call: "mining.ts（industrial 族产量）",
    },
    { skill: "armed-ops", per: 0.03, call: "combat.ts（armed 族单发）" },
    {
      skill: "armored-ops",
      per: 0.04,
      call: "combat.ts（armored 族甲/结构容量）",
    },
    { skill: "astro-geology", per: 0.04, call: "mining.ts（全矿产量）" },
    {
      skill: "deep-hole-blasting",
      per: 0.06,
      call: "mining.ts（低品位矿 ≤55 ISK）",
    },
    { skill: "deep-space-harvesting", per: 0.05, call: "mining.ts（气/冰）" },
    {
      skill: "rich-vein-prospecting",
      per: 0.2,
      call: "mining.ts richVeinFactor",
    },
    {
      skill: "core-smelting",
      per: 0.04,
      call: "industry.ts（主控手动炉周期）",
    },
    {
      skill: "furnace-expansion",
      per: 0.06,
      call: "industry.ts（主控手动炉批容）",
    },
    {
      skill: "batch-production",
      per: 0.03,
      call: "manufacturing.ts calcBuildDurationMs",
    },
    { skill: "materials", per: 0.015, call: "manufacturing.ts materialFactor" },
    {
      skill: "industrial-automation",
      per: 0.05,
      call: "industry.ts / manufacturing.ts（炉线与制造线周期）",
    },
    {
      skill: "ai-expert",
      per: 1,
      call: "ai.ts aiCoreCap（每级 +1 枚核心上限）",
    },
    {
      skill: "navigation",
      per: 0.04,
      call: "balance.travel.skillIds/cutPerLevel",
    },
    {
    ...
    },
    {
      ...
      per: 0.008,
      call: "manufacturing.ts materialFactor",
    },
    { skill: "ai-servicing", per: 0.03, call: "ai.ts（副船采矿循环）" },
    {
      skill: "offline-ops",
      per: 0.2,
      call: "simulation.ts simulateOffline（基础 8 小时）",
    },
    { skill: "station-engineering", per: 0.08, call: "station.ts engFactor" },
    {
      skill: "salvage-recycling",
      per: 0.04,
      call: "industry.ts（回收炉周期）",
    },
    { skill: "salvage-rigging", per: 0.03, call: "salvaging.ts（打捞器周期）" },
    {
      skill: "wreck-assaying",
      per: 0.2,
      call: "salvaging.ts assayChanceOf（×1.2/级）",
    },
    ...
    {
      skill: "repair-engineering",
      per: 0.1,
      call: "shipyard.ts（停站维修费）",
    },
    { skill: "station-protocol", per: 0.05, call: "shipyard.ts（停站维修费）" },
    {
      skill: "hull-quick-repair",
      per: 0.1,
      call: "shipyard.ts（停站修理组件恢复量）",
    },
    {
      skill: "ai-core-dispatch",
      per: 0.02,
      call: "balance.aiCore.dispatchPerLevel（百分点）",
    },
    {
      skill: "accelerated-learning",
      per: 0.04,
      call: "training.ts trainingTimeFactor",
    },
    { skill: "marketing", per: 0.012, call: "market.ts marketSellSkillMult" },
    {
      skill: "source-sweeping",
      per: 0.1,
      call: "market.ts SWEEP_PER_LEVEL（×1.1/级）",
      srcNear: false,
    },
    {
      skill: "secondhand-market",
      per: 0.02,
      call: "market.ts SECONDHAND_PER_LEVEL",
      srcNear: false,
    },
    {
      skill: "galactic-happenings",
      per: 0.08,
      call: "events.ts eventCadenceFactor + expedition.ts（×1.15/级）",
    },
    { skill: "event-dividend", per: 0.15, call: "events.ts（事件现金）" },
    { skill: "signal-analysis", per: 0.08, call: "explore.ts scanSkillFactor" },
    {
      skill: "signal-filtering",
      per: 0.06,
      call: "explore.ts scanSkillFactor",
    },
    {
      skill: "salvage-diving",
      per: 0.12,
      call: "expedition.ts lootFactor + salvaging.ts",
    },
    { skill: "seizure-appraisal", per: 0.1, call: "encounters.ts（缴获）" },
    { skill: "lowsec-survival", per: 0.12, call: "encounters.ts（被抢上限）" },
    {
      skill: "deep-space-logistics",
      per: 0.04,
      call: "inventory.ts（货仓容量）",
    },
    ...
    { skill: "hold-management", per: 0.03, call: "inventory.ts（货仓容量）" },
    {
      ...
      per: 0.08,
      call: "expedition.ts bountyRewardFactor",
    },
    {
      skill: "cartography",
      per: 0.06,
      call: "explore.ts scanSkillFactor（2026-09-11 船长裁决「乙」接活到就地扫描窗口）",
    },
  ];
  const skillById = new Map(SKILLS.map((s) => [s.id, s]));
  const claimsOfSkill = (d: string): number[] =>
    [...d.matchAll(/⟦([\d.]+)/g)].map((m) => Number(m[1]));
  let checked = 0;
  let srcChecked = 0;
  ...
  const nearCheck = (item: {
    ...
    while (at >= 0) {
      ...
      if (
        win.includes(String(item.per)) ||
        win.includes(String(Number((1 + item.per).toFixed(4))))
      )
        return;
  ...
  const verify = (p: Pair): void => {
    ...
    if (!def) {
      check(
        false,
        `技能说明契约：${p.from} 指向的技能 ${p.skill} 不在技能目录中`,
      );
      return;
    }
  ...
  for (const s of SKILLS) {
    if (claimsOfSkill(s.description).length > 0 && !registered.has(s.id)) {
      check(
        false,
        `技能说明契约：技能「${s.name}」(${s.id}) 说明里有 ⟦数值⟧ 却没在契约里登记来源（引擎为准：补登记或删掉说明里的数）`,
      );
    }
  }
...
for (const s of SHIPS) {
  ...
  check(
    s.cycleSeconds > 0 && s.oreUnitsPerCycle > 0 && s.cargoM3 > 0,
    `舰船 ${s.id} 数值非法`,
  );
  // V10.5 战斗数值契约：三层血量必填且 >0
  check(
    (s.shieldHp ?? 0) > 0 && (s.armorHp ?? 0) > 0 && (s.hullHp ?? 0) > 0,
    `舰船 ${s.id} 三层血量缺失或非正（V10.5 契约）`,
  );
  ...
  for (const r of ["shieldResist", "armorResist", "hullResist"] as const) {
    ...
    for (const [t, val] of Object.entries(res)) {
      if (!DMG_TYPES.has(t))
        errors.push(`舰船 ${s.id} ${r} 含未知伤害类型键：${t}`);
      if (
        typeof val !== "number" ||
        !Number.isFinite(val) ||
        val < 0 ||
  ...
  check(
    (s.cpu ?? 0) > 0 && Number.isInteger(s.cpu),
    `舰船 ${s.id} cpu 缺失或非法（V10.5b）`,
  );
  check(
    (s.droneBayM3 ?? 0) >= 0 && Number.isFinite(s.droneBayM3),
    `舰船 ${s.id} droneBayM3 非法`,
  );
  ...
  if (slots) {
    ...
    check(
      Number.isInteger(slots.high) &&
        slots.high >= 1 &&
        slots.high <= 6 &&
        Number.isInteger(slots.mid) &&
        slots.mid >= 1 &&
        slots.mid <= 5 &&
        Number.isInteger(slots.low) &&
        slots.low >= 1 &&
        slots.low <= 6,
    ...
    check(
      total >= 3 && total <= 12,
      `舰船 ${s.id} 总槽位 ${total} 超限（3~12）`,
    );
    // V18 族定位弱断言：武装舰高槽多、装甲舰低槽多（布局草案精神）
    if (s.role === "armed")
      check(
        slots.high >= slots.low + 1,
        `武装舰 ${s.id} 高槽应显著多于低槽（${slots.high} vs ${slots.low}）`,
      );
    if (s.role === "armored")
      check(
        slots.low >= slots.high + 1,
        `装甲舰 ${s.id} 低槽应显著多于高槽（${slots.low} vs ${slots.high}）`,
      );
  }
  // V12：回避 0~0.9、命中加成 0~0.5
  check(
    s.evasion === undefined || (s.evasion >= 0 && s.evasion <= 0.9),
    `舰船 ${s.id} evasion 越界：${String(s.evasion)}`,
  );
  check(
    s.hitBonus === undefined || (s.hitBonus >= 0 && s.hitBonus <= 0.5),
    `舰船 ${s.id} hitBonus 越界：${String(s.hitBonus)}`,
  );
  for (const f of [
    "maxSpeedMps",
    "warpSpeedAus",
    ...
    check(
      ...
      `舰船 ${s.id} 间接属性 ${f} 非法：${String(v)}`,
    );
  }
  if (s.role === "armed") {
    check(
      s.powerBonus !== undefined && s.powerBonus > 0 && s.powerBonus <= 2,
      `武装舰 ${s.id} 必须有合法 powerBonus`,
    );
    check(
      (s.shieldHp ?? 0) > (s.armorHp ?? 0),
      `武装舰 ${s.id} 护盾应大于装甲（族定位）`,
    );
  } else {
...
for (const [tier, b] of Object.entries(tierTotalAvg)) {
  ...
  check(
    indAvg < othAvg,
    `tier ${tier} 工业系平均总血量（${Math.round(indAvg)}）应低于非工业系（${Math.round(othAvg)}）`,
  );
}
...
for (const a of ammos) {
  check(
    a.damageType !== undefined && DMG_TYPES.has(a.damageType),
    `弹药 ${a.id} damageType 缺失或非法`,
  );
  check(
    (a.dmg ?? 0) > 0 && Number.isFinite(a.dmg),
    `弹药 ${a.id} dmg 缺失或非正`,
  );
}
...
for (const t of DMG_TYPES) {
  ...
  check(
    list.length === 2,
    `弹药：${t} 型应恰有 2 件（基础 -l + MK2 -2），实际 ${list.length}`,
  );
  check(
    list.some((a) => a.id.endsWith("-l")),
    `弹药：${t} 型缺基础弹（-l）`,
  );
  check(
    list.some((a) => a.id.endsWith("-2")),
    `弹药：${t} 型缺 MK2 弹（-2）`,
  );
}
{
  ...
  for (const suffix of ["-l", "-2"]) {
    const byType = (t: string) =>
      ammos.find((a) => a.damageType === t && a.id.endsWith(suffix))?.dmg ?? 0;
    check(
...
for (const d of drones) {
  check(
    d.damageType !== undefined && DMG_TYPES.has(d.damageType),
    `无人机 ${d.id} damageType 缺失或非法`,
  );
  check(
    (d.dmg ?? 0) > 0 && Number.isFinite(d.dmg),
    `无人机 ${d.id} dmg 缺失或非正`,
  );
  check(
    (d.cpuUse ?? 0) > 0 && Number.isInteger(d.cpuUse),
    `无人机 ${d.id} cpuUse 缺失或非法（V10.5b 放飞占 CPU）`,
  );
  // 2026-09-10 射程分类 + 分类字段契约（船长：机型分类入本体，界面「种类」显示无人机 · 侦察机）
  const DRONE_CLASS_RANGE: Record<string, number> = {
    scout: 2500,
    combat: 3000,
    assault: 3500,
    sentry: 5000,
  ...
  check(
    dc !== undefined && DRONE_CLASS_RANGE[dc] !== undefined,
    `无人机 ${d.id} droneClass 缺失或非法：${String(dc)}`,
  );
  ...
  check(
    !!def &&
      ...
      (def.armorHp ?? 0) > 0 &&
      (def.hullHp ?? 0) > 0,
    `无人机 ${d.id} defense 缺失或三层血量非正（V11）`,
  );
  check(
    def === undefined ||
      def.evasion === undefined ||
      (def.evasion >= 0 && def.evasion <= 0.9),
    `无人机 ${d.id} defense.evasion 越界`,
  );
  for (const r of ["shieldResist", "armorResist", "hullResist"] as const) {
    const res = def?.[r];
    if (!res) continue;
...
for (const m of MODULES) {
  check(
    (m.cpuUse ?? 0) > 0 && Number.isInteger(m.cpuUse),
    `装备 ${m.id} cpuUse 缺失或非法（V10.5b）`,
  );
  // V18：家族合法（六家族 + 无人机装置两家族）
  check([...MODULE_SLOTS].includes(m.slot), `装备 ${m.id} 家族非法：${m.slot}`);
  // V18：槽类归属 rack 必填且与 rackOf 推导一致（Q3 映射集中落数据；防标注漂移）
  check(
    m.rack !== undefined && RACK_SLOTS.includes(m.rack),
    `装备 ${m.id} 缺少 V18 rack 归属`,
  );
  if (m.rack !== undefined) {
    check(
      m.rack === rackOf(m),
      `装备 ${m.id} rack 标注（${m.rack}）与 Q3 推导（${rackOf(m)}）不一致`,
    );
  }
  if (m.slot === "drone-rack" || m.slot === "drone-tac") {
    ...
    check(
      hasBay !== hasDmg,
      `无人机装置 ${m.id} 必须且只能给一个效果字段（bay/dmg）`,
    );
    if (hasBay)
      check(
        (m.droneBayBonusM3 ?? 0) <= 500,
        `无人机甲板 ${m.id} droneBayBonusM3 越界`,
      );
    if (hasDmg)
      check((m.droneDmgBonus ?? 0) <= 1, `战术导控 ${m.id} droneDmgBonus 越界`);
  }
  ...
  if (m.slot === "miner" || m.slot === "cargo") {
    // V17：工业槽保留加成系数形态
    check(
      (m.bonus ?? 0) > 0 && Number.isFinite(m.bonus),
      `工业装备 ${m.id} bonus 缺失或非法（V17 仅工业槽使用）`,
    );
  } else if (m.slot === "turret") {
    // V17：炮台不再携带工业 bonus（火力参数 = 武器卡，见下方全参数检查）
    check(
      m.bonus === undefined,
      `炮台 ${m.id} 不应携带 bonus（V17 起炮台用武器参数）`,
    );
    check(
      m.damageType !== undefined && DMG_TYPES.has(m.damageType),
      `炮台 ${m.id} damageType 缺失或非法（V17.2 炮族制：固定弹种）`,
    );
    check(
      (m.ammoPerEngagement ?? 0) > 0 && Number.isInteger(m.ammoPerEngagement),
      `炮台 ${m.id} ammoPerEngagement 缺失或非法`,
    );
    // V12：武器参数必填且值域合法
    check(
      m.maxRangeM !== undefined && m.maxRangeM > 0,
      `炮台 ${m.id} maxRangeM 缺失或非法`,
    );
    check(
      m.minRangeM !== undefined &&
        m.minRangeM >= 0 &&
        m.minRangeM < (m.maxRangeM ?? 0),
      `炮台 ${m.id} minRangeM 非法`,
    );
    check(
      m.hitRate !== undefined && m.hitRate > 0 && m.hitRate <= 1,
      `炮台 ${m.id} hitRate 非法`,
    );
    check(
      m.falloff !== undefined && m.falloff > 0 && m.falloff <= 1,
      `炮台 ${m.id} falloff 非法`,
    );
    check(
      m.reloadMs !== undefined &&
        m.reloadMs > 0 &&
        Number.isInteger(m.reloadMs),
      `炮台 ${m.id} reloadMs 非法`,
    );
    check(
      m.dmgMult !== undefined && m.dmgMult > 0,
      `炮台 ${m.id} dmgMult 非法`,
    );
    ...
    check(
      !(hasCap && hasAdd),
      `护盾 ${m.id} 同时携带容量与抗性——抗性/容量件已拆族（V17.1）`,
    );
    if (hasCap)
      check(
        cap !== undefined && cap > 0 && cap <= 2,
        `护盾 ${m.id} shieldHpBonus 非法`,
      );
    if (hasAdd) {
      for (const [t, val] of Object.entries(add ?? {})) {
        if (
          ...
          errors.push(
            `护盾 ${m.id} shieldResistAdd.${t} 非法：${String(val)}（需 (0, 0.9]，乘入缺口值）`,
          );
        }
      }
    }
    ...
    check(
      !(hasCap && hasAdd),
      `装甲 ${m.id} 同时携带容量与抗性——抗性/容量件已拆族（V17.1）`,
    );
    if (hasCap)
      check(
        cap !== undefined && cap > 0 && cap <= 2,
        `装甲 ${m.id} armorHpBonus 非法`,
      );
    if (hasAdd) {
      for (const [t, val] of Object.entries(add ?? {})) {
        if (
          ...
          errors.push(
            `装甲 ${m.id} armorResistAdd.${t} 非法：${String(val)}（需 (0, 0.9]，乘入缺口值）`,
          );
        }
      }
    }
    ...
    if (m.speedPenaltyPct !== undefined) {
      ...
      check(
        hasCap,
        `装甲 ${m.id} 带 speedPenaltyPct 却不给容量——机动代价只挂在容量件上`,
      );
    }
    ...
    check(
      m.speedBonusPct !== undefined && m.speedBonusPct > 0,
      `推进器 ${m.id} speedBonusPct 非法（需为正）`,
    );
    check(
      m.hitPenalty === undefined || (m.hitPenalty >= 0 && m.hitPenalty <= 0.5),
      `推进器 ${m.id} hitPenalty 非法（需 [0, 0.5]，V17.1 命中代价）`,
    );
  } else if (m.slot === "missile") {
    // V18B-1 导弹架：爆炸系武器形态——追踪命中（不随距离衰减）+ 近盲安全射距（防自爆）
    check(m.bonus === undefined, `导弹架 ${m.id} 不应携带工业 bonus`);
    check(
      m.damageType === "explosive",
      `导弹架 ${m.id} damageType 必须为 explosive（爆炸系武器形态）`,
    );
    check(
      (m.ammoPerEngagement ?? 0) > 0 && Number.isInteger(m.ammoPerEngagement),
      `导弹架 ${m.id} ammoPerEngagement 缺失或非法`,
    );
    check(
      m.maxRangeM !== undefined && m.maxRangeM > 0,
      `导弹架 ${m.id} maxRangeM 缺失或非法`,
    );
    check(
      (m.minRangeM ?? 0) > 0 && (m.minRangeM ?? 0) < (m.maxRangeM ?? 0),
      `导弹架 ${m.id} minRangeM 必须为近盲正数（太近会炸到自己）`,
    );
    check(
      m.hitRate !== undefined && m.hitRate > 0 && m.hitRate <= 1,
      `导弹架 ${m.id} hitRate 非法`,
    ...
    check(
      m.falloff === 1,
      `导弹架 ${m.id} falloff 必须为 1（命中不随距离衰减）`,
    );
    check(
      m.reloadMs !== undefined &&
        m.reloadMs > 0 &&
        Number.isInteger(m.reloadMs),
      `导弹架 ${m.id} reloadMs 非法`,
    );
    check(
      m.dmgMult !== undefined && m.dmgMult > 0,
      `导弹架 ${m.id} dmgMult 非法`,
    ...
    check(
      m.damageType === "plasma",
      `激光炮 ${m.id} damageType 必须为 plasma（能量系武器形态）`,
    );
    check(
      (m.ammoPerEngagement ?? 0) > 0 && Number.isInteger(m.ammoPerEngagement),
      `激光炮 ${m.id} ammoPerEngagement 缺失或非法`,
    );
    check(
      m.maxRangeM !== undefined && m.maxRangeM > 0,
      `激光炮 ${m.id} maxRangeM 缺失或非法`,
    );
    check(m.minRangeM === 0, `激光炮 ${m.id} minRangeM 必须为 0（光束无近盲）`);
    check(m.hitRate === 1, `激光炮 ${m.id} hitRate 必须为 1（必中，不掷命中）`);
    check(
      m.falloff !== undefined && m.falloff > 0 && m.falloff <= 1,
      `激光炮 ${m.id} falloff（威力衰减参）非法`,
    );
    check(
      m.reloadMs !== undefined &&
        m.reloadMs > 0 &&
        Number.isInteger(m.reloadMs),
      `激光炮 ${m.id} reloadMs 非法`,
    );
    check(
      m.dmgMult !== undefined && m.dmgMult > 0,
      `激光炮 ${m.id} dmgMult 非法`,
    ...
    const stabKeys = Object.entries(m.damageTypeBonusPct ?? {}).filter(
      ([, v]) => (v ?? 0) > 0,
    ).length;
    ...
    check(
      kinds === 1,
      `支援件 ${m.id} 必须且只能给一类效果（伤害系/射速/命中/闪避/修复）`,
    );
    if (hasRepair) {
      // 修复系：装甲/结构修复值 ∈ [1, 100]、周期缺省 5 秒（2000~60_000 毫秒）、必须指明消耗的修理组件
      check(
        (m.repairArmorHp ?? 0) >= 0 &&
          (m.repairArmorHp ?? 0) <= 100 &&
          (m.repairHullHp ?? 0) >= 0 &&
          (m.repairHullHp ?? 0) <= 100,
        `支援件 ${m.id} 修复值非法（需 [0, 100] 且至少一层 > 0）`,
      );
      const interval = m.repairIntervalMs ?? 5_000;
      check(
        Number.isInteger(interval) && interval >= 2_000 && interval <= 60_000,
        `支援件 ${m.id} repairIntervalMs 非法（需 2000~60000 毫秒）`,
      );
      // 2026-09-10 船长：异形生体件 = **无消耗自愈**（repairFree）——与消耗件互斥：
      // 要么指明民用/军用修理组件，要么标 repairFree（不吃组件、永不停机）
      check(
        (m.repairKit === "repairkit-civ" || m.repairKit === "repairkit-mil") !==
          (m.repairFree === true),
        `支援件 ${m.id} 必须二选一：指明消耗组件（民用/军用修理组件）或标 repairFree（无消耗自愈）`,
      );
      check(m.rack === "mid", `支援件 ${m.id}（修复）应为中槽，实际 ${m.rack}`);
      check(
        m.reloadCutPct === undefined &&
          m.hitBonusPct === undefined &&
          m.evasionGapPct === undefined &&
          stabKeys === 0,
      ...
      for (const [t, val] of Object.entries(m.damageTypeBonusPct ?? {})) {
        if (
          ...
          errors.push(
            `支援件 ${m.id} damageTypeBonusPct.${t} 非法：${String(val)}`,
          );
        }
      }
      if (hasRof)
        check(
          (m.reloadCutPct ?? 0) > 0 && (m.reloadCutPct ?? 0) <= 0.9,
          `支援件 ${m.id} reloadCutPct 非法（需 (0, 0.9]）`,
        );
      if (hasHit)
        check(
          (m.hitBonusPct ?? 0) > 0 && (m.hitBonusPct ?? 0) <= 0.9,
          `支援件 ${m.id} hitBonusPct 非法（需 (0, 0.9]）`,
        );
      if (hasEva)
        check(
          (m.evasionGapPct ?? 0) > 0 && (m.evasionGapPct ?? 0) <= 0.9,
          `支援件 ${m.id} evasionGapPct 非法（需 (0, 0.9]）`,
        );
      // 归槽语义：伤害/射速 = 低槽；命中/闪避 = 中槽（数据显式 rack，rackOf 已校验一致）
      if (stabKeys > 0 || hasRof)
        check(
          m.rack === "low",
          `支援件 ${m.id}（伤害/射速）应为低槽，实际 ${m.rack}`,
        );
      if (hasHit || hasEva)
        check(
          m.rack === "mid",
          `支援件 ${m.id}（命中/闪避）应为中槽，实际 ${m.rack}`,
        );
    }
    check(m.bonus === undefined, `支援件 ${m.id} 不应携带工业 bonus`);
    check(
      m.maxRangeM === undefined && m.damageType === undefined,
      `支援件 ${m.id} 不应携带炮台武器参数`,
    );
    ...
    // 不携带其它效果/武器参数
    check(
      (m.lockDmgBonus ?? 0) > 0 && (m.lockDmgBonus ?? 0) <= 0.9,
      `锁定装置 ${m.id} lockDmgBonus 非法（需 (0, 0.9]）`,
    );
    check(m.rack === "high", `锁定装置 ${m.id} 应为高槽，实际 ${m.rack}`);
    check(
      m.bonus === undefined &&
        m.damageType === undefined &&
        m.maxRangeM === undefined,
      `锁定装置 ${m.id} 不应携带工业/武器参数`,
    );
  }
}
 ...
 *   mSec = 1 + 0.45×max(0,−sec)；mThreat = 1 + 0.004×threat；档基数 = RECYCLE_POOL_AVG_ISK[tier] */
{
  ...
  for (const def of ANOMALIES_FLAVORED) {
    ...
    const sec =
      typeof galaxy?.security === "number" && Number.isFinite(galaxy.security)
        ? galaxy.security
        : 0.5;
  ...
  console.log(
    `· B3.1 特色回收池：${flavored} 张（约束：池均价 = m × 档基数 ±3%）`,
  );
  ...
  for (const g of WRECK_BUY_GOODS) {
    const anoId = g.refId.startsWith("wreck-")
      ? g.refId.slice("wreck-".length)
      : "";
    const def = ANOMALIES_FLAVORED.find((a) => a.id === anoId);
    const sec =
      typeof ctx.galaxies.get(def?.galaxyId ?? "")?.security === "number"
        ? ctx.galaxies.get(def!.galaxyId)!.security!
        : 0.5;
    const density = Math.min(40, Math.max(10, Math.round(10 + 15 * (1 - sec))));
    const tier = density >= 30 ? "dire" : density >= 20 ? "risky" : "common";
    check(
      g.basePrice === wreckBuyPrice[tier],
      `残骸卡 ${g.key} 价格档错位：期望 ${wreckBuyPrice[tier]}（${tier}），实际 ${g.basePrice}`,
    );
    check(
      g.basePrice < noSkillPerM3,
      `残骸卡 ${g.key} 收价 ${g.basePrice} 不低于无技能拆解保底 ${noSkillPerM3.toFixed(1)}/m³——会击穿回收线最低锚`,
    );
    check(
      g.playerBuyable === false,
      `残骸卡 ${g.key} 必须只收不卖（playerBuyable=false）`,
    );
  }
  console.log(
    `· 残骸收购卡：${WRECK_BUY_GOODS.length} 张（收价 = 常 30 / 险 40 / 危 50 ISK·m³，须低于无技能拆解保底）`,
  );
  ...
  const weaponIds = new Set(
    MODULES.filter(
      (m) => m.slot === "turret" || m.slot === "laser" || m.slot === "missile",
    ).map((m) => m.id),
  );
  ...
  for (const def of ANOMALIES_FLAVORED) {
    ...
    check(
      sec < 0.5,
      `B3.1 主题彩头仅限 sec<0.5 星系：${def.name}（${def.galaxyId}）sec=${sec}`,
    );
    for (const group of ["modules", "mk2"] as const) {
      for (const id of loot[group] ?? []) {
        const isMk3 = id.endsWith("-3");
        const vaultMk3 =
          def.id === "ano-vault-sentinel" && mk3Weapons.includes(id);
        check(
          !isMk3 || vaultMk3,
          `B3.1 MK3 直出收口失败：${def.name} 主题件 ${id}（MK3 一律走碎片，唯一白名单 = 穹顶守卫 × 三把 MK3 武器）`,
        );
        check(
          !weaponIds.has(id) || vaultMk3,
          `B3.1 武器移出主题失败：${def.name} 主题追加件 ${id} 是武器（主题只放增幅装备；唯一武器例外 = 穹顶守卫三把 MK3）`,
        );
      }
    }
  }
 ...
 * ③窝点派生卡（`lairAnomalyOf`）= 同一主系 + **6:4**（60%/40%）；主系**不许变**。 */
{
  ...
  let pureBeam = 0;
  const positive = (
    mix: Partial<Record<string, number>> | undefined,
  ): Array<[string, number]> =>
  ...
  for (const def of ANOMALIES_FLAVORED) {
    if (def.id === TEACHING_CARD) {
      check(
        positive(def.dmgMix).length <= 1,
        `混伤契约：教学卡 ${def.id} 应保持纯系（不写两系 dmgMix）`,
      );
      continue;
    }
    ...
    if (rows.length === 1 && rows[0]![0] === "plasma") {
      const pureShips = [
        ...new Map((def.ships ?? []).map((s) => [s.ship.id, s.ship])).values(),
      ];
    ...
    check(
      rows.length === 2,
      `混伤契约：${def.name} 应写两系 dmgMix（主 8 : 副 2），实际 ${rows.length} 系`,
    );
    if (rows.length !== 2) continue;
    const sorted = [...rows].sort((a, b) => b[1] - a[1]);
    check(
      sorted[0]![1] === 8 && sorted[1]![1] === 2,
      `混伤契约：${def.name} 权重应为主 8 : 副 2，实际 ${sorted[0]![1]}:${sorted[1]![1]}`,
    );
    ...
    check(
      lRows.length === 2 &&
        lSorted[0]![1] === 6 &&
        lSorted[1]![1] === 4 &&
        (lSorted[0]![0] as DamageType) === main,
   ...
   * 其余族（非 A）舰级路径卡仍按上面的战术带；旧路径卡（未写 ships）一律不变。 */
  {
    ...
    const agiMul = (ag: number): number =>
      bal.speedFactor * (1 + (ag - 0.5) * 2 * bal.agilitySpeedBonus);
    const sortedShips = [...SHIPS].sort(
      (a, b) => (a.maxSpeedMps ?? 0) - (b.maxSpeedMps ?? 0),
    );
    const refShip = sortedShips[Math.floor(sortedShips.length / 2)]!;
    const refCombat =
      (refShip.maxSpeedMps ?? 0) * agiMul(refShip.agility ?? 0.5);
    ...
    for (const def of ANOMALIES_FLAVORED) {
      ...
      if (mains.length > 0) {
        ...
        for (const slot of mains) {
          ...
          const spd = Math.round(
            base * slot.ship.speedRatio * (slot.speedMul ?? 1),
          );
          ...
          if (def.foeFamily === "A") {
            ...
            pirateReadings++;
            pirateSample.push(
              `${def.id}/${slot.ship.name} ${spd}(${ratio.toFixed(2)})`,
            );
            check(
              spd > base,
          ...
          if (def.foeFamily === "C") {
            ...
            if (alienSample.some((s) => s.startsWith(`${slot.ship.id} `)))
            ...
            check(
              slot.ship.speedRatio >= ALIEN_SPEED_RATIO_BAND[0] &&
                slot.ship.speedRatio <= ALIEN_SPEED_RATIO_BAND[1],
   ...
   *    速度都快**」——数量弥补所以要 3 艘杂鱼，头目强大所以头目独占 60% 血与 60% 名义火力。 */
  {
    ...
    const unitCount = (slot: { count?: number }): number =>
      Math.max(1, Math.floor(slot.count ?? 1));
    /** 编成实算总血（舰级血 × hpMul × 数量） */
    const slotsHp = (
      slots: readonly {
        ship: { hp: number };
    ...
    for (const def of ANOMALIES_FLAVORED) {
      ...
      check(
        mains.length >= 1,
        `舰级契约：${def.name} 没有任何非僚机编成条目（至少需要一艘主体）`,
      );
      ...
      for (const slot of mains) {
        if (def.defProfile) {
          ...
          check(
            Math.abs(want.s - got.s) < 1e-9 &&
              Math.abs(want.a - got.a) < 1e-9 &&
              Math.abs(want.h - got.h) < 1e-9,
    ...
    for (const def of ANOMALIES_FLAVORED) {
      ...
      const tactics =
        def.ships && def.ships.length > 0
          ? def.ships
              .filter((x) => x.escort !== true)
              .map((x) => x.tactic ?? x.ship.tactic)
          : [def.tactic ?? "orbit"];
 ...
 *（当天补齐的正是 wreck / fragment / kit / salvager / target-lock 五个键）——新增内容种类时在此拦住。 */
{
  ...
  if (!existsSync(glyphPath)) {
    check(
      false,
      `图标契约：找不到渲染层图标库 ${glyphPath}（文件移位请同步本检查）`,
    );
  } else {
    const glyphSrc = readFileSync(glyphPath, "utf8");
    const keysOf = (block: string): Set<string> => {
      const m = glyphSrc.match(
        new RegExp(
          `${block}: Record<string, (?:string|ReactNode)> = \\{([\\s\\S]*?)\\n\\}`,
        ),
      );
      return new Set(
        [...(m?.[1] ?? "").matchAll(/^\s*'?([A-Za-z0-9-]+)'?:/gm)].map(
          (x) => x[1],
        ),
      );
    };
    ...
    const needed = [
      ...ITEM_KIND_ORDER,
      ...MODULE_SLOTS,
      ...Object.keys(SHIP_ROLE_LABELS),
      "blueprint",
    ];
    for (const key of needed) {
      check(
        shapes.has(key),
        `图标契约：${key} 缺 Glyphs 图形（列表/卡片行首会落兜底圆环徽；请在 ui/Glyphs.tsx 补 SHAPES.${key}）`,
      );
      check(
        tones.has(key),
        `图标契约：${key} 缺 TONES 色调（未知键一律落默认灰）`,
      );
    }
 ...
 * 另：**B 族（武装拾荒者）已取消**——任何 B 族卡都不得成为窝点候选（船长 2026-09-10 定）。 */
{
  ...
  for (const def of ANOMALIES_FLAVORED) {
    ...
    if (rareDef) {
      check(
        rareDef.kind === "wreck",
        `窝点契约：稀有残骸 ${rareId} 种类应为 wreck，实际 ${rareDef.kind}`,
      );
      check(
        rareDef.unitM3 === 1,
   ...
   * ④对应的**稀有残骸物品仍能被注册**（在 ctx.items 里）——这正是白名单存在的唯一目的。 */
  {
    ...
    for (const id of RETIRED_LAIR_CARD_IDS) {
      ...
      check(
        !!def,
        `退役窝点卡契约：白名单 id「${id}」在 ANOMALIES 里不存在（写错 id ⇒ 旧档稀有残骸照样认不出）`,
      );
      if (!def) continue;
      retiredChecked += 1;
      check(
   ...
   * ③（提示，不拦）同族内"级别更高而奖金更低"属反常，多半是标反了。 */
  {
    ...
    for (const def of ANOMALIES_FLAVORED) {
      ...
      if (lv === 1 || lv === 2 || lv === 3)
        levelByFam.set(fam, [...(levelByFam.get(fam) ?? []), lv]);
    }
    for (const [fam, levels] of levelByFam) {
      ...
      const cards = [...ANOMALIES_FLAVORED].filter(
        (d) => isLairCandidate(d) && d.foeFamily === fam,
      );
      for (const lo of cards) {
        for (const hi of cards) {
          if (
            lairLevelOf(hi) > lairLevelOf(lo) &&
            hi.rewardIsk < lo.rewardIsk
          ) {
            warn.push(
   ...
   * 适用范围：② 只对**舰级路径**候选卡生效（旧路径由威胁曲线负责，天生随 tier 变强）。 */
  {
    ...
    const hpOf = (card: AnomalyDef): number =>
      (card.ships ?? []).reduce(
        (n, s) =>
          n +
          s.ship.hp * (s.hpMul ?? 1) * Math.max(1, Math.floor(s.count ?? 1)),
        0,
      );
    const powerOf = (card: AnomalyDef): number =>
      (card.ships ?? []).reduce(
        (n, s) =>
          ...
          s.ship.shotDmg *
            (s.dmgMul ?? 1) *
            Math.max(1, Math.floor(s.count ?? 1)),
        0,
      );
    let waveChecked = 0;
    let scaledCards = 0;
   ...
   * 2026-09-10 船长：G 族第一件改为**无人机物品**（`item.exclusive`），同一套契约对它同样成立。 */
  {
    ...
    for (const bp of BLUEPRINTS)
      if (bp.moduleId) bpByModule.set(bp.moduleId, bp.id);
    let marketCards = 0;
    let itemGear = 0;
    for (const id of lairGearIds) {
      ...
      check(
        !bp,
        `来源唯一契约：窝点专属${kindText} ${id} 不得有蓝图（现被 ${bp} 产出；专属装备只能从高级箱出）`,
      );
      const card = [...lairCtx.marketGoods.values()].find(
        (g) => g.key === id || g.refId === id,
      );
      if (card) marketCards += 1;
      check(
        !card,
        `来源唯一契约：窝点专属${kindText} ${id} 不得有市场卡（现被 ${card?.key} 上架；专属装备只能从高级箱出）`,
      );
    ...
    check(
      marketCards === 0,
      `来源唯一契约：窝点专属装备共 ${marketCards} 件出现在市场上架（应为 0）`,
    );
    console.log(
  ...
  for (const plan of BOUNTY_ZONE_PLAN) {
    const have =
      plan.zone === "高安" ? 0 : zoneCount[plan.zone as "中安" | "低安"];
    check(
      have >= plan.count,
  ...
  console.log(
    `· 窝点契约：${lairCards} 张窝点卡（稀有残骸 + 三档称呼 + 专属装备齐备，覆盖 ${famWithGear.size} 个敌族）`,
  );
}
 ...
 * ⚠ 若 ① 真抓到不一致：**不要顺手改船的 tier**——把不一致的船交船长裁决。 */
{
  ...
  for (const t of [1, 2, 3, 4, 5] as const) {
    ...
    if (t < 5) {
      check(
        HULL_CLASS_MASS_RANGE[t][1] ===
          HULL_CLASS_MASS_RANGE[(t + 1) as 2 | 3 | 4 | 5][0],
   ...
   与**发件方引用契约**（factionId/deptId 必须存在、kind 必须落在势力白名单、每个势力至少被引用一次）。 */
{
  ...
  const loreHits = (
    text: string,
    opts?: { keepSelfKnowledge?: boolean },
  ): string[] => {
    let masked = text;
    for (const ex of LORE_EXEMPT)
      masked = masked.split(ex).join("□".repeat(ex.length));
  ...
  for (const f of COMMS_FACTIONS) {
    check(
      f.id.trim().length > 0 && !factionIds.has(f.id),
      `势力 id 重复或为空：${f.id}`,
    );
    factionIds.add(f.id);
    ...
    check(
      ...
      isSystem
        ? `系统来源 ${f.id} 缺物种标注`
        : `势力 ${f.id} 的物种必须是章鱼人（世界观铁律），实际：${f.species}`,
    );
    check(ALIGNMENTS.has(f.alignment), `势力 ${f.id} 立场非法：${f.alignment}`);
    check(
      /^#[0-9a-fA-F]{6}$/.test(f.tone),
      `势力 ${f.id} 色调不是六位十六进制：${f.tone}`,
    );
    check(
      ICON_NAMES.has(f.glyph),
      `势力 ${f.id} 图标不在既有线稿图标表内：${f.glyph}`,
    );
    ...
    for (const d of f.departments) {
      check(
        d.id.trim().length > 0 && !deptIds.has(d.id),
        `势力 ${f.id} 的部门 id 重复或为空：${d.id}`,
      );
      deptIds.add(d.id);
      ...
      check(
        d.kinds.length > 0,
        `势力 ${f.id} 部门 ${d.id} 没有可发内容类型白名单`,
      );
      for (const k of d.kinds) {
        check(
          KINDS.has(k),
          `势力 ${f.id} 部门 ${d.id} 白名单里的内容类型非法：${k}`,
        );
        check(
          f.kinds.includes(k),
          `势力 ${f.id} 部门 ${d.id} 白名单「${k}」超出势力白名单`,
        );
      }
    }
  }
  ...
  for (const m of COMMS_MESSAGES) {
    check(
      m.id.length > 0 && !seen.has(m.id),
      `通讯消息 id 重复或为空：${m.id}`,
    );
    seen.add(m.id);
    check(m.subject.trim().length > 0, `通讯 ${m.id} 缺主题`);
    check(
      m.body.length > 0 && m.body.every((p) => p.trim().length > 0),
      `通讯 ${m.id} 正文为空段`,
    );
    ...
    check(
      faction !== undefined,
      `通讯 ${m.id} 的发件势力不存在：${m.factionId}`,
    );
    check(KINDS.has(m.kind), `通讯 ${m.id} 内容类型非法：${m.kind}`);
    if (faction) {
      referenced.add(faction.id);
      check(
        faction.kinds.includes(m.kind),
        `通讯 ${m.id} 的内容类型「${m.kind}」不在势力 ${faction.id} 白名单内`,
      );
      if (m.deptId !== undefined) {
        const dept = faction.departments.find((d) => d.id === m.deptId);
        check(
          dept !== undefined,
          `通讯 ${m.id} 的发件部门不在势力 ${faction.id} 下：${m.deptId}`,
        );
        if (dept)
          check(
            dept.kinds.includes(m.kind),
            `通讯 ${m.id} 的内容类型「${m.kind}」不在部门 ${m.deptId} 白名单内`,
          );
      }
    }
    check(
      TRIGGER_KINDS.has(m.trigger.kind),
      `通讯 ${m.id} 触发器 kind 未知：${m.trigger.kind}`,
    );
    switch (m.trigger.kind) {
      case "day":
        check(
          Number.isInteger(m.trigger.days) && m.trigger.days >= 1,
          `通讯 ${m.id} day.days 应为 ≥1 整数`,
        );
        break;
      case "explored":
        check(
          Number.isInteger(m.trigger.count) && m.trigger.count >= 1,
          `通讯 ${m.id} explored.count 应为 ≥1 整数`,
        );
        break;
      case "galaxy":
        check(
          galaxyIds.has(m.trigger.galaxyId),
          `通讯 ${m.id} 指向的星系不存在：${m.trigger.galaxyId}`,
        );
        break;
      case "skill":
        check(
          skillIds.has(m.trigger.skillId),
          `通讯 ${m.id} 指向的技能不存在：${m.trigger.skillId}`,
        );
        check(
          Number.isInteger(m.trigger.level) && m.trigger.level >= 1,
          `通讯 ${m.id} skill.level 应为 ≥1 整数`,
        );
        break;
      case "isk":
      ...
      case "siteBuilt":
        check(
          siteIds.has(m.trigger.siteId),
          `通讯 ${m.id} 指向的建站点不存在：${m.trigger.siteId}`,
        );
        break;
      case "tutorial":
        // 教程通讯：0 = 序章简报（信息库检索重启），1..TUTORIAL_TOTAL = 七步教程（与 core 的 ONB_* 同值）
        check(
          Number.isInteger(m.trigger.step) &&
            m.trigger.step >= 0 &&
            m.trigger.step <= TUTORIAL_TOTAL,
    ...
    if (m.hint) {
      hints++;
      check(
        JUMP_PAGES.has(m.hint.page),
        `通讯 ${m.id} 跳转目标页非法：${m.hint.page}`,
      );
      check(m.hint.text.trim().length > 0, `通讯 ${m.id} 跳转提示为空`);
      if (m.hint.tab !== undefined) {
        check(
          m.hint.page === "map",
          `通讯 ${m.id} 只有星图页支持标签跳转，实际页：${m.hint.page}`,
        );
        check(
          MAP_TABS.has(m.hint.tab),
          `通讯 ${m.id} 星图标签非法：${m.hint.tab}`,
        );
      }
      if (m.hint.shipTab !== undefined) {
        check(
          m.hint.page === "ship",
          `通讯 ${m.id} 只有舰船页支持标签跳转，实际页：${m.hint.page}`,
        );
        check(
          SHIP_TABS.has(m.hint.shipTab),
          `通讯 ${m.id} 舰船标签非法：${m.hint.shipTab}`,
        );
      }
    }
    for (const p of [m.subject, ...m.body, ...(m.hint ? [m.hint.text] : [])]) {
      for (const w of BANNED) {
        check(
          !p.includes(w),
          `通讯 ${m.id} 的玩家可见文案含开发用词「${w}」：${p.slice(0, 24)}…`,
        );
      }
      // 船内系统（信息库）= 玩家自己的系统，可以直说「舰载 AI」这类自我认知；NPC 文案不许（见 loreHits 注释）
      for (const w of loreHits(p, {
        keepSelfKnowledge: faction?.alignment === "系统",
      })) {
        check(
          false,
          `通讯 ${m.id} 的文案触碰世界观铁律「${w}」（章鱼人不追问船里是谁）：${p.slice(0, 24)}…`,
        );
      }
      for (const w of LORE_SPLIT) {
        check(
          !p.includes(w),
          `通讯 ${m.id} 的文案把玩家与船分开说「${w}」（玩家就是那条船）：${p.slice(0, 24)}…`,
        );
      }
    }
  }
  for (const d of DIALOGUES) {
    ...
    check(
      ...
      `通讯剧本 ${d.id} 缺标题（收件箱发件人栏）`,
    );
    for (const w of LORE_SPLIT) {
      check(
        !d.title.includes(w),
        `通讯剧本 ${d.id} 标题把玩家与船分开说「${w}」`,
      );
    }
    ...
    if (d.commsFactionId !== undefined) {
      ...
      check(
        faction !== undefined,
        `通讯剧本 ${d.id} 的挂靠势力不存在：${d.commsFactionId}`,
      );
      if (faction) {
        referenced.add(faction.id);
        if (d.commsDeptId !== undefined) {
          ...
          check(
            dept !== undefined,
            `通讯剧本 ${d.id} 的挂靠部门不在势力 ${faction.id} 下：${d.commsDeptId}`,
          );
        }
      }
    } else {
      check(
        d.commsDeptId === undefined,
        `通讯剧本 ${d.id} 有挂靠部门却没有挂靠势力`,
      );
    }
    for (const line of d.lines) {
      for (const w of loreHits(line.text)) {
        check(
          false,
          `通讯剧本 ${d.id} 的台词触碰世界观铁律「${w}」：${line.text.slice(0, 24)}…`,
        );
      }
      for (const w of LORE_SPLIT) {
        check(
          !line.text.includes(w),
          `通讯剧本 ${d.id} 的台词把玩家与船分开说「${w}」：${line.text.slice(0, 24)}…`,
        );
      }
    }
  }
  ...
  for (const f of COMMS_FACTIONS) {
    check(
      referenced.has(f.id),
      `势力 ${f.id}（${f.name}）已登记但没有任何消息或剧本引用它`,
    );
  }
  console.log(
...
/* ── 输出 ── */
console.log(
  `· 蓝图：装备 ${BLUEPRINTS.length} 张 + 舰船 ${SHIP_BLUEPRINTS.length} 张`,
);
if (warn.length > 0) {
  console.log("· 警告：");
