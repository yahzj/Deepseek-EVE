...
/**
 * ⚠ **本表是 2026-09-13 那批的生成输入，之后有过追补，表内数值不再等于现状**——
 * 2026-09-14（船长「可以」）：① 鱼雷舰强化 —— 构件 `powerBonus` 0.25→**0.50**、亡军 0.62→**0.82**，
 * 亡军另付 `hitBonus` 0.21→**0.11** · `evasion` 0.105→**0.005**、槽位 5/5/3→**6/4/2**；
 * ② 槽位基准线核账（专属 = 该档默认 7/9/11 +1 ⇒ 8/10/12）——掠袭电子舰 `{mid:3}`→**4**、机库 `{mid:4}`→**3**、
 * 亡军后勤 `{low:3}`→**2**、巨构 `{mid:5}`→**4**、亡军鱼雷舰 `{5,5,3}`→**{6,4,2}**。
 * 与 `ships.ts` 冲突时**一律以 `ships.ts` 为准**（本工具带"检测到 `subClass` 即拒绝"的护栏，不能重跑）。
 */
/** 子分类（键 = 舰船 id） */
