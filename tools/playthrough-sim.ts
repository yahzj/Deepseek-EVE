 ...
 *
 * ⚠ **2026-09-21 从 120 提到 600**：实测 40 天档仍然**打满 120 次**（读数 `尝试 120 次`），
 * 而且全部集中在第 10~15 天用完 ⇒ 之后 25 天**一趟都不再进洞**、白白浪费后半程
 * （层深 4 就是在那 120 趟里打出来的，若不封顶本可以继续往层深 5 推）。
 * 每一趟本身很便宜（~0.05 游戏天），真正的限制是游戏时间 ⇒ 上限只用来防死循环。
 */
const WH_MAX_TRIES = 600
