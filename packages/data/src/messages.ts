...
export const COMMS_MESSAGES: readonly CommsMessageDef[] = [
  ...
  },
  {
     ...
     *   钉住——机动降到一成、推进器全数熄火、闪避失效、武器射程缩短 500 米，**本场不会自己松开**；
     *   唯一的解除办法是**击沉那艘电子舰**。不写数值推导与开发词（按 §5 / §12）。
     */
    ...
    body: [
      ...
      '被网住的那一艘会同时吃四种亏：机动掉到只剩一成、推进器全部熄火、闪避彻底失效、武器射程被压短 500 米。网一旦罩上，本场不会自己松开。',
      '好在它整场只张一次网，而且只罩得住当时被它锁定的那一艘——网是拴在它自己身上的：把它击沉，网立刻松开，被罩住的舰船当拍就能恢复正常。',
  ...
  },
  {
     ...
     * - 正文口径全部取自引擎现状：黑匣出自**入侵旗舰**（击毁必掉 ×1，也可打捞自己的残骸换回）；
     *   插件装在舰船上**不可拆卸、不可替换**，装了插件**不能入舰船仓库、不能挂卖**；
     *   图纸要用**协会声望**在章鱼人那里换；每件插件制造要吃掉**一个黑匣**。
     *   不写数值推导与开发词（按 §5 / §12）。
     */
    ...
    body: [
      '你手里那枚墨潮旗舰黑匣，工业协会收。凭它可以兑换「舰船插件」的制造图纸——组装机的门类里已经为你开出了这一档。',
      '插件像装备一样装在舰船上，但装上去就拆不下来、也换不了别的，所以下单之前想清楚要哪一件。装了插件的舰船不能放进舰船仓库，也不能挂到市场上卖。',
      '图纸不卖钱，要用协会声望换：完成入侵之后按你的贡献结算声望，声望攒着别急着花，一张图纸要价不低。学会图纸之后，每造一件插件都要再交出一枚黑匣。',
      '黑匣的来源有两条：击毁入侵旗舰必定掉落一枚；另外，打捞自己沉船的残骸时，船上原本装着的插件会按件数给你换回黑匣。',
    ],
