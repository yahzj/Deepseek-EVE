...
export const COMMS_MESSAGES: readonly CommsMessageDef[] = [
  ...
  },
  {
    ...
    body: [
      '刚才那次撤离不是故障。你的船在低安空域挨了打——装甲或结构掉到一半以下时，它会按保命规矩自动脱离交火；如果随船与仓库里的修理组件都补不动，它才会收手返港待命，等你回来处理。',
      '规矩一直是这两条：结构掉到一半以下自动脱离，绝不弃船；挨过打之后，船会自己拿随船（货舱优先、仓库兜底）的修理组件把装甲与结构补到约六成——补得动就留在原地继续干活，补不动才回家。',
      '想让它少回家，就把修理组件当消耗品带着：民用修理组件（一批 5 枚、每枚补得少）与军用修理组件（一批 3 枚、每枚补得多）都能在工业页自制，图纸市场有售。去低安之前给货舱或仓库留一批，被打完它自己就能站起来接着干。',
      '断料是被送回家的唯一原因——在低安跑久了，修理组件就是你的第二层装甲。',
    ],
    trigger: { kind: 'ambushRetreat' },
    hint: { text: '工业页 · 造一批修理组件随船带着', page: 'industry' },
  },
  {
    /**
