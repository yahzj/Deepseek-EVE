...
export const COMMS_MESSAGES: readonly CommsMessageDef[] = [
  ...
  },
  {
     ...
     *
     * - 触发 = `{ kind: 'ambushRetreat' }`（随档一次性标记 `state.ambushRetreatSeen`，两处置位：
     *   被袭船自动收手返港待命 · 应战中途结构过半自动脱离交火）——**老档不追溯**，等下次遇袭撤离时发；
     * - 弹窗 = **默认弹**（2026-09-14 船长：「所有除新手教程外的讯息也弹窗」）；点「知道了」关掉，收件箱留一份；
     * - 跳转 = **工业页**（船长选定：去造修理组件）；
