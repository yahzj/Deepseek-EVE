...
export const COMMS_MESSAGES: readonly CommsMessageDef[] = [
  ...
  {
    /**
     * **虫洞扫描解锁**（2026-09-14 船长：「**扫码虫洞需要玩家35声望才会解锁。解锁时发送通讯给玩家
     * （同时也要直接弹窗）**」）。
     *
     * - 触发 = **协会声望 ≥ 35**（`{ kind: 'standing', factionId: 'dsi', min: 35 }`）——
     *   门槛常量在 core 的 `WORMHOLE_SCAN_UNLOCK_STANDING`（两处必须同值，改一处要一起改）；
     * - `popup: true` ⇒ 送达那一刻**同时弹窗**（玩家点「知道了」关掉，收件箱里照样留一份）；
     * - **`unreleased: true`（施工期闸门）**：虫洞上线前这封信**不送达** —— 与"入口走调试开关、
     *   数据走 unreleased"同一条铁律；**上线时删掉这一个字段**即可开送（文案已按正式口径写好）。
     */
    ...
    body: [
      '协会测绘处通知你：你在协会的声望已经够高，可以接入深空裂隙扫描阵列了。',
      '阵列装在船上，扫的是星域之间那些不稳的裂隙——扫满一个窗口就能标出一处可进入的裂隙。标出来的裂隙会先存着，最多同时存五处，等你决定什么时候去。',
      '扫描期间照常会遇上航线上的那些事，遇袭也不影响进度：阵列自己会接着扫。',
      '要开始扫描，去星图「出港」页的「扫描虫洞」标签；标出的每一处都带一个起始深度，越深越险、带回来的东西也越多。',
    ],
    trigger: { kind: 'standing', factionId: 'dsi', min: 35 },
    popup: true,
    unreleased: true,
