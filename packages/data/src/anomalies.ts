 ...
 * 一律清空（26 张里原有 20 张带附赠：19 张是矿物、1 张是原矿橄榄岩×150，合计 ≈19.3 万 信用点
...
const ANOMALIES_BASE: readonly AnomalyDef[] = [
  ...
  ...WORMHOLE_FOE_CARDS,
  // 周末入侵的独立敌卡（2026-09-24 · M2 逐族铺开）：同样 `hidden: true`，只由入侵活动取用
  // （见 `wormholeFoes.ts` 末段的 `WEEKEND_FOE_CARDS` 与 core 的 `weekendFoeCardOf`）。
  ...WEEKEND_FOE_CARDS,
  {
    id: 'ano-training',
  ...
  },
  {
    ...
    //   巡洋条目 52 ＋ 两艘驱逐条目 29×2；巡洋条目再拆出机群 **24**（2 架各 12）⇒
    //   **本卡实收总单发仍是迁移前的 138**（炮 114 ＋ 机 24）。锚点写在**巡洋那一条目**上
    //   （`firepowerAnchor` / `droneFireShare` 是**条目级**字段，不是卡级）。
    //   两台僚位**不挂机群**（蜂群只在巡洋舰那条），故只有巡洋条目写占比/锚点。
    // ⚠ 占比 **0.30** = 船长 2026-09-12「**G族无人机仅占30%**」（原按我提议落 0.35 ⇒ 已按裁决下调）。
    ships: [
      {
