 ...
 * | 噬口巨兽 | **4 战列舰**（205） | `1.35` | **277** | 噬口猎杀令 80（**精锐 · 全族唯一**，开**冲锋**） |
 *
 ...
 * 速度 = **4 战列舰基准 205 × `1.35` = 277**（船长 2026-09-11「**巨兽提速降为 1.35**」，
 * 并**给巨兽开启之前做过的冲锋能力**补偿其慢——`foeCanCharge: true`，
 * 见 `combat.updateFoeCharge` 与 `docs/design/foe-faction-c-alien-20260911.md` §十）。
 * 射程带 **1~4000**（船长「**巨兽射程增加**」；原 1~2713）——⚠ **期望交距与开战距离都不动**：
 * 那两个量由「波 0 编制的首个单位」即**畸变幼虫**的带（1~2713）锚定，巨兽排在末波之后，
 * 故它「多打得到」而「全队想站的位置仍是 543 m」（见设计稿 §十 三）。
...
export const FOE_ALIEN_MAW: FoeShipDef = {
  ...
  speedRatio: 1.35, // = 277 / 205（船长「巨兽提速降为 1.35」；T4 仍是族内唯一破例的慢档）
  ...
  reloadMs: 4000,
  rangeMinM: 1,
  rangeMaxM: 4000, // 船长「巨兽射程增加」（原 2713）；**不动**期望交距/开战距离（见上方 ⚠）
  falloff: 0.5,
  blindDmgMul: 0.3,
