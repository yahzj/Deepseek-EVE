...
export const FOE_MISSILE_HULK: FoeShipDef = {
   ...
   * 与 D 族「静滞卫舰」**同一件、同一倍率**（`foe-mount-gun-range-x1-5` ⇒ 从它射程之外被命中，本舰炮台射程 ×1.5）。
   *
   * 为什么要给它：导弹残段是 E 族里唯一的**远射平台**（11,000m），玩家在它射程外"站着点"就是它的天敌；
   * 挨打增程让它把射程反推回来（11,000 → 16,500m），与 D 族静滞卫舰同一套反制语言。
   * ⚠ 归属判据见 `content:check` 的「炮台受击增程」契约：原先**只允许静滞卫舰**，本批按船长指令扩到本舰级。
   */
  mounts: [FOE_MOUNT_IDS.gunRangeX15],
}
