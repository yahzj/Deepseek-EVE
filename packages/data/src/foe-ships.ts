 ...
 * | 射程 · 战术 | **100~15,000m** · `orbit` | 船长指定；**玩家武器上限约 7,350（动能）~9,600（E 族专属件）** ⇒ 必须顶着满效火力接近 |
 * | 构成 | **纯爆炸**（`{ explosive: 10 }`） | 船长「伤害为100%纯爆炸」；⇒ 已登记 `FOE_SHIP_MIX_AUTHORITY_IDS`（白名单） |
 * | 命中 | **0.85** | 制导修正（船长指定）；E 族内另有 0.65（老化失准）与 0.95（奥罗）⇒ 0.85 属族内既有区间 |
...
export const FOE_MISSILE_HULK: FoeShipDef = {
  ...
  falloff: 1, // **导弹：命中不随距离衰减**
  blindDmgMul: 1, // **导弹：无视近盲带**（近身也满效）
  dmgMix: { explosive: 10 }, // 船长「伤害为100%纯爆炸」
  tactic: 'orbit',
}
 ...
 *
 * 由来：2026-09-15 船长两处点名要的口径**与既有混伤契约冲突**，他裁定「**撞到的契约开白名单**」：
 * - `foe-d-throne`（守墓王座舰）= **等离子 6 : 动能 4**（原话「50%动能+50%能量的混合炮台」→「按乙调整为 6:4」）；
 * - `foe-missile-hulk`（导弹残段，批 4）= **纯爆炸**（原话「伤害为100%纯爆炸」）。
 *
