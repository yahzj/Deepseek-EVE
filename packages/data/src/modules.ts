...
export const MODULES: readonly ModuleDef[] = [
   ...
   * 掉落链路 = H 族稀有残骸（`wreck-rare-h-hi`）开**高级箱** ⇒ 池 `FOE_LAIR_GEAR.H`（**5%／箱** ·
   * 集齐前不重复）。重袭机是**无人机物品**（`data/items.ts`，一次 ×10 架），另两件是本表的模块。 */
  {
    ...
    description:
      '缴获改装的对舰电子战核心：装在参战编队里就持续压制敌方武器射程 15%，与电子舰的削弱乘法叠加。占用 150 点 CPU——装得下它的船，只为它服务。',
  },
  {
    id: 'mod-lair-web-h',
    name: '墨潮捕获网',
    slot: 'support',
    rack: 'high',
    // 周期装置（船长：和武器一样有冷却周期）：3800 米内张网 → 目标被击沉 / **超距 4500 米断开**（两者都算一次使用）⇒ 冷却 → 再选新目标
    captureWebCycleMs: 20_000,
    cpuUse: 44,
    description:
      '周期张网：3800 米内钉住一艘未被钉住的敌舰，机动 ×0.5、推进器全关、闪避归零；与目标距离超过 4500 米、或目标被击沉时即断开并冷却 20 秒；本舰被击沉则解除。',
  },
  ...
  },
  {
    ...
    description:
      '塔状的机群指挥中枢：放飞无人机单发 +50%。无人机套系缺的从来不是数量，是让它们打得疼。',
  },
  {
    id: 'mod-wh-e-cpu',
    name: '巨构协处理器',
