...
export const MODULES: readonly ModuleDef[] = [
      ...
      引擎口径见 core/types.ts 的 `ModuleDef.stealthMs` 与 combat 的 `isMyUnitTargetable`。 ═══ */
  {
    ...
    stealthMs: 20_000,
    description:
      '高槽隐秘装置：本舰武器开火之前保持隐身——敌方无法锁定、也无法攻击，最长 20 秒；本舰一旦开火立即现形，超时同样现形。装着推进器时装置直接失效（船一动就藏不住）。占用 55 点 CPU，压得下它的多是巡洋以上的舰体。',
  },
  {
    id: 'mod-stealth-3',
    name: '隐秘行动装置 MK3',
   ...
   * 窝点套已占：转管炮（动能近程）・导弹巢（爆破中程）・赃物强化舱（货舱+甲容量） */
  {
    ...
    //   第二段伤害，见设计稿 §3.6「待落机制」。
        description:
      '会自己炸开的破片弹：7.3 km 内撒出一片高爆碎片，单发是攻坚炮台 MK3 的 1.37 倍；碎片不看装甲缝隙——额外造成 50% 的动能伤害（与所耗的爆破弹药无关）。代价：命中只有 0.8，装填 5.6 秒。',
  },
  {
    id: 'mod-wh-a-hangar',
