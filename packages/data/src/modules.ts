...
export const MODULES: readonly ModuleDef[] = [
  ...
  },
  {
    ...
    cpuUse: 36,
    description:
      '海盗窝点专属：掠袭艇拆下来的近程导弹巢——贴到脸上齐射爆破弹，追踪命中不随距离衰减、近盲安全射距仅 200 m；射程只有巡航导弹架的三成，靠的是"贴上去就有量"。',
  },
  {
    id: 'mod-lair-cargo-a',
    name: '赃物强化舱',
  ...
  },
  {
    ...
    cpuUse: 30,
    description:
      '异形生物窝点专属：层叠生体甲壳——装甲层三系减伤各 +10%，并自带**无消耗自愈**：战斗中每 5 秒自动修复 6 点装甲（不吃组件）。单系抗性远不如专精镀层，胜在能自己长回来。',
  },
  {
    id: 'mod-lair-dc-c',
    name: '生体损管腔',
  ...
  },
  {
    ...
    cpuUse: 34,
    description:
      '异形生物窝点专属：腺体加压的酸液喷吐——光束级必中、无近盲；相对原型走"射速更慢、单发更重"的路线（单发略高于攻坚激光炮、装填更短，总输出相当）。射程只有 2.8 km，必须贴上去喷。',
  },
  {
  ...
  },
  {
    ...
    cpuUse: 62,
    description:
      '守墓古舰窝点专属：陵寝守备炮——**基础命中 100%**（必中量级）、12 km 超远程点名，射速只有攻坚炮台的四成、单发威力是它的两倍（总输出 ≈ 攻坚炮台的八成）；远端命中衰减也更轻（射程尽头仍有六成命中）。装填极慢，打空一发就是七秒。',
  },
  {
    id: 'mod-lair-armor-d',
    name: '陵寝装甲层',
    ...
    cpuUse: 42,
    description:
      '守墓古舰窝点专属：陵寝级复合重甲——装甲容量 **+110%**（比装甲增厚板 MK3 还厚四成），代价是**战斗机动速度 −25%**（多件不叠加）与 42 点 CPU：装它等于少一门重炮、也跑不快。守墓者从来不需要追人。',
  },
  {
  ...
  },
  {
    ...
    cpuUse: 34,
    description:
      '鱿烬亡军窝点专属：用废桅杆拼起来的中继阵——放飞无人机射程 **+65%**（蜂鸟 4125 m / 赤鸢 4950 m / 猎鹰 5775 m / 雷鸥 8250 m）。比无人机中继天线 MK3 近一档，却省下 12 点 CPU。',
  },
   ...
   * 窝点套已占：转管炮（动能近程）・导弹巢（爆破中程）・赃物强化舱（货舱+甲容量） */
  {
    ...
    //   第二段伤害，见设计稿 §3.6「待落机制」。
        description:
      '会自己炸开的破片弹：**7.3 km 内**撒出一片高爆碎片，单发是攻坚炮台 MK3 的 1.37 倍；碎片不看装甲缝隙——**额外造成 50% 的动能伤害**（与所耗的爆炸弹无关）。代价：命中只有 0.8，装填 5.6 秒。',
  },
  {
    id: 'mod-wh-a-hangar',
    ...
    //   （只有 bay/dmg/range 三件），需新增字段（建议 `droneIntervalCutPct`）+ 战斗侧接入，见设计稿 §3.6。
        description:
      '把抢来的货舱隔板焊成的机库夹层：无人机舱 **+30 m³**，且**放飞无人机的出击周期 −8%**。装得下又放得快，代价只有 25 点 CPU——是本套最省的一件。',
  },
  {
    id: 'mod-wh-a-prop',
    name: '掠袭加力器',
  ...
  },
  {
    ...
    cpuUse: 34,
        description:
      '一层会骗测距的涂层：被命中缺口再削 **28%**（姿态陀螺 MK3 是 20%），代价是**全抗性 −15**——盾、甲、结构三层一起变脆。保命也保货，但别指望它扛。',
  },
  {
    id: 'mod-wh-a-scan',
    name: '赃物扫描阵',
    ...
    cpuUse: 36,
        description:
      '拼装起来的火控阵列：炮台命中整体 **×1.24**（索敌阵列 MK3 是 ×1.16），代价是**武器射程 −15%**——看得更准，但得让对方更靠近。',
  },
  {
    id: 'mod-wh-a-shield',
    ...
    //   需新增（建议 `rangeCutPct`，多件取最重一件）+ 战斗侧接入，见设计稿 §3.6。
        description:
      '把三块抢来的护盾发生器串成一个笼：护盾容量 **+80%**（护盾扩展器 MK3 是 +60%）。代价是**武器射程 −25%**：护盾笼挤占了炮座的位置。',
  },
  {
    id: 'mod-wh-c-laser',
    name: '生体棱镜束',
  ...
  },
  {
    ...
    cpuUse: 42, // 船长审核：32 → 42（CPU +10）
        description:
      '以生体脉搏驱动装填链：炮台装填间隔 **÷1.06**（射速计算机 MK3 是 ÷1.12），并让**舰船速度 +10%**。占用 42 点 CPU——打得快，也跑得快。',
  },
  {
    id: 'mod-wh-c-missile',
    name: '孢子导弹巢',
  ...
  },
  {
    ...
    cpuUse: 46,
        description:
      '把整副骨架换成几丁质复合层：结构层容量 **+75%**（巨构骨架是 +60%），并让**舰船速度 +5%**——壳更厚，却更轻。护盾与装甲被打穿之后，最后那段血就靠它。',
  },
   ...
   * 窝点套已占：陵墓护盾阵列（三系盾抗）・守墓者长炮（动能必中 12 km）・陵寝装甲层（甲容量） */
  {
    ...
    cpuUse: 46,
        description:
      '陵卫的连装炮：5.4 km 内 2.8 秒一轮，单发是攻坚炮台 MK3 的 1.79 倍——**一轮打出两发弹药**。长炮点名硬目标，它负责把贴上来的一群清掉。',
  },
  {
    id: 'mod-wh-d-shield',
    name: '陵墓护盾芯',
  ...
  },
  {
    ...
    cpuUse: 40,
    description:
      '锁定即宣判：被本舰锁定的目标**受击加深 30%**（目标锁定阵列 MK3 是 20%），且全舰武器转为集火同一目标——先敲最硬的那一个。',
  },
  {
    id: 'mod-wh-d-laser',
    name: '陵寝棱镜炮',
  ...
  },
  {
    ...
    cpuUse: 0, // 协处理器口径：自身不占 CPU
        description:
      '巨构造物的并行计算核心：装配 CPU 上限 **+90**，自身不占 CPU，代价是**全舰装填 +12%**——算力是借来的，得用射速还。',
  },
  {
    id: 'mod-wh-e-pd',
    name: '巨构近防阵列',
  ...
  },
  {
    ...
    cpuUse: 36,
        description:
      '从沉船里捡回的火控残骸，还在按老参数工作：炮台命中整体 **×1.12**，并让**全部武器单发 +6%**。它记得上一任主人的射击习惯。',
  },
  {
    ...
    cpuUse: 60, // 船长审核：30 → 60（CPU +30）
        description:
      '一把没有主人的弹道仪：**动能武器单发 +22%**，并把**动能武器的射程拉长 22%**。占用 60 点 CPU——亡军的炮都是捡来的动能炮，校正器也只好认这一系。',
  },
  {
    id: 'mod-wh-g-hull',
    name: '残兵结构层',
