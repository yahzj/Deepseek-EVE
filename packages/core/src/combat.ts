 ...
 * 拉不开的成因是**机动拔河**——C 族畸变/星髓幼虫 544 m/s（冲锋 ×1.5 ⇒ 816）、星髓成虫 398，
 * 我方 T3 巡洋基础仅 272（推进 MK2 点火 435 / MK3 544）⇒ 推力永不占优；且**冲锋触发线
 * = 敌期望交距（C 族 532）+ 1,000 = 1,532 m** 正好卡在"玩家想拉开"的方向上（往外一拉就全队冲锋）。
 * 洞内比洞外更刺眼：近战怪按 2026-09-13 规则**开局就站在玩家中距档（3,220 m）**，洞内敌卡血/火力约 ×4，
 * 贴住后我方主炮（动能炮近界 700 m）落在**自己的近盲带**里不发炮（近盲带开火是 V18B 给敌方的特权）。
 * **船长 2026-09-16 裁定：戊案——现状即设计，不改**；换快船可拉开（实测最快档 864 m/s 能把距离
 * 稳在 4,690 m 且全程零挨打）⇒ 对策在配船与信息提示，不在改机制。
 *
