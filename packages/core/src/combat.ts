 ...
 */
export function typeLayerMult(
  t: DamageType,
  layer: "shield" | "armor" | "hull",
): number {
  if (t === "kinetic")
    return layer === "shield" ? 1.5 : layer === "armor" ? 0.5 : 1;
  if (t === "explosive")
    return layer === "shield" ? 0.5 : layer === "armor" ? 1.5 : 1;
...
export function hitChance(
  ...
  return clamp(
    bal.hitMin,
    bal.hitMax,
    raw * (weapon.eqHitMul ?? 1) * (attacker.hitMul ?? 1),
  );
}
...
export function applyDamage(
  ...
  const layerName: Array<"shield" | "armor" | "hull"> = [
    "shield",
    "armor",
    "hull",
  ];
  ...
  for (let i = 0; i < 3 && rest > 0; i++) {
    ...
    const layerDmg =
      rest * typeLayerMult(type, layerName[i]!) * (1 - clamp(0, 0.9, res));
...
export function applyFoeShot(
  ...
  const entries = shots
    ? Object.entries(shots).filter(([, v]) => (v ?? 0) > 0)
    : [];
  if (entries.length <= 1)
    return applyDamage(hp, resists, totalDmg, mainType).hp;
  ...
  entries.forEach(([t, v], i) => {
    const dmg =
      i === entries.length - 1
        ? left
        : Math.max(1, Math.round((totalDmg * (v ?? 0)) / sum));
...
function combatSpeed(
  ...
  return Math.max(
    ...
    maxSpeedMps *
      bal.speedFactor *
      (1 + (agility - 0.5) * 2 * bal.agilitySpeedBonus),
  );
}
 ...
 */
export function effectiveHitMul(
  spec: Pick<UnitSpec, "hitMul">,
  boosting: boolean,
): number {
...
export function thrusterPhase(
  battle: Pick<
    ...
    "lastTickGameMs" | "startedAtGameMs"
  >,
  bal: BattleBalance,
  ...
  return {
    ...
    posMs,
    cycleMs,
    remainMs: boosting ? bal.thrusterBoostMs - posMs : cycleMs - posMs,
  };
}
 ...
 * 对无基础层 = 模块值直接成面板）。
 */
export function mergeResist(
  base: DamageResists | undefined,
  add: DamageResists | undefined,
): DamageResists {
...
export function foeDamageComposition(
  anomaly: AnomalyDef,
): Array<{ type: DamageType; share: number }> {
...
export function splitShotByComposition(
  ...
  if (shotDmg <= 0) return [];
  if (comp.length <= 1)
    return [{ type: comp[0]?.type ?? "kinetic", dmg: shotDmg }];
  const out: Array<{ type: DamageType; dmg: number }> = [];
  let left = shotDmg;
  comp.forEach((c, i) => {
    const dmg =
      i === comp.length - 1 ? left : Math.max(1, Math.round(shotDmg * c.share));
...
export function foeLayerSplit(profile: DefProfile | undefined): {
  s: number;
  a: number;
  h: number;
} {
...
export function createPlayerSpec(
  ...
  const supportDefs = allFittedModules(fitted, ctx).filter(
    (d) => d.slot === "support",
  );
  // 无人机装置（高槽 rack 件；甲板扩展/战术导控/中继天线按字段判别）
  const droneGear = allFittedModules(fitted, ctx).filter(
    (d) =>
      d.droneBayBonusM3 !== undefined ||
      d.droneDmgBonus !== undefined ||
      d.droneRangeBonusPct !== undefined,
  );
  ...
  let hullHpMult = 1;
  for (const m of allFittedModules(fitted, ctx))
    hullHpMult += m.hullHpBonus ?? 0;
  ...
  const armoredOpsLv =
    ship.role === "armored"
      ? Math.min(5, state.skills.trained["armored-ops"] ?? 0)
      : 0;
  ...
  for (const m of allFittedModules(fitted, ctx))
    if (m.hullResistAdd) applyAdds(hullRes, m.hullResistAdd);
  ...
  const dmgBonus: Record<DamageType, number> = {
    kinetic: 0,
    explosive: 0,
    plasma: 0,
  };
  ...
  for (const m of supportDefs) {
    for (const [t, v] of Object.entries(m.damageTypeBonusPct ?? {}))
      dmgBonus[t as DamageType] += v ?? 0;
  ...
  const worstSpeedPen = Math.max(
    0,
    ...allFittedModules(fitted, ctx).map((m) => m.speedPenaltyPct ?? 0),
  );
  ...
  const gunneryLv =
    state.skills.trained[ctx.balance.combat.gunnerySkillId] ?? 0;
  // 批次五：武装舰操作（armed 族驾驶 +3%/级 全武器单发，乘于炮术学之外）
  const arOpsLv =
    ship.role === "armed"
      ? Math.min(5, state.skills.trained["armed-ops"] ?? 0)
      : 0;
  const dmgScale =
  ...
  for (const group of gunGroups.values()) {
    ...
    if (turret.maxRangeM === undefined || turret.reloadMs === undefined)
      continue;
    ...
    const famLv =
      famKey !== null
        ? Math.min(
            5,
            state.skills.trained[ctx.balance.battle.familySkillIds[famKey]] ??
              0,
          )
        : 0;
    let famMult =
      famLv > 0 ? 1 + ctx.balance.battle.familySkillPerLevel * famLv : 1;
    ...
    for (const [droneId, want] of Object.entries(droneLoad)) {
      ...
      const shot = Math.round(
        (def.dmg ?? 0) *
          ...
          (1 +
            DRONE_SKILL.warfarePerLevel *
              droneSkillLv(state, "drone-warfare")) *
          (1 +
            DRONE_SKILL.strikePerLevel * droneSkillLv(state, "drone-strike")),
      );
      for (let i = 0; i < n; i++) {
        weapons.push({
          label: def.name,
  ...
  return {
    ...
    hitBonus:
      ...
      (1 +
        bal.hitPerLevel *
          Math.min(5, state.skills.trained[bal.hitSkillId] ?? 0)),
    ...
    speedMps:
      ...
      (1 +
        bal.speedPerLevel *
          Math.min(5, state.skills.trained[bal.speedSkillId] ?? 0)) *
...
export function playerAmmoType(
  state: GameState,
  ctx: SimContext,
  shipId: string,
): DamageType {
...
export function foeHpOfThreat(threat: number, bal: BattleBalance): number {
  ...
  const d =
    (bal.foeHpCurveDMin ?? 5) +
    (bal.foeHpCurveDSpan ?? 85) * Math.pow(t, bal.foeHpCurveExp ?? 1.6);
...
export function foeClassName(
  tactic: string | undefined,
  profile: string | undefined,
): string {
  return (
    FOE_CLASS[tactic ?? "orbit"]?.[profile ?? "balanced"] ?? FOE_CLASS_FALLBACK
  );
}
...
function foeShipAtTag(
  anomaly: AnomalyDef,
  tag: string,
): { ship: FoeShipDef; escort: boolean } | null {
...
export function foeUnitNameOf(anomaly: AnomalyDef, tag: string): string {
  ...
  if (waves && waves.length > 0) {
    ...
    if (waveHpShareOf(tag, anomaly) / maxShare <= FOE_LIGHT_FRAC)
      return `${FOE_LIGHT_WORD}${base}`;
  }
  return base;
}
...
function foeMultiShipCompMul(anomaly: AnomalyDef): number {
  const n = (anomaly.ships ?? []).reduce(
    (s, x) => s + Math.max(1, Math.floor(x.count ?? 1)),
    0,
  );
 ...
 */
function createFoeSpecsFromShips(
  anomaly: AnomalyDef,
  bal: BattleBalance,
  opts: FoeSpecOpts,
): UnitSpec[] {
  ...
  return enumerateShipUnits(anomaly, waveIdx).map((u) => {
    ...
    const shotDmg = Math.max(
      1,
      Math.round(ship.shotDmg * (u.slot.dmgMul ?? 1) * comp),
    );
    ...
    const rangeMax = Math.max(
      2,
      u.slot.rangeMaxM ?? Math.round(ship.rangeMaxM * rangeMul),
    );
    const rangeMin = Math.max(
      1,
      Math.min(
        rangeMax - 1,
        u.slot.rangeMinM ?? Math.round(ship.rangeMinM * rangeMul),
      ),
    );
    ...
    const energyForm =
      (u.slot.energyForm ?? ship.energyForm) === "spit"
        ? ("spit" as const)
        : ("beam" as const);
    ...
    return {
      ...
      speedMps: Math.round(
        bal.hullClassBaseSpeedMps[ship.hullClassTier] *
          ship.speedRatio *
          (u.slot.speedMul ?? 1),
      ),
      agility: 0.3,
...
function normReinforceTrigger(
  at?: FoeReinforceTrigger,
): FoeReinforceTrigger | null {
  if (!at) return null;
  const out: FoeReinforceTrigger = {};
  if (typeof at.sec === "number" && Number.isFinite(at.sec) && at.sec > 0)
    out.sec = at.sec;
  if (
    typeof at.afterKills === "number" &&
    Number.isFinite(at.afterKills) &&
    at.afterKills > 0
  ) {
    out.afterKills = Math.floor(at.afterKills);
  }
  if (typeof at.hpBelow === "number" && at.hpBelow >= 0 && at.hpBelow <= 1)
    out.hpBelow = at.hpBelow;
  return Object.keys(out).length > 0 ? out : null;
}

/** 本波编成（`curFoes` 全部条目）的**存活剩余总血比例**：分子 = 在场且未全灭的单位的剩余三层血之和，
 *  分母 = 本波编成的**满血总量**（含尚未入场的增援——"残部呼救"要按编制总量看，不是按在场量看）。 */
function foeResidualHpFrac(
  b: import("./state").BattleState,
...
function reinforceTriggered(
  ...
  if (
    at.sec !== undefined &&
    b.lastTickGameMs - b.startedAtGameMs >= at.sec * 1000
  )
    return true;
  if (at.afterKills !== undefined && foeKillCount(b) >= at.afterKills)
    return true;
  if (at.hpBelow !== undefined && foeResidualHpFrac(b, curFoes) <= at.hpBelow)
    return true;
  return false;
}
...
function resolveReinforcements(
  ...
  if (b.pdCd && b.dronePools) {
    for (const spec of arrived) {
      ...
      if (i >= 0 && i < b.pdCd.length)
        b.pdCd[i] = Math.max(100, Math.round(bal.pdJudgementMs));
    }
  }
...
export function createFoeSpecs(
  anomaly: AnomalyDef,
  bal: BattleBalance,
  opts: FoeSpecOpts = {},
): UnitSpec[] {
  ...
  if (anomaly.ships && anomaly.ships.length > 0)
    return createFoeSpecsFromShips(anomaly, bal, opts);
  const tactic = anomaly.tactic ?? "orbit";
  const split =
    PROFILE_SPLIT[anomaly.defProfile ?? "balanced"] ?? PROFILE_SPLIT.balanced!;
  ...
  const rangeMax = Math.min(
    cap,
    Math.round(rangeBase.max * (1 + growMul * growT)),
  );
  const rangeMin = Math.max(
    1,
    Math.min(rangeMax - 1, Math.round(rangeBase.min * (1 + growMul * growT))),
  );
  ...
  const foeSpeed =
    ...
    Math.min(
      spdCap,
      Math.round(refSpeed * foeSpeedBase(anomaly.threat, bal) * tacticSpd),
    );
  ...
  const make = (
    ...
    const baseHp =
      (anomaly.foeHpOverride ?? foeHpOfThreat(anomaly.threat, bal)) * hpShare;
    ...
    const effHit =
      type === "plasma" ? 1 : (anomaly.foeHitRate ?? bal.foeHitRate);
    ...
    const shotDmg =
      ...
      Math.max(
        1,
        Math.round(
          ((dps * bal.foeReloadMs) / 1000) *
            (effHit < 1 ? bal.foeHitCompMul / effHit : 1),
        ),
      );
    // 2026-09-10 船长（窝点混伤）：按火力构成拆成逐系单发（Σ = 总单发，敌总伤不变）。
    // 纯系卡只有一条 → 与旧行为完全一致；混伤卡 = 主 60% / 副 40% 两键。
    const shotSplit = splitShotByComposition(
      shotDmg,
      foeDamageComposition(anomaly),
    );
    const multiShots: Partial<Record<DamageType, number>> | undefined =
      shotSplit.length > 1
        ? shotSplit.reduce<Partial<Record<DamageType, number>>>((acc, r) => {
  ...
  for (let k = 0; k < waveUnits; k++) {
    ...
    for (let i = 1; i <= escorts; i++) {
      const escortTag = legacySquad
        ? `foe-${i}`
        : `${squadPrefix}foe-${k}-e${i}`;
      specs.push(
        make(
          escortTag,
          foeUnitNameOf(anomaly, escortTag),
          mainThreat * 0.6,
          mainType,
        ),
      );
...
export function battleOpenM(
  ...
  for (const f of foes)
    for (const w of f.weapons) top = Math.max(top, w.maxRangeM);
  const pad = Math.max(
    bal.openRangePadM,
    Math.round(top * (bal.openRangePadShare ?? 0.1)),
  );
...
export function desiredRangeFor(
  ...
  if (tactic === "assault")
    return Math.max(bal.minDistanceM, Math.round(mainMin * 0.6));
  if (tactic === "kite")
    return Math.max(bal.minDistanceM + 1, Math.round(mainMax * 0.95));
  // mid：有效射程中点
...
export function foeDesiredRange(
  ...
  return Math.max(
    bal.minDistanceM,
    Math.round(band.min + pos * (band.max - band.min)),
  );
}
...
export function ammoLoadTotals(
  ...
  const condLv = Math.min(
    5,
    state.skills.trained["ammunition-condensing"] ?? 0,
  );
  ...
  const addFor = (t: DamageType, reloadMs: number, volley: number): void => {
    const volleys = Math.max(
      1,
      Math.ceil(
        ((bal.ammoTimeCapMs * condFactor) / Math.max(100, reloadMs)) *
          bal.ammoMargin,
      ),
    );
  ...
  for (const w of me.weapons) {
    if (w.kind === "gun") {
      const t =
        (Object.keys(w.shotsByType ?? {})[0] as DamageType | undefined) ?? null;
...
export function loadAmmo(
  ...
  type: DamageType,
  total: number,
): { kin: number; exp: number; pla: number } {
...
function loadAmmoOf(
  ...
  const stock = Math.floor(
    (cargoItemsOf(state)[id] ?? 0) + countWare(state, id),
  );
  if (stock <= 0) return 0;
...
export function loadAmmoTier(
  ...
  if (wantId !== null) {
    const have = Math.floor(
      (cargoItemsOf(state)[wantId] ?? 0) + countWare(state, wantId),
    );
...
export function nextAmmoType(ammo: {
  kin: number;
  exp: number;
  pla: number;
}): DamageType | null {
...
export function fittedRepairModules(
  state: GameState,
  ctx: SimContext,
  shipId: string,
): ModuleDef[] {
  ...
  if (!ship) return [];
  return allFittedModules(ship.fitted, ctx).filter(
    (d) => (d.repairArmorHp ?? 0) > 0 || (d.repairHullHp ?? 0) > 0,
  );
}
...
function pulseRepairs(
  ...
  for (const u of r.units) {
    ...
    if (u.free) {
      ...
      if (ag0 < u.armorPerPulse && hg0 < dh0)
        hg0 += Math.min(u.armorPerPulse - ag0, dh0 - hg0);
      if (hg0 < u.hullPerPulse && ag0 < da0)
        ag0 += Math.min(u.hullPerPulse - hg0, da0 - ag0);
    ...
    if (kitNow <= 0) {
      ...
      addLog(
        state,
        "warn",
        `🔧 ${modName}的${kitName}耗尽，自动停机——战斗中装甲/结构修复暂停。`,
      );
      continue;
    }
    ...
    if (ag < u.armorPerPulse && hg < dh)
      hg += Math.min(u.armorPerPulse - ag, dh - hg); // 甲通道剩余 → 结构
    if (hg < u.hullPerPulse && ag < da)
      ag += Math.min(u.hullPerPulse - hg, da - ag); // 结构通道剩余 → 甲
  ...
  r.pulses += 1;
  if (active === 0)
    r.nextPulseAtMs = undefined; // 全部停机：停调度
...
function seedUnit(
  ...
  b.units[spec.tag] = {
    ...
    weapons: opts.enterReload
      ? spec.weapons.map((w) => Math.max(1, w.reloadMs))
      : spec.weapons.map(() => 0),
  };
}
...
export function startBattleFor(
  ...
  const anomaly = battleAnomalyOf(
    ctx,
    anomalyId,
    state.expedition.lairTier,
    state.expedition.factionActive,
  );
  ...
  const waves =
    anomaly.waves && anomaly.waves.length > 0 ? anomaly.waves : null;
  const foes = waves
    ? createFoeSpecs(anomaly, bal, {
        units: waves[0]!.units,
        hpShare: waves[0]!.hpShare,
      })
    : createFoeSpecs(anomaly, bal);
  ...
  for (const [t, n] of Object.entries(totals)) {
    ...
    if (state.fleet[shipId]?.ammoPref?.[type] && res.loaded > 0)
      ammoIds[type] = res.id;
    if (res.fellBack && res.loaded > 0) {
      const wantName =
        ctx.items.get(state.fleet[shipId]!.ammoPref![type]!)?.name ?? type;
      const useName = ctx.items.get(res.id)?.name ?? type;
      addLog(
        state,
        "warn",
        `⚙ ${wantName}库存不足，本场改用${useName}（预载 ${res.loaded} 发）。`,
      );
    }
  }
  ...
  const durMul =
    (1 +
      DRONE_SKILL.durabilityPerLevel *
        droneSkillLv(state, "drone-durability")) *
    (1 +
      DRONE_SKILL.reinforcePerLevel * droneSkillLv(state, "drone-reinforce"));
  const evaMul =
    1 + DRONE_SKILL.evasionPerLevel * droneSkillLv(state, "drone-evasion");
  ...
  if (repair) {
    ...
    if (ready.length > 0) {
      ...
      for (const u of repair.units) {
        ...
        if (u.free) {
          parts.push(
            `${modName}（自愈：每跳修甲 ${u.armorPerPulse} / 结构 ${u.hullPerPulse}，无需组件）`,
          );
          continue;
        }
        const n = repair.kits[u.kitId] ?? 0;
        parts.push(
          `${modName}${u.stopped ? `（缺${ctx.items.get(u.kitId)?.name ?? u.kitId}停机）` : ` ×${n}枚组件`}`,
        );
      }
      addLog(
        state,
      ...
      addLog(
        state,
        "warn",
        `🔧 已装维修装置但货舱/仓库没有${kitName}——本场不会自动修复，请先补给。`,
      );
    }
...
export function battleZonesFor(
  state: GameState,
  ctx: SimContext,
): {
  openM: number;
...
export function battleArcsFor(
  ...
  maxHp: {
    me: { s: number; a: number; h: number };
    foe: Record<string, { s: number; a: number; h: number }>;
  };
  ...
  me.weapons.forEach((w, i) => {
    ...
    if (w.kind === "fixed") type = w.fixedType ?? "kinetic";
      ...
      type = battle.ammo.pla >= Math.max(1, w.count ?? 1) ? "plasma" : null; // 激光吃能量弹药键（按门数）
  ...
  for (const [tag, u] of Object.entries(battle.units)) {
    if (u.side !== "foe") continue;
    foeMaxHp[tag] = u.hpMax ?? {
      s: Math.max(0.001, u.hp.s),
      a: Math.max(0.001, u.hp.a),
      h: Math.max(0.001, u.hp.h),
    };
  }
  ...
  return {
    ...
    ...(battle.droneLost && Object.keys(battle.droneLost).length > 0
      ? { droneLost: battle.droneLost }
      : {}),
...
export function settleDroneLosses(
  ...
  let rest = Math.max(
    0,
    Math.round(total * rate) - rows.reduce((s, r) => s + r.back, 0),
  );
...
export function waveGapTotalMs(
  anomaly: Pick<AnomalyDef, "waves"> | undefined,
  bal: BattleBalance,
): number {
...
export function advanceBattleFor(
  ...
  const favor =
    favorAdv === null
      ? null
      : {
          meMul: 1 + bal.aiFavorStrength * favorAdv,
          foeMul: 1 - bal.aiFavorStrength * favorAdv,
        };
  ...
  while (
    ...
    if (
      waves &&
      waveIdx < lastIdx &&
      !curFoes.some((f) => isAlive(battle, f.tag))
    ) {
      ...
      if (
        ...
        state.gameMs < battle.waveClearAt
      )
        break; // 演出窗口未走完：停表等待，下一拍再续
      battle.waveClearAt = undefined;
      waveIdx += 1;
      ...
      if (battle.pdCd && battle.dronePools) {
        battle.pdCd = curFoes.map(() =>
          Math.max(100, Math.round(bal.pdJudgementMs)),
        );
      }
      ...
      if (reopen > 0 && Number.isFinite(openM)) {
        battle.distanceM = Math.round(
          openM * reopen + battle.distanceM * (1 - reopen),
        );
      }
      const waveName = ctx.galaxies.get(anomaly.galaxyId)?.name ?? "";
      addLog(
...
export function droneRecoveryRate(state: GameState): number {
  const rate =
    DRONE_SKILL.recoveryBase +
    DRONE_SKILL.recoveryPerLevel * droneSkillLv(state, "drone-recovery");
...
function stepBattle(
  ...
  if (meRt && isAlive(b, "player")) {
    for (let wi = 0; wi < me.weapons.length; wi++) {
      ...
      if (w.kind === "gun") {
        ...
        const pick =
          (Object.keys(w.shotsByType ?? {})[0] as DamageType | undefined) ??
          null;
        ...
        dmg = Math.max(
          1,
          Math.round((w.shotDmg ?? 0) * beamPowerFactor(b.distanceM, w)),
        );
        meRt.weapons[wi] = w.reloadMs;
        autoHit = true;
      } else {
  ...
  for (const f of foes) {
    ...
    if (w.kind === "beam") {
      const pow =
        b.distanceM < w.minRangeM
          ? (w.blindDmgMul ?? 0.3)
          : beamPowerFactor(b.distanceM, w);
    ...
    const shotDmg =
      blindMul < 1
        ? Math.max(1, Math.round((w.shotDmg ?? 0) * blindMul))
        : (w.shotDmg ?? 0);
  ...
  if (
    meRt &&
    bal.shieldRegenPerSec > 0 &&
    meRt.hp.s < me.hp.s &&
    (meRt.hp.a > 0 || meRt.hp.h > 0)
  ) {
...
function randomAliveFoe(
  ...
  const i = Math.min(
    alive.length - 1,
    Math.floor(nextRandom(state.rng) * alive.length),
  );
  return alive[i]!;
}
...
function steadyPreview(
  ...
  const foes =
    peakUnits > 1
      ? createFoeSpecs(anomaly, bal, { units: peakUnits })
      : createFoeSpecs(anomaly, bal);
  ...
  for (const w of me.weapons) {
    ...
    if (hit <= 0) continue;
    const mult = effectiveDmgMultAgainst(
      foes,
      ammoType ?? w.fixedType ?? "kinetic",
    );
  ...
  const foeCompWeighted = (type: DamageType): number =>
    foeComp.length <= 1
      ...
      : foeComp.reduce(
          (s, c) => s + c.share * avgLayerMult(meHpTotal, me, c.type),
          0,
        );
  let foeDpsPeak = 0;
  for (const f of foes) {
  ...
  const ttrFoe =
    meDps > 0 ? foeHpTotal / Math.max(1e-9, meDps * meMul) : Infinity; // 我击毁敌方所需秒数
...
export function bountyDamageForecast(
  ...
  const armorLoss =
    sp.me.hp.a > 0
      ? clamp(0, 1, afterShield / sp.me.hp.a)
      : afterShield > 0
        ? 1
        : 0;
  const hullLoss =
    sp.me.hp.h > 0
      ? clamp(0, 1, Math.max(0, afterShield - sp.me.hp.a) / sp.me.hp.h)
      : afterShield > sp.me.hp.a
        ? 1
        : 0;
...
export function bountyWinPercentGuarded(
  ...
  const bal = ctx.balance.battle;
  const penalty =
    f.armorLoss * bal.winPenaltyArmorPerFull +
    f.hullLoss * bal.winPenaltyHullPerFull;
  return Math.max(0.02, Math.min(0.98, shown - penalty));
}

/** 玩家口径预估胜率：无 favor 模型 + logit 扩散（悬赏卡/玩家手动战斗展示用；实际结算与之对应） */
export function battleWinPreview(
  state: GameState,
  ctx: SimContext,
  anomaly: AnomalyDef,
  shipId: string = state.shipId,
...
function effectiveDmgMultAgainst(foes: UnitSpec[], type: DamageType): number {
  ...
  for (const f of foes) {
    ...
    total += s;
    acc +=
      f.hp.s * typeLayerMult(type, "shield") +
      f.hp.a * typeLayerMult(type, "armor") +
      f.hp.h * typeLayerMult(type, "hull");
  }
  return total > 0 ? acc / total : 1;
}

/** 我方三层占比 × 类型克制 × (1−层抗) 的期望倍率 */
function avgLayerMult(
  meHpTotal: number,
  me: UnitSpec,
  type: DamageType,
): number {
  ...
  const s =
    (me.hp.s / meHpTotal) *
    typeLayerMult(type, "shield") *
    (1 - (me.resists.shield?.[type] ?? 0));
  const a =
    (me.hp.a / meHpTotal) *
    typeLayerMult(type, "armor") *
    (1 - (me.resists.armor?.[type] ?? 0));
  const h =
    (me.hp.h / meHpTotal) *
    typeLayerMult(type, "hull") *
    (1 - (me.resists.hull?.[type] ?? 0));
  return s + a + h;
}
