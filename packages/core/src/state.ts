...
export interface ImportantTaskState {
  ...
  started?: boolean
  /**
   * **打捞器兜底补发已给过**（**2026-09-22 船长报障**：「**玩家依旧出现被打捞器卡进度的情况，给所有玩家
   * 发一个打捞器 MK1 吧。**」；只用在 `first-salvage` 这条上）——去重键，**只在真补发那一刻写**
   * （缺省 = 没补过 ⇒ 老档/新档快照零迁移），一台档一辈子只补一次（也堵住"卖掉再领一台"）。
   */
  salvagerGift?: boolean
}
