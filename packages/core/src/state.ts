...
/** 日志类型：显示端按类型配色/筛选 */
export type LogKind =
  "system" | "info" | "queue" | "levelup" | "warn" | "trade";
...
export interface GameStateV9 extends Omit<
  GameStateV8,
  "version" | "blueprints"
> {
  version: 9;
...
/** AI 副船任务（采矿 / 打捞 / 远征 / 待命） */
export type AiTask =
  AiMiningTask | AiSalvageTask | AiExpeditionTask | AiStandbyTask;
...
export interface GameStateV8 extends Omit<
  GameStateV7,
  "version" | "aiCoreLevel"
> {
  version: 8;
...
export function createInitialState(opts?: {
  ...
  const state: GameStateV24 = {
    ...
    sideTasks: {
      ...
      bountyWindow: 0,
      deliver: null,
    }, // v24：任务中心·时效任务板（资源/快递 20 分钟整点开刷；赏金每天本地 0 点开板；faction = 当日派系活跃；deliver = 快递投送在途挂账，缺省 null）
    logs: [],
  };
  if (prologue) {
    ...
    addLog(
      state,
      "warn",
      "自检异常：船体装甲/结构受损（80%），乘员生命信号——无。记忆档案损坏。",
    );
    addLog(
      state,
      "info",
      "初始资金 0 ISK：一切从采集第一舱矿石开始。鲣鱼级护卫舰（待修）与沙猫级采矿艇同在机库；装备库与弹药库为空——首门炮台与弹药将在完成协会试炼后解锁。",
    );
  } else {
    ...
    addLog(
      state,
      "info",
      `初始资金 ${DEFAULT_START_ISK} ISK 已到账；沙猫级采矿艇已停靠机库，另有鲣鱼级护卫舰待命（装备库含轻型炮台 MK1，仓库配三型通用弹各 60 发，可直接体验远征战斗）。`,
    );
  }
  addLog(
    state,
