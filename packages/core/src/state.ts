 ...
 * 只作用于被钉的**那一艘**；**击杀发动者即解除**（2026-09-16）**或交战距离超过 4500 米断开**
 * （**船长 2026-09-26**「将断开距离提高到4500米，且这个断开对敌我都有效」）；多艘不叠加。
 */
...
export interface BattleState {
   ...
   *
   * 口径（船长 2026-09-26）：**交战距离 ≤ 3800 米**（`combat.MY_WEB_RANGE_M`）时钉第一个可选目标
   * → 目标被击沉 或 **距离超过 4500 米断开**（`combat.WEB_BREAK_DIST_M`，敌我通用）⇒ 记冷却、
   * 冷却结束选新目标；**不选重复目标**（已被别的网钉住的跳过）；无目标可选或距离过远 ⇒ 保持待发、
   * **不空转冷却**。
   * **随档**（同上：冷却丢了就是白赚）。 */
   ...
   * **增援时间是60秒**，**每次随机复活一艘**」；**2026-09-26 改判**：「…每60秒复活2艘船。且必定会复活干扰舰」）：
...
export interface GameStateV7 {
   ...
   * - 兼容字段无版本号：老档缺省由 `save.normalizeState` 原样搬**可支配**那一本（**不补任何下界**）。
   */
  standingsEarned?: Record<string, number>
  /**
   * **"按悬赏进度回正累计声望"这一趟已经走过没有**（**2026-09-26 船长令**：
   * 「**只生效一次，已经削过的玩家不再削**」）。
   *
   * `true` = 这份档已经过账 ⇒ `expedition.repairStandingFromBountyProgress` 直接返回、**永不再削**；
   * 缺省（老档 / 新档）= 还没走过 ⇒ 下一次读档走一趟并把本键置 `true`（无论那一趟有没有真削到）。
   * 兼容字段无版本号（可选）⇒ 零迁移。
   */
  standingClawbackDone?: boolean
  /**
