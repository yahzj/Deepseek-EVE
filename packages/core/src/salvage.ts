 ...
 *  2026-09-14 随"所有池补三钛"重配（期望单价不变口径）：常 9.8（未动）· 险 27.60 → **27.7** · 危 92.45 → **92.6**。
 ...
 *  没有三钛合金的，在保持价值不变的前提下将其他材料减少」⇒ 险/危两档补入**三钛 20%**，
 *  其余矿物按 `w × (价/均价)^α` 重配（α 二分求解），使**整池期望单价不变**
 *  （险 27.60 → 27.72 · 危 92.45 → 92.60）；常驻档本来就有三钛 ⇒ **一字未动**。
 *  敌群专属池同批处理（`packages/data/src/salvageFlavors.ts`，15 个补三钛、7 个原样）。
 *  ⚠ **导出**：体检脚本要用它对"档位基础池也必须含三钛 + 均价 = 档基数"做硬契约（B3.2）。 */
