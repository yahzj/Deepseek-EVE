 ...
 *
 * ⚠ **2026-09-23 船长令**（问「将所有普通残骸的精炼炉回收每批的量提高到 100 立方米」⇒ 裁定**甲**；
 * 再问「残骸回收量十倍了，特色掉落怎么算？」⇒ 确认「普通 100 / 稀有 30」）：
 * 批 **10 ⇒ 100 m³**、周期仍 **25 秒** ⇒ 单炉吞吐 **1,440 ⇒ 14,400 m³/h（×10）**、
 * **单炉保底收益同 ×10（≈82k ⇒ ≈820k ISK/h）**；旧口径「10 m³ 与 4×MK1 打捞（1,440 m³/h）对齐」**作废**。
 * **特色掉落不受影响**：彩头是**按体积**掷的（`rollRecycleLoot` 里逐 m³ 一次骰）⇒ 每 m³ 期望不变、
 * 每小时产出随吞吐 ×10；稀有残骸的彩头另按"累计已烧体积每满 30 m³ 必给一次"结算，与批次大小无关。
 * ⚠ **稀有残骸不适用本常量**：它走「每炉锁 1 件 = `RARE_UNIT_M3`（30 m³）」那条独立口径
 * （起炉门槛与批大小都由 `industry.ts` 按残骸类型分支取值）。
 */
