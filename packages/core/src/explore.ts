...
/**
 * **扫描时长的危险度曲线指数**（**2026-09-25 船长令**：「**原先最后一个星系是24小时，削减到12小时，
 * 并且采用曲线形式**」；四答 = **Q1 乙**（高安保持 10 分钟不动）· **Q2 ①**（幂次曲线）·
 * Q3 接受中低安大幅压缩 · Q4 母港／技能／上限钳制照旧）。
 *
 * 形状（`u = 0.5 − 安全`）：
 * - `u ≤ 0`（**高安，`sec ≥ 0.5`**）⇒ 系数恒 **1** ⇒ 窗口 = 基准 10 分钟（与 2026-09-23 那版**逐字相同**）；
 * - `u > 0`（中安／低安）⇒ `(1 + u) ^ 本值`——**上凸幂次曲线**：`u` 小时涨得慢、`u` 大时涨得快。
 *
 * 指数由端点反解：最深星系（`sec = −1.0 ⇒ u = 1.5`）要落在 **12 小时** ⇒ `2.5^p = 720/10 = 72`
 * ⇒ `p = ln 72 / ln 2.5 = 4.667368…`。端点校验：`2.5^4.667368 = 72.0000` ⇒ 10 分 × 72 = **12 小时**。
 *
 * 逐档读数（无技能）：高安 **10 分** · 0.4 → 16 分 · 0.2 → 34 分 · 0.1 → 48 分 · **0.0 → 1 时 06 分** ·
 * −0.2 → 1 时 59 分 · −0.5 → 4 时 14 分 · −0.7 → 6 时 36 分 · −0.9 → 9 时 55 分 · **−1.0 → 12 时**。
 *
 * ⚠ **本常量取代 `SCAN_LOWSEC_PENALTY`（已删）**：那个是**线性**系数（2026-09-05 立、2026-09-23 放大到
 * `143/1.5 = 95.333…` ⇒ 最长 24 小时），**其口径已被本条作废**（同一件事不留两套）。
 * ⚠ **母港（大鲸鱼Ⅳ）仍固定 10 秒**、不吃本曲线（船长 2026-09-18 定、09-25 复核「确定」）。
 * 技能照旧乘算（信号分析学／信号过滤学／星图测绘学，各封顶 5 级，满级 ×0.294）⇒
 * 满技能下最深星系 = 12 时 × 0.294 ≈ **3 时 32 分**；上面那张表说的是**无技能基准**。
 */
export const SCAN_CURVE_EXP = Math.log(72) / Math.log(2.5)
/** 全游戏安全等级下限（`data/universe.ts` 最危险的星系 = −1.0）——用于算扫描窗口的**合法上限** */
export const SEC_FLOOR = -1

/**
 * 扫描窗口的**合法上限**（毫秒；无技能 + 最危险星系）＝ `SCAN_WINDOW_MS` ×(1 + 1.5)^`SCAN_CURVE_EXP`
 * = 10 分 × 72 = **12 小时**（2026-09-25 船长令；旧版是线性 ×144 = 24 小时，已作废）。
 *
 * **由指数推导、不写死乘数** —— 以后调指数，上限自动跟随。
 * 用途：**读档兜底**——`save.normalizeState` 没有 ctx、拿不到目标星系的安全等级，只能按"全游戏可能出现的
 * 最大有效窗口"钳制；**不能**拿基准 `SCAN_WINDOW_MS` 去钳，否则低安星系（窗口最长 12 小时）的续扫进度
 * 会在读档时被截断（2026-09-11 修复）。技能只缩短窗口，故上限与技能无关。
 */
export function maxScanWindowMs(): number {
  return Math.round(SCAN_WINDOW_MS * Math.pow(1 + (0.5 - SEC_FLOOR), SCAN_CURVE_EXP))
}
...
/** 目标星系的实际扫描窗口（毫秒）：技能缩短 × **危险度曲线**（2026-09-25 船长令：上凸幂次，
 *  高安 10 分钟不动、最深 12 小时；取代 2026-09-05 的线性惩罚）；
...
export function scanWindowMsFor(state: GameState, ctx: SimContext, galaxyId: string): number {
  ...
  // 危险度曲线：高安（sec ≥ 0.5 ⇒ u ≤ 0）恒 1；中低安走 (1+u)^SCAN_CURVE_EXP
  const u = 0.5 - sec
  const shape = u > 0 ? Math.pow(1 + u, SCAN_CURVE_EXP) : 1
  return Math.round(SCAN_WINDOW_MS * scanSkillFactor(state) * shape)
}
