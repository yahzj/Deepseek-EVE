...
export interface CommandResult {
  ...
  taken?: number
  /**
   * 虫洞"激活/打捞"专用（船长 2026-09-13）：**已结算、但等玩家确认的战斗**（目前只有 `'ruins'`
   * = 打捞遗迹惊动了守备）。界面据此弹确认条，玩家点「迎战」后再开打 —— 战斗不再毫无提示地突然发生。
   * 确认之前 `run.pendingRuinsBattle` 为真，别的动作一律被拦（`gridActionBlocked`）。
   */
  pendingBattle?: 'ruins'
  /**
