 ...
 * 答「**现在**」）。
 *
 * 值 = **2026-09-26 23:11 +08:00**。**本次改动**（**船长令**：「**将之前的补发时间调到现在截止**」）——
 * 原先写的是 21:33，而"母舰被章鱼收走却没拿到黑匣"的真根因是**那一掷被漏掉了**（系统性，见
 * `weekendEvent.weekendTickBoss` 的同日修复），不是"某一场的问题" ⇒ 旧截止的前提（"修复后掉落就好了"）
 * 不成立 ⇒ 按船长令把截止推到现在，让 21:33~23:11 之间结束的场次也进补偿面。
 * **只补这一批修复上线之前打完的入侵**。⚠ 若日后还要补，**改这个常量**（别改成 `Date.now()`：
 * 那会让"以后每一场没掉黑匣的入侵"都被补一遍）。
 */
export const WEEKEND_BOX_COMPENSATION_CUTOFF_WALL_MS = 1_790_435_460_000
