 ...
 * ① 低安遇袭受损档（`encounters.ts`，一口 = 敌群火力 × `encounter.hitFirepowerSec` = **5 秒**）；
 * ② 战斗撤退/自动撤退/超时判负/无法交战那一口（`expedition.ts`，一口 = 敌群火力 ×
 *    `combat.retreatHitFirepowerSec` = **5 秒**，四档同一 K）。
 * ⚠ 秒数沿革：低安遇袭 0.3 → 5 秒 · 战斗撤退 1 → 10 → **5 秒**（船长 2026-09-14 改 10、
 *   2026-09-26 裁定「甲：单纯削减时间」改 5）⇒ **两处现在同为 5 秒**。
 * 两处若各写一份，日后调数值/改底线必然漂移，故算法、底线、日志用的比例换算全部收在这里。
 *
 * 口径（与既有文案一致，别改错）：
 * - **一口伤害 HP = 敌群火力（威胁 × `battle.foeDpsPerThreat`）× 秒数**；
 * - **不计玩家抗性**（2026-09-26 船长明令「之前没有抗性就算了」）——本口伤害不吃盾/甲/壳三层抗性，
 *   现状如此、**照旧保持**；
