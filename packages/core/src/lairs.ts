...
export const FOE_LAIR_GEAR: Record<FoeFamily, readonly string[]> = {
  ...
  // 开高级箱 ⇒ 本池（绝境档 **10%**，集齐前不重复）；同批**打开 H 族残骸回收**（原先"等 H 势力装备"而关着）。
  // 🔴 2026-09-26 第二批（船长令「**将H族稀有残骸按照其他族那样混池**」）：本池**只走专属支**——
  // 高级箱未命中那 90% 改由**通用主题件**兜底（`h-hi` 组的 `theme` = D 高安组同款 `mod-shield-pla-2`）
  // ⇒ 不再"未命中也是这三件"（改前两支同池 ⇒ 稀有箱必出 H 件）。⚠ 代价：集齐三件 ≈3 件稀有残骸 → ≈30 件。
