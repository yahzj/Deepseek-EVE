...
export type CommsTrigger =
   ...
   *
   * 判定 = 已点亮的星系里存在**安全等级 < +0.5** 者。**阈值刻意与既有低安判定同源**——
   * `encounters.ts` 的暴露面掷骰（「高安 sec ≥ 0.5 不掷」）与 `explore.ts` 的扫描窗口惩罚（`sec < 0.5`）
   * 用的都是这条线 ⇒ **安全等级被改判时本触发器自动跟随**（2026-09-11 船长就改判过烬火星区/蜃影星系的
   * 安全等级），不必回来改星系 id 清单。missing security 一律按高安（`?? 1`，与 `secOf` 同口径）。
   */
  | { kind: 'lowSec' }
  /**
