...
export interface FoeShipSlot {
   ...
   * ⚠ 舰级路径下射程带是**绝对值**、不再由战术推导，故换战术通常要同时写
   * `rangeMinM` / `rangeMaxM`，否则会出现"想打远战、武器却只有 2.6 km"的矛盾。
   */
  tactic?: FoeTactic
  /** 射程带下限**绝对覆写**（缺省 = 舰级值 × `rangeMul`）；换战术时与 `rangeMaxM` 配对使用 */
  rangeMinM?: number
  /** 射程带上限**绝对覆写**（缺省 = 舰级值 × `rangeMul`） */
  rangeMaxM?: number
