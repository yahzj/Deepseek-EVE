...
export type FoeMountId =
  ...
  /** **姿态陀螺仪**（**船长 2026-09-24**）：「A族添加一个挂载件：姿态陀螺仪：增加10%闪避」⇒ 该舰闪避 **+0.10 加算** */
  | 'foe-mount-gyro-stabilizer'
  /** **船体修理装置**（**船长 2026-09-24**）：「给G族添加挂载件：船体修理装置。每5秒恢复5装甲和5结构，会吃威胁的加成」 */
...
export interface FoeMountDef {
  ...
  /**
   * **姿态陀螺仪**（**船长 2026-09-24**：「A族添加一个挂载件：姿态陀螺仪：**增加10%闪避**」＋追问裁决
   * 「**加算 +10 个百分点**」）：挂本件的敌舰，战斗中 **闪避 +`add`（加算）**，消费方夹上限 0.9。
   * 只挂**虫洞卡的条目**（星图悬赏/低安遭遇不引用 ⇒ 零影响）。
   */
  evasionBonus?: { add: number }
  /**
   * **船体修理装置**（**船长 2026-09-24**：「给G族添加挂载件：船体修理装置。**每5秒恢复5装甲和5结构**，
   * **会吃威胁的加成**」）：挂本件的敌舰每 `everyMs` 回 `armor` 装甲与 `hull` 结构（**夹到满值、不回超**）；
   * 实数 = 本字段基数 × **该层威胁倍率 k**（`k = 层威胁 ÷ 45`，层 1 = ×1.00 —— 船长裁决「不改动」基准）。
   */
...
export interface ModuleDef {
  ...
  rangeCutPct?: number
  /** **按系武器射程加成**（幽灵弹道校正器「动能武器射程 +22%」）：射程 × (1 + 该系值)；
   *  多件按系**加算**。缺省不写 = 无加成。 */
