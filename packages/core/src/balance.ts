...
export const DEFAULT_BALANCE: BalanceConfig = {
  ...
  market: {
    ...
    /**
     * **数字稀有度 4 档刷新权重乘子**（**2026-09-16 船长选「选项 B」**）。
     *
     * 由来：当日「甲＋乙」把 5 艘官方巡洋舰**保 4** 挪进稀有订单，而当时的 `rareTierWeight` **只特判档 3**
     * ⇒ 档 4 落进 `else` 拿 ×1（与大众档同频：实测单行 **3.2 窗 ≈32 分钟**，改动前在奇货 ≈20.8 小时/件）。
     * 船长问清 2/3/4 的权重（1 / 0.15 / 1）后，在两个已量过的旋钮里选 **B = 给档 4 单独系数**：
     * **0.05** ⇒ 实测单行 **57.2 窗 ≈9.5 小时**（档 3 仍 0.15 ≈3.9h；档 2 仍 1 ≈0.8h）。
     * ⚠ 档 **5** 目前无商品、也**没有**自己的系数（仍走 else ⇒ ×1）；将来启用时一并定。
     */
    rareTier4Weight: 0.05,
