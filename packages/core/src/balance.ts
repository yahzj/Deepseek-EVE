...
export const DEFAULT_BALANCE: BalanceConfig = {
  ...
  battle: {
    ...
    //
    // 历史读数（钳制开着时的实测，供恢复时对照）：**它并非空转**——`巨构核心勘探令 3 档`
    //   单波舰体 159.4 DPS 确被压到 150（−5.9%）；单条目卡（`穹顶守卫`/`虚海守望者`/`巨构核心`
    //   的母舰条目）也都在压。⚠ 旧注释曾写「27 张卡全部未越线 ⇒ 零行为变化」，那是**测试夹具的
    //   假绿**（`hullDps` 只统计第一条条目，多条目卡漏算），**已作废**，详见该用例文件头的待办。
    foeDpsCap: 0,
