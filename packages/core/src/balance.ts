...
export const DEFAULT_BALANCE: BalanceConfig = {
  ...
  battle: {
     ...
     * 每艘敌舰独立结算点防：按射速对"在点防射程内存活的我方放飞无人机"逐架射击，
     * 命中按机型闪避判定、伤害按机型三层抗性消费；血量归零 = 该架本场击落并**永久损失**
     * （战后从无人机舱清单扣除）。低威胁敌舰没有点防（见 pdThreatFloor）。 */
    pdThreatFloor: 45, // 点防起始威胁：威胁 ≤ 此值的敌舰无点防
    pdThreatSpan: 65, // 满档跨度：威胁 ≥ floor+span 满档（强度按威胁线性缩放）
    pdRatePerSec: 1.2, // 满档每舰每秒点防射击次数（多舰叠加；2026-09-10 首轮实测调档）
    pdAcc: 0.55, // 点防基础命中（直接减机型闪避；不叠敌方通用命中加成，突出"闪避"价值）
    pdDmg: 14, // 点防单发伤害（走该机型三层抗性）
    pdRangeM: 4000, // 点防射程：放飞无人机在此距离内才可能被击落（哨戒 5km 常在射程外）
    pdMaxLossFrac: 0.5, // 单场点防最多击落比例（相对本场放飞总数，防团灭）
  },
  events: {
