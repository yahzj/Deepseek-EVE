...
export const DEFAULT_BALANCE: BalanceConfig = {
  mining: {
    ...
    yieldPerLevel: 0.06,
    timeSkillId: 'mining-frigate', // 采矿舰入门学（原「采集器入门学」，2026-09-22 船长再改名；更早叫「采矿护卫舰操作」）：每级 -3% 循环时间
    timePerLevel: 0.03,
  ...
  combat: {
     ...
     *
     * ⚠ **2026-09-14 船长改判：「玩家撤离战斗按照 10 秒算」⇒ 1 秒作废**；
     * **2026-09-26 船长再改判「甲：单纯削减时间」：10 → 5 秒**——起因 = 船长「**目前撤退的修理损伤
     * 有些大，我记得之前是敌人10秒的DPS输出吧？**」；核对结论：现值**确实**就是 10 秒（不是被谁改高的），
     * 但**卡面威胁已按新定价式整体上调** ⇒ 10 秒 × 已上调威胁 = 双重放大。船长裁定**只削秒数**：
     * 10 → **5**，与低安遇袭那口（`encounter.hitFirepowerSec` = 5 秒）同口径。
     * 范围照旧：**四档一起改、同一 K**——玩家主动撤退 / 结构<50% 自动脱离 / 战斗超时被迫撤退 /
     * **无法交战**（够不着、一炮未发）；**虫洞的撤离战不在内**（走真实战斗损伤，不吃本值）。
     * 读数：一口 = 威胁 × 4 HP（威胁 42 的入门低安卡 ⇒ 168 HP；威胁 115 ⇒ 460 HP；
     * 参照船体总量 沙猫级 51 / 灰鲭鲨级 192 / 长尾鲨级 323 / 玳瑁级 790 / 玄武级 1107）。
     *
     * ⚠ **不计玩家抗性**（**2026-09-26 船长明令：「之前没有抗性就算了」**）：本口伤害**不吃**三层抗性
     * （盾/甲/壳），现状如此、**照旧保持**——想加抗性前先回船长处裁定，别顺手"补全"。
     * 窝点档按**派生后威胁**算（见 `expedition.settleBattleRetreat`，与界面/战斗同口径）。
     */
    retreatHitFirepowerSec: 5,
  ...
  repair: {
    ...
    perHpCost: 300,
    // 「修理组件恢复量」加成技能表（**两条用件路径共用同一处系数**）——
    // ① 直接使用（手动 / 遇袭自动修 / 重复清剿自动修，`shipyard.kitHealFor`）
    // ② 船体维修装置战斗中每跳（`combat.preloadRepairFor` 开战预载时按本系数放大每跳值）。
    // 无消耗自愈件（异形生体件 `repairFree`）**不吃**（它不消耗组件，与"修理组件用法"无关）。
    // 2026-09-13 船长三条裁定：① 装置也吃舰体快修学；②「效果需要削弱，修理效果加成都降低到 5% 每级」
    // （快修学 0.10 → 0.05）；③ 维修工程学（rank 2→**3**）**保留停站费减免**并**新增同款恢复量加成**
    // ⇒ 两条技能**按级线性相加**（都满级 = +50%，与削弱前单技能上限持平）。
    quickRepairSkills: [
      { id: 'hull-quick-repair', perLevel: 0.05 },
      { id: 'repair-engineering', perLevel: 0.05 },
    ],
  },
  ...
  encounter: {
    ...
    //
    // ⚠ **2026-09-14 船长改判：「遇袭失败损失按照 5 秒算」⇒ 0.3 秒作废，现值 5 秒**（×16.7）。
    // 范围经船长逐条裁定：**遇袭的两处一起改**——① 文字结算「受损」档（离线结算 / 超时未响应 /
    // 点「快速脱离」）② 应战失利后追加的那一口；两处本就共用本旋钮。
    // 读数：一口 = 威胁 × 4 HP（威胁 42 的入门低安卡 ⇒ 168 HP）⇒ T1/T2 级船必被打到结构 5% 底线，
    // 之后走既有「先修后判」（修理组件补到 60%，组件耗尽或结构仍 <50% 才收手返港待命）。
    hitFirepowerSec: 5,
  ...
  market: {
    ...
    /**
     * 限定商品（奇货）每个抽取窗（10 分钟）独立掷骰的出单概率。
     * ⟪**2026-09-25 船长令**⟫「**0.8%的概率上调到1%**」⇒ 0.008 → **0.01**
     *（约每件每 **16.7 小时**一轮；旧值 0.008 ≈ 20.8h。2026-09-06 船长定窗口 10 分钟）。
     * ⚠ 同日另令「**奇货订单每窗上限再+2**」⇒ `market.EXOTIC_CAP_PER_DRAW` 2 → 4。
     */
    exoticWindowChance: 0.01,
    /**
     * **数字稀有度刷新权重乘子**（⟪**2026-09-25 船长令**⟫：「**2~5稀有度的稀有订单，权重调整为
     * 1/0.5/0.2/0.05**」）⇒ 现行四档阶梯 = **档 2 ×1 · 档 3 ×0.5 · 档 4 ×0.2 · 档 5 ×0.05**。
     *
     * 沿革（**别照旧文重开**）：
     * - 2026-09-09 立表：只给档 3 系数 `rareTier3Weight` = **0.25**（档 2 = 1；档 4/5 无系数 ⇒ ×1）；
     * - 2026-09-10：「高阶稀有订单更难碰」⇒ 0.25 → **0.15**（实测 tier3 占比 10% → 6%）；
     * - 2026-09-16 船长选「选项 B」：补 `rareTier4Weight` = **0.05**（此前档 4 也落 else ⇒ ×1）；
     * - ⚠ **档 5 始终没有系数**（走 else ⇒ ×1，与大众档同频）——2026-09-20 放宽渠道带时曾明确记账
     *   「要档 5 系数另立条目」，但一直没立。**实测后果**：`bp-shieldfield-3`（档 5）单行间隔
     *   **68 窗（≈11.4h）**，反而比 `bp-shieldfield-2`（档 4 = 0.05，**214 窗 ≈35.7h**）**常见 3 倍**
     *   ⇒ 档位越高越常见，与其语义相反（读数 = `market-rarity-sim`，1440 窗 × 5 种子）。
     * - ⟪2026-09-25⟫ **本次按船长给的四个数一次补齐**（档 5 首次有系数）。
     */
    rareTier3Weight: 0.5,
    rareTier4Weight: 0.2,
    rareTier5Weight: 0.05,
    blueprintWeight: 0.05, // 蓝图书权重乘子（2026-09-10 船长定：50% → 5%，稀有抽取 + 奇货掷骰两渠道都乘）
    singleUseBlueprintWeight: 0.5, // **一次性舰船蓝图**权重乘子（2026-09-13 船长：「还是有惩罚吧，按50%算」）——一次性图纸是消耗品，不吃普通蓝图那档 ×0.05
    ...
    // 每层未衰减惩罚 → NPC 买单挂单量放大（**乘法叠加**）。
    // 沿革：2026-09-11 船长定 +8%/层（线性）；**2026-09-17 船长改判**：「买单量 +8%/层，卖单量 −8%/层，
    // 分别改为 **+40%/−40% 每层**，并且是**乘法叠加**」⇒ 公式 = `(1+0.4)^层数`。
    // ⚠ 量级提醒：砸盘稳态 8.2 层 ⇒ **×15.8**（旧线性口径只有 ×1.66）；9.2 层 ⇒ ×21.9。
    dumpBuyVolumePerLayer: 0.4,
    // 砸盘时 NPC 挂卖单量同步削减（**乘法叠加**）：公式 = `max(0.1, 0.6^层数)`，下限 10%（不把供应簿削光），
    // 只在砸盘方向生效。⚠ 0.6^层数 在 ~4.5 层就撞到 10% 下限 ⇒ 深砸盘时供应侧基本停摆。
    dumpSellVolumePerLayer: 0.4,
    ...
    // 价格输出下限（相对基准），防归零。
    // 沿革：2026-09-11 船长定 0.2 → 0.1（"让 10%/层的砸盘惩罚真跌得下去"）——
    // **2026-09-17 船长改回 0.2**：「地板取0.2，不加吸收上限」⇒ 硬地板 = 基础价 ×20%，
    // 大额倾销仍照旧叠冲击（8.2 层、买单量 +8%/层、卖单量 −8%/层），只是**跌到基础价两成就到底**。
    minPriceRatio: 0.2,
  ...
  battle: {
     ...
     *
     * 2026-09-15 船长裁定（起因＝玩家报「进行赏金任务时，敌我的初始距离非常近，对远程武器不利」）：
     * **星图与洞内统一取 0.8（射程带高位）**——⚠ 沿革：当天我先按"星图 0.8 / 洞内维持中段 0.5"分档落码，
     * 船长随后更正「**这个是我口误，可以回滚那句**」⇒ **取消分档**、两边同值
     * （导弹架 1,400~14,880 ⇒ **12,184 m**；旧中点 8,140 作废）。
     * ⚠ 与下面 `wormholeBrawlOpenBand` **刻意分开**：那个只管"洞内近战怪的开局距离"。
     * 贴脸（`assault` = 最小射程×0.6）与风筝（`kite` = 最大射程×0.95）两档**不受本值影响**。
     */
    desireBandMid: 0.8,
    /**
     * **洞内「近战怪开局距离」用的档位**（0.5 = 射程带中点）。船长选定「乙」（2026-09-15）：
     * 近战怪（`tactic === 'brawl'`）一开场就站在**我方中段**，是 2026-09-13 为洞内定的
     * 「进去就得挨打」张力 ⇒ 默认期望抬到 0.8 时**不把它一起带走**（两者各自一个常量，互不牵连）。
     */
    wormholeBrawlOpenBand: 0.5,
    ...
    // **敌舰体火力「越线折扣」阈值**（2026-09-12 船长「按照 DPS 上限 150 算」定值 ·
    //   2026-09-15 船长改判形态：「不是钳制到150，而是超过150的部分进行一个约15%的折扣」）。
    // 判线口径：**整卡每波舰体总 DPS**（含多舰补偿 2N/(N+1)、**不含机群**、
    //   **不含被 droneFireShare / firepowerAnchor 拆分的条目**）；越线只对**超出部分**按
    //   `foeDpsOverCapDiscount` 打折，再等比例施加于全卡舰体单发（**不封顶**）。
    //
    // ⚠ **口径沿革（旧口径作废登记）**：2026-09-12 首落为**硬钳制**（越线一律压到上限），同日船长就
    //   "赏金分波分流"议题收口「回退到赏金维持现有配置，仅按照威胁加强敌方。并且火力钳制也暂时关闭」
    //   ⇒ 置 0 关闭；**2026-09-15 船长两条指示**（「超过150的火力，按比例衰减」→「不是钳制到150，
    //   而是超过150的部分进行一个约15%的折扣」＋「是，都生效」）⇒ 改为折扣制、写回 **150**，
    //   **洞外与虫洞一律生效**。"越线一律压到 150" 的旧硬钳口径自此作废（用例与头注已同步改写）。
    //
    // 现行读数（`150 / 0.15` 实测 · 探针 `tools/_dps-cap-audit.ts`）：
    //   · **洞外 27 张基础卡全部未越线**（最高 = 虚海守望者 131.3）⇒ **逐字零变化**；
    //   · **窝点派生档**越线三张：穹顶 L1/2/3 **164.8 / 203.1 / 253.2** · 虚海 L1/2 **170.3 / 210.0** ·
    //     噬口 L2/3 **172.3 / 215.5** ⇒ 折扣后 **162.7/195.2/237.8 · 167.3/201.0 · 169.3/205.5**
    //     （−1.3%~−6.1%；贴线未越的坟场 L2 140.0 / 巨构核心 L3 134.5 逐字不动）；
    //   · **虫洞派生**：层 1 起陆续越线（浅档 A/C/G 层 1 即 163/215/235），深层 node 最高
    //     **1,234.5 → 1,071.8**、boss **1,665.8 → 1,438.5**（−13%）⇒ 属轻收敛，既有洞内难度口径不需重校。
    //   ⚠ 2026-09-12 旧注释只登记穹顶/虚海为越线卡——**噬口 L2/L3 是 2026-09-11 虫群编成改造后新越的线**，
    //     本次一并更正；同一条旧注释还曾把"基础卡未越线"误写成"全表零行为变化"（09-12 深夜已改正）。
    foeDpsCap: 150,
    /** 越线折扣率（2026-09-15 船长「超过150的部分进行一个约15%的折扣」）：
     * `D′ = foeDpsCap + (D − foeDpsCap) × (1 − 本值)`。「约 15%」⇒ 本值即调校旋钮；
     * **未写 / 0 ⇒ 关闭**（不缩放，也不会退回旧硬钳制语义）。 */
    foeDpsOverCapDiscount: 0.15,
    foeHitRate: 0.85, // 敌方动能/爆炸武器基础命中（2026-09-08 船长定：普遍高命中；能量=光束必中不消费本值；逐卡 foeHitRate 可覆写做低命中特例）
    /**
     * 动能/爆炸**等效补偿**（方案 A 初值：按典型回避 0.22 × 中距衰减 0.7 折算回旧 0.55 模型，矩阵迭代校准）——
     * 作用点是**敌方单发伤害**（`shotDmg = DPS × 装填 ÷ 有效命中 × 本值`），不在命中公式里。
     *
     * ⚠ **2026-09-15 口径沿革（船长「改为从初始命中中扣」）**：命中公式改成
     * `(基础命中 + 加成 − 回避) × 距离衰减`（回避不再免疫衰减）⇒ **本补偿值未动**（船长当日选「先不动、看读数」），
     * 于是同一补偿下敌方**期望承伤上升**：典型回避 0.22 时中距（df 0.7）+17.6%、远端（df 0.5）+53.7%；
     * 本船（回避 0.34）中距 +40%、远端 +200%。雷达读数见 `tools/_ui-artifacts/calib-{before,after}.log`
     * 与 `hit-{before,after}.log`（`battle:calibrate --std` / `battle:hit-profile` 前后对照）：
     * A0~A3 平均胜率漂移 +1.9 / −1.3 / 0 / +1.9 个百分点、平均残血 ±3.5 个百分点以内。
     * **要重锚就改这一个数**（对齐中距锚点需 ≈ 0.527、对齐远端锚点需 ≈ 0.403），改前先跑那两个工具复核。
     */
    foeHitCompMul: 0.62,
    ...
    /* ═══ 敌冲锋（2026-09-10 定；2026-09-11 改判结束条件；**2026-09-14 两次改判**）═══
     * **触发两条取或**：① 够不着（距离在自己武器射程之外）——原口径；
     *   ② **距离 > 期望交距 + `foeChargeTriggerMarginM`**（船长 2026-09-14：「冲锋按乙方案来」）。
     *   为什么加 ②：射程长的单位（噬口巨兽 +1000 后 3,713 m）在原口径下**永不触发**，体感=冲锋不存在。
     * **解除两条取或**（船长 2026-09-14：「冲锋解除机制为自身攻击命中后解除冲锋状态，并进入 10 秒冷却」）：
     *   ① **自身炮台命中我方**（新增，也是"冲锋到达目标距离后并不解除"那条 BUG 的出口——
     *      我方更快且期望交距更远时距离会**停在期望交距之上**，`arrived` 永不可达）；
     *   ② 压到**期望交距**（2026-09-11 口径，**留作兜底**）。
     * **逐单位**：每个挂 `foeCanCharge` 的单位各持一份"在冲 / 冷却到某时刻"（`BattleState.foeCharges`），
     *   倍率取 **舰级 `FoeShipDef.foeChargeMul` ?? 本全局值**；编队接近速度 = 逐单位乘各自倍率 → 取平均。
     * 资格两条**互相独立**的来源：**舰级级 opt-in**（`FoeShipDef.foeCanCharge`，无条件，噬口巨兽 + 三种小虫）
     *   与**老路**（威胁 ≥ 门槛 且 战术 = brawl）。
    ...
    /**
     * 冲锋期敌机动倍率（临时加速，仍走距离拔河公式）——**缺省值**，舰级可覆写。
     * ⚠ 沿革：原 2.0 → 2026-09-14 船长试验 4.0 → **同日定为 3.0**（船长：「大虫子的冲锋倍率改为 3」，
     *   同时给三种小虫各配 1.5，故本条退回"没写覆写时的缺省值"这一角色）。
     */
    foeChargeMul: 3.0,
    /**
     * **冲锋触发余量**（船长 2026-09-14：「冲锋按乙方案来」）：距离 > 期望交距 + 本值 ⇒ 开始冲锋。
     * 与"够不着（超出自己射程）"**取或**；结束条件见上（命中解除 / 到达兜底）。
     * 例：噬口巨兽期望交距 543 ⇒ **1,543 m 起冲**（原口径要 3,713 m 以外才会冲）。
     */
    foeChargeTriggerMarginM: 1_000,
    foeChargeMaxHoldMs: 2_000, // ⚠ **已停用**（2026-09-11 改判：结束条件改"到达目标距离"，不再用维持时长）
    foeChargeCooldownMs: 10_000, // 冷却：解除后这么久内不能再次冲锋（2026-09-14 船长由 20 秒改判为 **10 秒**）
    foeChargeThreatFloor: 60, // 威胁门槛（与 pdThreatFloor 同口径；只管老路）
    /* ═══ 单波次内增援（2026-09-11 机制落地 → **2026-09-19 启用**）═══
     * **机制**：编成条目的 `enterAt` 给"第几秒 / 击毁几个 / 残血到多少"三种入场触发，
     * 未触发的单位**开战不进战场**，由 `advanceBattleFor` 每拍检查、条件命中才补入（`enterReload` 哑火窗口）。
     * ⚠ **启用依据**（船长 2026-09-19「支援呼叫装置」批）：「**战斗开始20秒后，增援2艘幽灵舰。
     *   如果对方在自己最远射程之外时，增援2艘静滞卫舰。**」——第一批用户 = 洞内深层卡「陵墓王庭」，
     *   分支判定见 `combat.resolveReinforcements` 与 `FoeMountDef.supportCall`。
     *   content:check 的旧「增援机制未启用契约」已同步改写成「支援呼叫装置契约」
     *   （只允许挂了该件的卡写 `enterAt`/`enterBranch`、两支成对且守恒）。
     * ⚠ 与**多舰补偿系数 `2N/(N+1)`**（A 族数值批）是**结构解法 vs 数值补偿**两条路：
     *   洞内派生会把总量归一（补偿只影响血/火力比 `r`，本卡已按"与补偿前同 r"配平）⇒ **不叠加**；
     *   全局复核仍按原话挂着（船长未定）。 */
    foeReinforceEnabled: true, // 总开关（2026-09-19 船长批「支援呼叫装置」⇒ 由 false 改 true）
    foeReinforceReopenFrac: 0, // 增援入场的距离重开比例（语义同 waveReopenFrac；0 = 原地入场不重开＝缺省口径）
    /**
     * **「支援舰船召唤装置」总开关**（2026-09-25 船长令）：置 true ⇒ 挂了该件的单位（先只有
     * H 族入侵母舰）每 60 秒把**当前波已阵亡**的一艘敌舰满血复活入场（上限 = 不超本波原编成）。
     * 没挂件的战斗**一次都不消费随机数**、一行行为都不变（见 `combat` 的"支援舰召唤"一段）。
     */
    foeReviveEnabled: true,
    // P0 承伤持久化：护盾战中被动回充（每秒回满盾的 2%；P2 随流派平衡再校准）
    // ⚠ **2026-09-20 船长加「速度下限」**（原话：「舰船护盾的恢复速度下限改为1%。
    //   但是当护盾被击穿时，依旧是0%」→ 追问后补：「**满盾依旧是2%，当盾量接近0的时候是1%**」）：
    //   本值仍是**按当前盾**的费率（满盾时 `100%×2% = 2%`，与"满盾依旧 2%"一致）；
    //   另有一条**下限 = 满盾 × 1%/秒**，在盾**低于半盾**后接管 ⇒ 见 `combat.ts` 的
    //   `SHIELD_REGEN_FLOOR_PCT` 与 `stepBattle` 的回充段（盾 = 0 仍为 0，那条裁定不变）。
