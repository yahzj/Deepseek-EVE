 ...
 *   该四张模板 2026-09-11 已按船长裁定「废弃F族，将F族融合进A族」**显式登记为 A 族**、`'F'` 字母位空置）；
 *   威胁 = 该敌群威胁（不再随承担船火力缩放）；
 ...
 *    ⚠ **2026-09-16 船长改判**：这一步**需要该船装着船体维修装置**，且**消耗的组件类型跟着装置走**
 *    （民用装置吃民用组件、MK1/MK2 吃军用组件）——**没装装置 ⇒ 完全不修**（`hasDevice=false`），
 *    此时 `outOfKits` 恒为 `false` ⇒ **不会**被误判成"断料返港"，只按下面的结构线判。
 ...
 * **族口径（2026-09-11）**：四张模板已显式登记 `foeFamily: 'A'`（原"未录族 → 兜底 F 形"作废，
 * F 族废弃、字母位空置）——故旧档遭遇战的造型/签名/窝点词表一律走 A 族口径。 */
