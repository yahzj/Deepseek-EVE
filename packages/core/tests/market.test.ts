...
describe('市场 · 消耗品在稀有渠道的批量档（2026-09-20 船长令 · 乙案）', () => {
  ...
  it('收购额度：贴买盘价挂 5 万发，当窗就能被站内吸收（额度 ≥ 21,600/窗）', () => {
    /**
     * ⚠ **种子敏感（2026-09-25 记录）**：本用例验的是**机制**（贴买盘价挂大单 ⇒ 站内当窗吸收），
     * 但"吃掉多少"取决于开盘铺簿的随机相位——实测同一套机制下逐种子差异极大：
     ...
     * ⇒ 机制未变、只是"哪个种子落在大额度配置"变了 ⇒ 本用例改用 seed 1（同口径下 5 窗全吃 5 万，
     * 断言留足余量）。读数出处：一次性探针（镜像本用例、逐种子打表），已在汇报里列明。
     */
...
describe('建站收购网络扩容：卖出侧 ×1.5^N（单件商品放大；池品不乘）', () => {
  const siteDef = (id: string, tiersN: number): StationSiteDef => ({
    ...
    galaxyId: 'galaxy-far',
    standingReq: 0,
    tiers: Array.from({ length: tiersN }, (_, i) => ({ name: `档${i + 1}`, bill: [{ itemId: 'min-a', count: 100 }], unlockDesc: '测试' })),
