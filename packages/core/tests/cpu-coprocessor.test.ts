 ...
 *
 * 船长四条裁定：
 * ① **自身不占用**（`cpuUse = 0`）、只加 `cpuBonus`（MK1/MK2/MK3 = +10 / +15 / +20）；
 * ② 新开一族「协处理器」（低槽）；
 ...
 *
 * 测试世界刻意做成"刚好压线"：船 100 CPU、件 40 / 60 / 20，扩容 MK3 后正好 120 ⇒ 20 的件装得下；
...
/** 船 100 CPU（槽位给足，不参与判定）+ 三档协处理器 + 40 / 60 / 20 CPU 的三个件 */
function world(): { state: GameState; ctx: SimContext; uid: string } {
  const ctx = makeTestCtx({
    ...
    modules: [
      moduleDef('mod-cpu-1', 'cpu', 0, { rack: 'low', cpuUse: 0, cpuBonus: 10 }),
      moduleDef('mod-cpu-2', 'cpu', 0, { rack: 'low', cpuUse: 0, cpuBonus: 15 }),
      moduleDef('mod-cpu-3', 'cpu', 0, { rack: 'low', cpuUse: 0, cpuBonus: 20 }),
      moduleDef('eat-40', 'armor', 0, { rack: 'low', armorHpBonus: 0.2, cpuUse: 40 }),
      moduleDef('eat-20', 'armor', 0, { rack: 'low', armorHpBonus: 0.1, cpuUse: 20 }),
...
describe('协处理器（低槽 CPU 预算扩容件）', () => {
  it('自身不占 CPU、只加预算：MK1/MK2/MK3 = +10/+15/+20，装配占用恒为 0', () => {
    ...
    expect(cpuBudgetOf(state, ctx, uid)).toBe(110)
    // 原子换装升档：MK1 → MK3（预算 110 → 120），占用仍 0
    addModule(state, 'mod-cpu-3')
    expect(swapModuleAt(state, 'mod-cpu-3', ctx, { rack: 'low', index: 0, shipId: uid }).ok).toBe(true)
    expect(cpuBudgetOf(state, ctx, uid)).toBe(120)
    expect(fittedCpuUsed(state.fleet[uid]!.fitted, ctx)).toBe(0)
    // 三档叠加也成立（多件全额叠加）：MK2 再装一件 → 120 + 15 = 135
    addModule(state, 'mod-cpu-2')
    expect(fitModule(state, 'mod-cpu-2', ctx, { shipId: uid }).ok).toBe(true)
    expect(cpuBudgetOf(state, ctx, uid)).toBe(135)
  })

  it('预算真能用：100 CPU 只装得下 40+60，装协处理器 MK3 后 20 的件也装得下', () => {
    const { state, ctx, uid } = world()
    fillToBudget(state, ctx, uid)
    addModule(state, 'eat-20')
    const denied = fitModule(state, 'eat-20', ctx, { shipId: uid }) // 100 + 20 > 100
    expect(denied.ok).toBe(false)
    expect(denied.error).toContain('协处理器') // 拒绝文案点名扩容这条路
    addModule(state, 'mod-cpu-3')
    expect(fitModule(state, 'mod-cpu-3', ctx, { shipId: uid }).ok).toBe(true) // 预算 120
    expect(fitModule(state, 'eat-20', ctx, { shipId: uid }).ok).toBe(true) // 120 = 120 正好
    expect(fittedCpuUsed(state.fleet[uid]!.fitted, ctx)).toBe(120)
    expect(cpuBudgetOf(state, ctx, uid)).toBe(120)
  })
  ...
  it('防套利（船长点名）：扩容被吃满后**卸不掉协处理器**，腾出别的件才卸得掉', () => {
    ...
    addModule(state, 'eat-20')
    expect(fitModule(state, 'eat-20', ctx, { shipId: uid }).ok).toBe(true) // 占用 120 = 预算
    // 预演：卸下协处理器 ⇒ 预算回 100 < 占用 120 ⇒ 拒绝，并给出可读原因
    ...
    expect(cpuBudgetOf(state, ctx, uid)).toBe(120) // 拒绝后状态没变
    // 先卸 20 的件（占用回到 100 = 船体预算）⇒ 再卸协处理器（预算 100 = 占用 100）通过
    const smallBay = bayOf(state, uid, 'eat-20')
  ...
  it('无人机放飞共用同一份预算：扩容后可多放一架，且超载时连协处理器也卸不掉', () => {
    const { state, ctx, uid } = world()
    ...
    addModule(state, 'mod-cpu-3')
    expect(fitModule(state, 'mod-cpu-3', ctx, { shipId: uid }).ok).toBe(true) // 预算 120
    expect(adjustDroneLoad(state, ctx, 'drone-x', 2, uid).ok).toBe(true) // 100 + 20 = 120
    expect(adjustDroneLoad(state, ctx, 'drone-x', 1, uid).ok).toBe(false) // 第 3 架超预算
    // 无人机占用计入 ⇒ 此刻卸协处理器会让预算回 100 < 占用 120 ⇒ 拒绝
    const cpuBay = bayOf(state, uid, 'mod-cpu-3')
    expect(unfitAt(state, cpuBay.rack, cpuBay.index, uid, ctx)).toBe(false)
  ...
  it('换装按**最终状态**一次预演：会超载的换装被拒，合法换装不被中间态卡住', () => {
    ...
    addModule(state, 'eat-20')
    expect(fitModule(state, 'eat-20', ctx, { shipId: uid }).ok).toBe(true) // 占用 120 = 预算
    // 降档换装 MK3 → MK1：最终预算 110 < 占用 120 ⇒ 拒绝（原子预演按最终态判，不留中间态漏洞）
    addModule(state, 'mod-cpu-1')
    const cpuBay = bayOf(state, uid, 'mod-cpu-3')
    const bad = swapModuleAt(state, 'mod-cpu-1', ctx, { ...cpuBay, shipId: uid })
    expect(bad.ok).toBe(false)
    expect(bad.error).toContain('CPU 超载')
    expect(cpuBudgetOf(state, ctx, uid)).toBe(120) // 被拒后原装配未动
    // 腾出 20 的件后（占用 100）再换 MK1（110 ≥ 100）⇒ 通过
    const smallBay = bayOf(state, uid, 'eat-20')
    expect(unfitAt(state, smallBay.rack, smallBay.index, uid, ctx)).toBe(true)
