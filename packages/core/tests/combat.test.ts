...
describe('V18B 随机目标与"尸体不承接火力"（船长 2026-09-05）', () => {
  it('多编队：我方命中分散到多个目标；主体已死后火力绝不瞄准尸体', () => {
    ...
    const tiger = ship('sh-tigershark', {
      cpu: 400,
    ...
    const ctx = makeTestCtx({
      ships: [tiger],
      modules: [tur],
      anomalies: [anomaly('ano-squad', 'galaxy-hub', { threat: 60, reward: 10_000, escorts: 2 })],
    })
    state.warehouse.items['ammo-kinetic-l'] = 5_000
    addModule(state, 'tur-f', 1)
    expect(fitModule(state, 'tur-f', ctx).ok).toBe(true)
    const battle = startBattleFor(state, ctx, 'sh-tigershark', 'ano-squad', 0)!
    // 阶段一：全员存活推进 6 秒 → 命中应分散到多个不同目标（随机目标 ≠ 只打主体）
    state.gameMs = 6_000
    advanceBattleFor(state, ctx, battle, 'sh-tigershark', 'ano-squad')
