...
function runBattle(
  ...
  state.gameMs =
    c.balance.battle.maxBattleMs +
    5_000 +
    waveGapTotalMs(c.anomalies.get(anomalyId), c.balance.battle);
...
describe("机群战损：无人机可被击落（2026-09-10 船长拍板，永久损失制）", () => {
  ...
  it("高威胁（有点防）会击落：机群递减、清单被扣除（战斗内可 100% 损坏，无单场上限）", () => {
    ...
    const calmCard: typeof hi = {
      ...hi,
      ships: hi.ships?.map((s) => ({
        ...s,
        ship: { ...s.ship, shotDmg: 0.2 },
      })),
    };
    const calmCtx: SimContext = {
      ...ctx,
      anomalies: new Map([...ctx.anomalies, [HIGH, calmCard]]),
    };
    ...
    // ⚠ **2026-09-11 反击制**（船长重新定义近防炮）+ 无人机血条 ×2 之后：敌方点防只在
    // 「出击型**每攻击一次换一次**无视射程的反击 / **哨戒机进射程**」时才开火 ⇒
    // **本场景是否产生战损属平衡读数**（由标定轮 P-41 回答），不再由单元用例钉死。
    // 零战损时下面那条"账本链路"无损坏可结算，直接收尾。
    ...
    const dronesInArcs = arcs.me
      .filter((w) => w.src === "drone")
      .reduce((s, w) => s + (w.count ?? 1), 0);
    ...
    for (const [id, n] of Object.entries(before)) {
      ...
      expect(load[id] ?? 0).toBe(
        Math.max(0, (LOAD as Record<string, number>)[id]! - row.gone),
      );
    }
    // 余数确实按价值优先：低价值机型拿到余数 ⇒ 更高价值机型必须已用满自己的余量
    const byValue = [...rep.rows].sort(
    ...
    expect(state.logs.some((l) => l.text.includes("优先回收高价值"))).toBe(
      true,
    );
    // 仓库中的补充库存不受影响（回港可再装）
  ...
  it("优先回收高价值（2026-09-11 船长）：余数名额给最贵的机型，其余净损失", () => {
    ...
    expect(rep.rows.map((r) => r.id)).toEqual([
      "drone-sentry",
      "drone-heavy",
      "drone-assault",
      "drone-scout",
  ...
  it("无距离豁免（2026-09-10 船长：放飞出去就在威胁之下）＋哨戒机优先豁免：先打非哨戒机", () => {
    ...
    const noPd: SimContext = {
      ...
      balance: {
        ...ctx.balance,
        battle: { ...ctx.balance.battle, pdThreatFloor: 999 },
      },
    };
    const stateA = makeState(11);
    const battleA = runBattle(stateA, HIGH, noPd)!;
    expect(Object.values(battleA.dronePools ?? {}).every((p) => p.alive)).toBe(
      true,
    );
    ...
    const nonSentryAlive = Object.values(battleB.dronePools ?? {}).some(
      (p) => p.alive && p.artId !== "drone-sentry",
    );
  ...
  it("结算幂等：同一场战斗重复结算不重复扣（清单不会扣穿）", () => {
    ...
    expect(settleDroneLosses(state, ctx, state.shipId, battle)).toBeNull();
    expect(JSON.stringify(state.fleet[state.shipId]!.droneLoad ?? {})).toBe(
      after,
    );
  });
});
