...
describe('敌冲锋（2026-09-10 定资格；2026-09-11 改"到达解除"；**2026-09-14 乙触发 · 逐单位 · 命中解除 · 冷却 10 秒**）', () => {
  ...
  it('C 族冲锋配置（2026-09-16 船长「C族全部添加冲锋，按照级别分别为1.5/2/2.5/3/4」）：按档契约（**走挂载件**）', () => {
    ...
    /**
     * **洞内条目级挂载的守卫：劫掠电子舰**（船长 2026-09-16「**新增A族敌人劫掠电子舰，添加挂载件冲锋**，
     * 并额外加装一件新的挂载件，劫掠捕获网…**添加进深层的海盗战团里**」＋ 2026-09-19
     * 「**海盗电子舰的冲锋也移除，只在洞内单独挂载**」）。
     * 两件（冲锋 ＋ 捕获网）都写在**深层战团的条目**上，舰级不带件——与 A 族另三条舰级同口径。
     * ⚠ 有效挂载是「条目 ?? 舰级」（**替换**不是叠加）⇒ 条目必须**两件都写**，只写一件会顶掉另一件。
     */
    const raider = FOE_SHIPS.find((s) => s.id === 'foe-pirate-raider')!
    expect(raider.mounts, '劫掠电子舰舰级不得带挂载件（2026-09-19 船长：只在洞内单独挂载）').toBeUndefined()
    const warband = WORMHOLE_FOE_CARDS.find((a) => a.id === 'wh-pirate-warband')!
    const raiderSlots = (warband.ships ?? []).filter((sl) => sl.ship.id === 'foe-pirate-raider')
    // 深层战团必须带它一条（船长「添加进深层的海盗战团里」）
    expect(raiderSlots, '深层战团应带劫掠电子舰 ×1').toHaveLength(1)
    const raiderMounts = resolveFoeMounts(raiderSlots[0]!.mounts ?? raiderSlots[0]!.ship.mounts)
    expect(raiderMounts.foeCanCharge, '战团条目应具冲锋资格').toBe(true)
    expect(raiderMounts.foeChargeMul, '战团条目冲锋倍率').toBe(1.6)
    expect(raiderMounts.foeChargeCooldownMs, '战团条目冲锋冷却').toBe(30_000)
    expect(raiderMounts.foeCaptureWeb, '战团条目应带捕获网件（两件都得写）').toBeDefined()
    ...
    for (const a of ANOMALIES_FLAVORED) {
      ...
      for (const sl of a.ships ?? []) {
        expect(sl.ship.id, `洞外卡 ${a.id} 不得用劫掠电子舰（深层战团专属舰级）`).not.toBe('foe-pirate-raider')
      }
    }
  })
