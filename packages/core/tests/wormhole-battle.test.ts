...
describe('虫洞 · 洞内敌卡按层派生（F 批）', () => {
   ...
   * 写了模式的三张卡各挂 0.6；BOSS 的模式虽被 core 改成「打最大的」，概率同样 0.6（同一条裁定）；
   * 随机模式的 G 卡 / E 卡**不写**该字段 ⇒ 派生结果 = 1（不掷骰，行为与上线时逐字一致）。
   */
  it('选靶倾向概率：写了模式的卡 0.6 · BOSS 也 0.6 · 随机模式的卡不写（= 1）', () => {
    const pirate = ctx.anomalies.get('wh-pirate-scout')!
    const grave = ctx.anomalies.get('wh-grave-watch')!
    const exile = ctx.anomalies.get('wh-exile-blockade')!
    const titan = ctx.anomalies.get('wh-titan-echo')!
    expect(pirate.foeTargetingChance).toBe(0.6)
    expect(grave.foeTargetingChance).toBe(0.6)
    expect(exile.foeTargetingChance).toBeUndefined()
    expect(titan.foeTargetingChance).toBeUndefined()
    // 派生：普通节点 / 撤离战 = 卡上概率；BOSS = core 常量（同样 0.6，模式仍是"打最大的"）
    expect(wormholeAnomalyOf(pirate, 3, 'node', 1).foeTargetingChance).toBe(0.6)
    expect(wormholeAnomalyOf(pirate, 3, 'extract', 1).foeTargetingChance).toBe(0.6)
    expect(wormholeAnomalyOf(pirate, 3, 'boss', 1).foeTargetingChance).toBe(WORMHOLE_BOSS_TARGETING_CHANCE)
    expect(WORMHOLE_BOSS_TARGETING_CHANCE).toBe(0.6)
