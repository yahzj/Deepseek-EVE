 ...
 *    弹药 MK2 一张单 1~3 发 ⇒ **100 单 ≈ 200 发 × 45 = 9,000** = 它的书价
 *    ⇒ **倍率 = 一单货值 × 2 = 200 件货值**。本批四条同法：
 *    **书价 = 一单（100 架）货值 × 2**，且与 `blueprintTierCoefOf` 的「民用/基础档 ×2」同值
...
describe('制式无人机永久图纸（2026-09-26 船长令）', () => {
  ...
  it('书价 = 一单货值 ×2（= 弹药「100 单」口径：200 件货值），与市场行同值', () => {
    const ammo = bpOf('bp-ammo-kinetic-2') // 锚：弹药 MK2 —— 100 单 ≈ 200 发 × 45 = 9,000 = 它的书价
    const ammoUnit = itemOf(ammo.itemId!).baseSellPriceIsk!
    expect(ammoUnit * 200).toBe(9_000)
    expect(ammo.priceIsk).toBe(9_000)
    for (const id of DRONE_BPS) {
      const p = PRODUCTS[id]!
