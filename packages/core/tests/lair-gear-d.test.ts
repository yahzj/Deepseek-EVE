/**
 * D 族「守墓古舰」专属装备（2026-09-10 船长逐件过审：D 族为最强敌族，可对标异星档、略强）：
 * - D1 陵墓护盾阵列：护盾层三系抗各 **+35%**（缺口复合、0.9 封顶）——全能重盾，代价 = 42 CPU；
 * - D2 守墓者长炮：**基础命中 100%** 的 12 km 点名炮——射速降到攻坚炮台 MK3 的四成、
 *   单发提到 2.24 倍，**总输出 ≈ MK3 的九成（−10.3%）**，远端衰减更轻；
...
describe('D 族守墓古舰专属装备（2026-09-10 船长）', () => {
  ...
  it('D1 陵墓护盾阵列：护盾三系抗各 +35%（缺口复合）、CPU 42；装甲/结构不受影响', () => {
    ...
    for (const t of ['kinetic', 'explosive', 'plasma'] as const) {
      ...
      // 缺口乘入：new = 1 − (1−base) × (1−0.35)
      expect(after).toBeCloseTo(1 - (1 - base) * 0.65, 6)
      expect(after).toBeGreaterThan(base)
    }
    // 灰鲭鲨基础盾抗只有动能 0.5 → 装后动能 0.675（缺口剩一半再削 35%）、另两系各 0.35
    expect(withShield.resists.shield!.kinetic).toBeCloseTo(0.675, 6)
    expect(withShield.resists.shield!.explosive).toBeCloseTo(0.35, 6)
    expect(withShield.resists.shield!.plasma).toBeCloseTo(0.35, 6)
    // 装甲/结构层不被盾件影响
  ...
  it('D2 守墓者长炮：射速 −60%、单发 2.24 倍、总输出 ≈ 攻坚炮台 MK3 的九成', () => {
    ...
    // 单发：两倍多（2.24 倍）
    expect(d2.dmgMult! / mk3.dmgMult!).toBeCloseTo(2.242, 2)
    // 总输出 = −10.3%（船长定「保证 DPS −10%」）
    const ratio = dps(d2) / dps(mk3)
    expect(ratio).toBeGreaterThan(0.88)
    expect(ratio).toBeLessThan(0.92)
    expect(ratio - 1).toBeCloseTo(-0.1033, 3)
  })
