...
const hpOf = (u: { hp: { s: number; a: number; h: number } }): number =>
  u.hp.s + u.hp.a + u.hp.h;
...
describe("舰级路径：允许混编", () => {
  ...
  it("tag 口径与旧路径同规则：首队 foe-0、同波后续小队 w0-foe-{k}、僚机跟最近主体", () => {
    const specs = createFoeSpecs(mixedCard(40), bal);
    expect(specs.map((s) => s.tag)).toEqual([
      "foe-0",
      "w0-foe-1",
      "w0-foe-2",
      "w0-foe-3",
  ...
  it("分波：slot.wave 指定第几波，前缀 w{n}- 与旧多波口径一致", () => {
    const w1 = createFoeSpecs(
      { ...mixedCard(40), ships: [{ ship: BOSS, wave: 1 }] },
      bal,
      { tagPrefix: "w1-" },
    );
...
describe("舰级路径：头目可配多战术（2026-09-11 船长「头目建议允许多个战术」）", () => {
  ...
  it("混编可各用各的战术（狙击头目 + 贴脸杂鱼同场）", () => {
    ...
    expect(createFoeSpecs(def, bal).map((s) => s.foeTactic)).toEqual([
      "kite",
      "brawl",
      "brawl",
    ]);
...
describe("旧路径不受影响（未写 ships 的卡）", () => {
  it('仍按威胁推导建档，命名仍走"战术 × 血型"舰种名', () => {
    const def = anomaly("ano-t-legacy", "galaxy-hub", {
      threat: 20,
      tactic: "brawl",
    });
    const specs = createFoeSpecs(
      { ...def, defProfile: "armor", foeHpOverride: 300 },
      bal,
    );
...
describe("舰种档与速度倍率（A 族提速 / B 族偏慢 / C 族更快）", () => {
  ...
  it("A 族每档实速都**高于本档舰种基准**（护卫 340 / 驱逐 295 / 巡洋 258）——船长「速度都快」", () => {
    ...
    for (const ship of pirates) {
      ...
      const spd = createFoeSpecs({ ...shell, ships: [{ ship }] }, bal)[0]!
        .speedMps;
      expect(spd).toBeGreaterThan(
  ...
  it("B 族（武装拾荒者）每档实速都**低于**本档舰种基准——船长「速度偏慢」＋新手过渡族", () => {
    const shell = anomaly("ano-t-hull-undershoot", "galaxy-hub", {
      threat: 20,
    });
  ...
  it("舰种档与倍率口径：实速 = round(基准 × 倍率 × 条目 speedMul)（倍率按精确分数登记）", () => {
    for (const ship of FOE_SHIPS) {
      ...
      expect(
        createFoeSpecs(
          {
            ...anomaly("ano-t-hull-mul", "galaxy-hub", { threat: 20 }),
            ships: [{ ship }],
          },
          bal,
        )[0]!.speedMps,
      ).toBe(Math.round(base * ship.speedRatio));
      // 条目 speedMul：与原绝对值口径同规则（乘后取整）
      const withMul = createFoeSpecs(
        {
          ...anomaly("ano-t-hull-mul2", "galaxy-hub", { threat: 20 }),
          ships: [{ ship, speedMul: 1.25 }],
        },
        bal,
      )[0]!.speedMps;
...
describe("多舰船补偿：舰级路径按 N 缩放单发，旧路径不动", () => {
  ...
  it("N=1 不生效；N=4 → ×1.6；N=5 → ×5/3（单发 = round(舰级单发 × dmgMul × 补偿)）", () => {
    ...
    const four = createFoeSpecs(
      { ...shell, ships: [{ ship: SKIFF, count: 4 }] },
      bal,
    );
    expect(four.map((u) => u.tag)).toEqual([
      "foe-0",
      "w0-foe-1",
      "w0-foe-2",
      "w0-foe-3",
    ...
    expect(five.map((u) => u.weapons[0]!.shotDmg)).toEqual([
      100, 17, 17, 17, 10,
    ]);
  });
  ...
  it("混编卡：头目与杂鱼同吃补偿；多舰补偿不改变血量/速度/射程口径", () => {
    ...
    expect(specs.map((u) => u.weapons[0]!.maxRangeM)).toEqual([
      2600, 2200, 2200, 2200, 2200,
    ]);
  });

  it("旧威胁推导路径**不启用**补偿：单发逐字等于旧推导链（威胁 40 → 93）", () => {
    const def = anomaly("ano-t-legacy-comp", "galaxy-hub", {
      threat: 40,
      tactic: "brawl",
    });
    const spec = createFoeSpecs(
      { ...def, foeHpOverride: 300, defProfile: "armor" },
      bal,
    )[0]!;
...
describe("赤潮 / 蜃影 火力重锚（船长 2026-09-11 裁决：按实际算 + 纸面锚 ×1.2）", () => {
  ...
  const realCard = (id: string): AnomalyDef =>
  ...
  const shots = (id: string): number[] =>
    createFoeSpecs(realCard(id), bal).map((u) => u.weapons[0]!.shotDmg ?? 0);
  ...
  it("重锚只动 dmgMul：血/编成/头目占比不受影响（`foeDmgMul` 字段保持退休）", () => {
    for (const id of ["ano-redring-raiders", "ano-mirage-hijackers"]) {
      ...
      expect(
        card.ships!.reduce(
          (n, s) => n + Math.max(1, Math.floor(s.count ?? 1)),
          0,
        ),
      ).toBe(4);
...
describe("血型随卡走 + 头目射程多重方案（船长 2026-09-11 裁决①③）", () => {
  ...
  it("条目 `split` 覆写生效；缺省仍走舰级 split", () => {
    const def: AnomalyDef = {
      ...
      ships: [
        { ship: SKIFF, split: { s: 0.5, a: 0.25, h: 0.25 } },
        { ship: SKIFF },
      ],
    };
...
describe("期望交距（舰级路径取自身射程带 · 2026-09-11 船长裁决②）", () => {
  const bandOf = (u: {
    weapons: { minRangeM: number; maxRangeM: number }[];
  ...
  it("旧路径一字不动：brawl 440m / orbit 2688m / kite 8000m（全局战术表口径）", () => {
    ...
    for (const [tactic, desire] of want) {
      const shell = anomaly(`ano-t-desire-${tactic}`, "galaxy-hub", {
        threat: 20,
      });
      const specs = createFoeSpecs(
        { ...shell, tactic: tactic as "brawl" | "orbit" | "kite" },
        bal,
      );
      expect(
        specs[0]!.foeRangeBand,
        "旧路径不得携带 foeRangeBand",
      ).toBeUndefined();
  ...
  it("舰级路径：期望交距 = 自身射程带的带内偏好位置（不再取全局表）", () => {
    ...
    const shell = anomaly("ano-t-desire-ship", "galaxy-hub", {
      threat: 20,
      tactic: "brawl",
    });
    ...
    const orb = createFoeSpecs(
      { ...shell, tactic: "orbit", ships: [{ ship: SKIFF, tactic: "orbit" }] },
      bal,
    );
  ...
  it('射程覆写 = 纠偏旋钮：条目 rangeMinM/rangeMaxM 改写带 ⇒ 期望交距随之（"同卡同带"）', () => {
    const shell = anomaly("ano-t-desire-ovr", "galaxy-hub", {
      threat: 20,
      tactic: "kite",
    });
    const specs = createFoeSpecs(
      // 条目同时覆写**射程带**与**战术**（舰级路径的战术取自"条目 ?? 舰级"，不是卡面）
      {
        ...shell,
        ships: [
          { ship: SKIFF, tactic: "kite", rangeMinM: 1000, rangeMaxM: 4000 },
        ],
      },
      bal,
    );
  ...
  it("全 14 张舰级路径卡：期望交距必须落在**自身射程带内**（否则敌人站在自己打不到的位置）", () => {
    ...
    for (const def of ANOMALIES) {
      ...
      expect(
        desire,
        `${def.id} 的期望交距 ${desire}m 落在自身射程带 ${band.min}~${band.max}m 之外`,
...
describe("C 族（异形生物）：虫群编成 + 稀有头目 + 总盘守恒", () => {
  ...
  it("能量形态覆写（裁定⑤）：缺省 = 光束必中（不消费命中）；`spit` = 掷命中（消费命中、吃回避）", () => {
    ...
    const overridden = createFoeSpecs(
      { ...shell, ships: [{ ship: spitShip, energyForm: "beam" }] },
      bal,
    )[0]!.weapons[0]!;
  ...
  it("`energyForm` 只作用于能量主系：动能/爆炸主系两种形态下都恒为掷命中", () => {
    ...
    for (const energyForm of ["beam", "spit"] as const) {
      const ship: FoeShipDef = {
        ...SKIFF,
        id: `t-foe-kinetic-${energyForm}`,
        energyForm,
      };
      const w = createFoeSpecs({ ...shell, ships: [{ ship }] }, bal)[0]!
        .weapons[0]!;
  ...
  it('速度口径（裁定②③）：倍率带 1.30~2.10、同档快于 A 族最快、T4 走"巨兽"白名单且允许慢', () => {
    ...
    for (const s of FOE_SHIPS.filter((x) => x.family === "A")) {
      const spd = Math.round(
        bal.hullClassBaseSpeedMps[s.hullClassTier] * s.speedRatio,
      );
      fastestA.set(
        s.hullClassTier,
        Math.max(fastestA.get(s.hullClassTier) ?? 0, spd),
      );
    }
    ...
    for (const s of aliens) {
      ...
      if (s.hullClassTier === 4) {
        expect(
          ...
          `${s.id} 用 T4 必须登记为"巨兽"用途`,
        ).toContain(s.id);
      } else {
        const a = fastestA.get(s.hullClassTier)!;
        expect(
          spd,
          `${s.id} 实速 ${spd} 未高于 A 族同档最快 ${a}`,
        ).toBeGreaterThan(a);
      }
    }
  });
