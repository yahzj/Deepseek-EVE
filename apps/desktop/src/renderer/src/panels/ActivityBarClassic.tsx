/**
 * **旧版（classic）活动栏 —— 从「昨天那版」原样拆出**（2026-09-25 · ui-redesign-2）
 *
 * 船长令：「旧版建议你从昨天的版本中 git 下来进行拆解」（另：「不能将各个窗口模块化吗」
 * ⇒ 裁定甲：**只把窗口模块化，两套外壳先保留**）。
 *
 * 来源：`git show 99eeb93e:apps/desktop/src/renderer/src/panels/ActivityBar.tsx`
 * （昨天 09-24 与合并前 main 的这份**完全相同**；只有"组件名"改成了 `ActivityBarClassic`）。
 *
 * 与新版 `ActivityBar.tsx` 的差异（**这就是船长要的"旧版活动窗口"**）：
 *   · 有标题栏（「活动」+ 副船/工业两枚 AI 徽标）；
 *   · 只有「玩家活动」「技能训练」两组（**没有**新版的「计时中」组）；
 *   · 每行右侧带操作按钮（停扫 / 移除 …）；
 *   · **不显示副AI活动迷你卡**（AI 只用徽标计数）。
 *
...
export function ActivityBarClassic({
  engine,
  onToast,
  onAiCenter,
  onGoPage,
