 ...
 *   系统保留进行测试」** ⇒ 当批**默认关**（阶梯表 / `foeShipTierOf` 反查 / 逐单位布局全部保留就位）。
 *   为什么当时"距离不准"：距离尺本身是米 → 百分比（与舰体无关），但**同一米数对应的视觉缺口**
 *   是按统一 170/90 舰体标定的；舰体一大一小之后，读数（米）与观感（缺口）不再对应。
 *
 * ✅ **2026-09-14 船长改判：打开（「直接打开阶梯（发布默认改 true）」）** ——
 *   船长确认他指的就是"战斗里舰体大小看起来都一样大"这一处（原话问：「好像之前关闭了舰船大小区分？」），
 *   并选择直接打开而不是先测；**已知代价照旧成立**（距离尺读数与视觉缺口不再严格对应，
 *   要让两者重新对应 = 按逐舰体积重标距离尺，属另一件活，船长已知）。 */
...
/** 代码里的正式默认值（发布口径）：**开** = 按舰种档缩放（2026-09-14 船长改判）；
 *  改 `false` 即回到"统一 170/90"的还原口径（阶梯表与逐单位布局仍全部就位）。 */
const SIZE_BY_TIER_DEFAULT = true
/** 缓存的开关值（首次调用时读一次；测试/探针可在首次调用前注入 `localStorage` 桩） */
let sizeTierFlag: boolean | null = null
/**
 * 是否启用「舰种体积」——默认**是**（2026-09-14 船长改判）；临时改回还原口径可由
 * `localStorage['whale-idle:ship-size-tier'] = '0'` 免重建关闭（刷新生效）。
 */
