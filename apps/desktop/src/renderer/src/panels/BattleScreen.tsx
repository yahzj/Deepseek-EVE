...
function droneHomeStation(
  ...
  const slot =
    model.slots[
      lane % Math.max(1, Math.min(model.slots.length, DRONE_SHOW_MAX))
    ] ?? model.slots[0]!;
...
function dronePoseAt(
  ...
  const base =
    DRONE_STYLE === "sortie" && !model.resident
      ? droneTakeoff(lane)
      : model.slots[lane % model.slots.length]!;
  ...
  const station =
    DRONE_STYLE === "sortie" && !model.resident
      ...
      : droneHomeStation(model, lane, lay);
...
export function BattleScreen({
  ...
  const thruster = battle
    ? thrusterPhase(battle, engine.ctx.balance.battle)
    : null;
  ...
  const moveSnapRef = useRef<{
    prev: { m: number; w: number } | null;
    cur: { m: number; w: number } | null;
  }>({
    prev: null,
    cur: null,
  });
  ...
  const mapRef = useRef<{ openM: number; nearM: number }>({
    openM: 1,
    nearM: 200,
  });
  ...
  useEffect(() => {
    ...
    if (stage === "live") {
      ...
      droneReportRef.current =
        dr && (!snap || dr.battleStartedAtGameMs === snap.startedAtGameMs)
          ? dr
          : null;
      const report =
  ...
  useEffect(() => {
    const iv = window.setInterval(() => {
      ...
      if (++widthCheckRef.current >= 10) {
        ...
        if (
          (fw && Math.abs(fw - dimsRef.current.foeW) > 2) ||
          (mw && Math.abs(mw - dimsRef.current.meW) > 2)
        ) {
          measureRef.current();
        }
      }
      setSmoothM((old) =>
        old === null || Math.abs(old - vis) >= 0.05 ? vis : old,
      );
      ...
      const boostNow = thrusterPhase(b, engine.ctx.balance.battle).boosting
        ? thrusterBoostRef.current
        : 0;
      const vMag = Math.min(
        320,
        40 + meSpeedRef.current * (1 + boostNow) * 0.32,
      ); // 40px/s 底速 + 船速比例
      ...
      for (let i = 0; i < STAR_LAYERS.length; i++) {
        ...
        if (el)
          el.style.transform = `translate3d(${-offs[i].toFixed(1)}px, 0, 0)`;
      }
    }, 33);
  ...
  useEffect(() => {
    return () => {
      ...
      if (v !== null && m.openM > 1) {
        engine.battleSetDesireAt(
          Math.round(m.openM - (v / 1000) * (m.openM - m.nearM)),
        );
      }
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
            ...
            {droneReport ? (
              ...
              <div className="app-bts-report-stats is-loss">
                ...
                {Object.entries(snap.droneLost)
                  .map(
                    ([artId, n]) =>
                      `${droneModelOf(artId)?.name ?? artId} ×${n}`,
                  )
                  .join("、")}
                （无人机舱清单已扣除，回港需补充）
              </div>
            ) : null}
            <div className="app-bts-report-note">
              奖励/战利品已入账，舰队自动返航中；本报告 12
              秒后自动关闭（完整记录见右侧事件日志）。
            </div>
  ...
  if (!initedFxRef.current) {
    const tail =
      battle.fx.length > 0 ? battle.fx[battle.fx.length - 1] : undefined;
  ...
  const foeAnomaly = state.expedition.anomalyId
    ? engine.ctx.anomalies.get(state.expedition.anomalyId)
    : undefined;
  ...
  const foeNameOf = (tag: string): string =>
    foeAnomaly
      ? foeUnitNameOf(foeAnomaly, tag)
      : (combat.foeHp[tag]?.name ?? tag);
  const ended = battle.ended !== null;
  ...
  const rowFxTags = foeTags.filter(
    (t) => !deadRef.current.has(t) || !dropNow.has(t),
  );
  ...
  if (myGap > 2)
    facingRef.current.meFlip = true; // 我方想拉开 → 掉头背向
  else if (myGap < -2) facingRef.current.meFlip = false; // 我方想接近 → 船头朝敌
  if (foeGap > 2)
    facingRef.current.foeFlip = false; // 敌想拉开 → 掉头背向
  ...
  if (arrivals.length > 0) {
    ...
    for (const fx of arrivals) {
      ...
      const aimTag = isMeShot
        ? (fx.to ?? foeAliveTags[0])
        : (fx.to ?? "player");
      ...
      if (dm) {
        ...
        if (DRONE_STYLE === "sortie" && !dm.resident) {
          ...
          from = droneHomeStation(
            dm,
            n % Math.max(1, Math.min(dm.slots.length, DRONE_SHOW_MAX)),
            layFx,
          );
        }
      } else {
        mounts = isMeShot
          ? mountsOf(meShip?.id, undefined)
          : mountsOf(undefined, foeKey);
    ...
    if (flashRef.current.length > 6)
      flashRef.current.splice(0, flashRef.current.length - 6);
  }
  // 惰性清理过期元素（渲染输出不再包含它们即从 DOM 移除；延迟弹道按 delay 延长存活）
  boltsRef.current = boltsRef.current.filter(
    (b) => now - b.born < BOLT_LIFE + (b.delay ?? 0),
  );
  flashRef.current = flashRef.current.filter(
    (f) => now - f.at < FLASH_LIFE + (f.delay ?? 0),
  );
  ...
  if (!hpInitRef.current) {
    ...
    for (const tag of foeTags) {
      ...
      if (sum === 0 && prev > 0 && !deadRef.current.has(tag)) {
        ...
        const killerFly =
          (killerType ? BOLT_LOOK[killerType]?.fly : undefined) ?? FLY_MS;
  ...
  const foeRowTags = foeTags.filter(
    (t) => !deadRef.current.has(t) || !dropFinal.has(t),
  );
  ...
  const waveNext =
    wavePending && foeAnomaly?.waves && foeAnomaly.waves.length > 1
      ? (battle.waveIdx ?? 0) + 2
      : 0;
  ...
  const arcR = (rangeM: number, minPx: number): number =>
    Math.max(
      minPx,
      Math.min(arcCap, gunBasePx + Math.max(0, rangeM - nearM) * sPxPerM),
    );
    ...
    return (
      <g key={`me${wi}`} opacity={(hollow ? 0.55 : 1) * dim}>
        <path
          d={fanPath(r0, r1)}
          fill={color}
          fillOpacity={hollow ? 0 : 0.12}
        />
        <path
          d={ringPath(r1)}
          fill="none"
          stroke={color}
          strokeWidth={hollow ? 1.2 : 2}
  ...
  const foeR0 =
    arcs.foe.minM >= nearM
      ? Math.min(arcR(arcs.foe.minM, 12), foeR1 - 4)
      : Math.min(12, foeR1 - 4);
  ...
  const repairTotal = repairRt
    ? Object.values(repairRt.kits).reduce((a, b) => a + b, 0)
    : 0;
  const repairRunning = repairRt
    ? repairRt.units.some((u) => !u.stopped)
    : false;
  ...
  const sliderToDesire = (v: number): number =>
    Math.round(openM - (v / 1000) * (openM - nearM));
  ...
  const secs = Math.max(
    1,
    Math.round((state.gameMs - battle.startedAtGameMs) / 1000),
  );
  ...
  const boltEls = boltsRef.current.map((bv) => {
    ...
    const flyMs = dm
      ? Math.max(60, Math.round(look.fly * (dm.bolt.flyMul ?? DRONE_FLY_MUL)))
      : look.fly;
    if (dm && dm.bolt.style === "beam") {
      return (
        <div
          key={bv.key}
          className={`app-bts-bolt is-${bv.type} is-drone is-sentry`}
          style={{
            left: bv.x1,
            top: bv.y1,
            transform: `rotate(${bv.angDeg}deg)`,
          }}
        >
          <i
    ...
    if (dm) {
      return (
        <div
          key={bv.key}
          className={`app-bts-bolt is-${bv.type} is-drone`}
          style={{
            left: bv.x1,
            top: bv.y1,
            transform: `rotate(${bv.angDeg}deg)`,
    ...
    return (
      <div
        key={bv.key}
        className={`app-bts-bolt is-${bv.type}`}
        style={{
          left: bv.x1,
          top: bv.y1,
          transform: `rotate(${bv.angDeg}deg)`,
      ...
      const phase: "out" | "back" | "deck" = model.resident
        ...
        : elapsed < DRONE_SORTIE_OUT_MS + DRONE_DWELL_MS
          ...
          : elapsed < cycleMs
            ? "back"
            : "deck";
      const show = model.resident
        ? 1
        : Math.max(1, Math.min(w.count ?? 1, DRONE_SHOW_MAX));
  ...
  const foeUnitEls = foeRowTags.map((tag) => {
    ...
    const fadeT =
      sinceBoom >= BOOM_LIFE
        ? clamp01((sinceBoom - BOOM_LIFE) / WRECK_FADE_MS)
        : 0;
    return (
      <div
        key={tag}
      ...
      >
        ...
        <span
          className="app-bts-corpse"
          style={fadeT > 0 ? { opacity: Math.max(0, 1 - fadeT) } : undefined}
        >
          <ShipSprite
            foeKey={foeKey}
            flip={foeFlip}
        ...
        <span
          className="app-bts-name"
          style={{ color: isMain ? "#ffb3a6" : "#d8a08f" }}
        >
    ...
    <div className="app-battle-screen">
      <div className="app-battle-screen-top">
        {stage === "live" ? (
            ...
            <button
              ...
              onClick={() => {
                if (!retreatAsk) {
                  setRetreatAsk(true);
                  onToast(
                    "撤退 = 轻损脱离（仅损失少量舰船耐久、无弃船风险）——再点一次确认。",
                    true,
                  );
                  return;
                }
          ...
          <span className="app-bts-outro-tag">
            {ended
              ? defeat
                ? "战斗结束 · 正在撤离…"
                : "战斗结束 · 正在结算…"
        ...
        <span className="app-dim">
          交火 {secs}s · 我方开火 {meStats.meShots}/命中 {meStats.meHits} ·
          敌开火 {meStats.foeShots}/命中 {meStats.foeHits}
        </span>
      </div>
      ...
      <div className="app-bts-stage">
        ...
        <div className="app-bts-ruler">
          <div className="app-bts-ruler-head">
            <span className="app-dim">
              ◀ 拉开（远 {Math.round(openM).toLocaleString("zh-CN")}m）
            </span>
            <span className="app-dim">
              贴脸（近 {Math.round(nearM).toLocaleString("zh-CN")}m）▶
            </span>
          </div>
          <div className="app-bts-scale">
            <i
              className="app-bts-zone is-me"
              style={{
                left: `${pct(mainMeArc?.maxM ?? openM)}%`,
                width: `${Math.max(0.6, pct(mainMeArc?.minM ?? 0) - pct(mainMeArc?.maxM ?? openM))}%`,
              }}
              title={`我方主武器有效 ${mainMeArc?.minM ?? 0}~${mainMeArc?.maxM ?? 0}m`}
            />
            <i
              className="app-bts-zone is-foe"
              style={{
                left: `${pct(arcs.foe.maxM)}%`,
                width: `${Math.max(0.6, pct(arcs.foe.minM) - pct(arcs.foe.maxM))}%`,
              }}
              title={`敌方射程 ${arcs.foe.minM}~${arcs.foe.maxM}m`}
            />
        ...
        >
          ...
          >
            ...
            <g
              transform={`translate(${foeGunX} ${lay.foe[0]?.y ?? 0}) scale(-1 1)`}
              opacity={foeInBand ? 0.25 : 1}
            >
              <path
                d={fanPath(foeR0, foeR1)}
                fill={foeColor}
                fillOpacity={0.16}
              />
              <path
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* 爆散射线（八向短线） */}
                      <path
                        className="app-bts-wreck-rays"
                        d="M0 -6 L0 -10 M4.4 -4.4 L7.4 -7.4 M6 0 L10 0 M4.4 4.4 L7.4 7.4 M0 6 L0 10 M-4.4 4.4 L-7.4 7.4 M-6 0 L-10 0 M-4.4 -4.4 L-7.4 -7.4"
                      />
                      {/* 崩落碎屑（三片，各自下坠） */}
                      <path
                        className="app-bts-wreck-bit"
                        d="M-3 -1 l2.6 1.2 l-2.6 1.6 z"
                      />
                      <path
                        className="app-bts-wreck-bit is-b"
                        d="M1.6 -2.2 l2.4 1 l-2.2 1.8 z"
                      />
                      <path
                        className="app-bts-wreck-bit is-c"
                        d="M-0.6 2 l2.2 1 l-2 1.6 z"
                      />
                    </svg>
                  </span>
                );
          ...
          <div
            className="app-bts-col is-foe"
            ref={foeColRef}
            style={{ left: lay.foeLeft }}
          >
          ...
          {wavePending ? (
            <span
              ...
              style={{
                left: lay.foe[0]?.x ?? lay.me.x,
                top: (lay.foe[0]?.y ?? lay.me.y) + 42,
              }}
            >
              {waveNext > 0
                ? `第 ${waveNext}/${foeAnomaly?.waves?.length} 波增援正在接近…`
                : "敌方增援正在接近…"}
            </span>
          ) : null}
        ...
        <div className="app-bts-dock">
          <div className="app-bts-legends">
            {arcs.me.map((w, wi) => (
              ...
              >
                <i
                  style={{ background: w.type ? DMG_COLOR[w.type] : "#93a4b8" }}
                />
                {w.label} {w.minM.toLocaleString("zh-CN")}~
                {w.maxM.toLocaleString("zh-CN")}m
                {w.kind === "gun" ? (
                  w.type ? (
                    <span className={`app-a-chip app-a-${w.type}`}>
                      {DMG_LABEL[w.type]}弹
                    </span>
                  ) : (
                    "（无弹）"
            ...
            >
              ...
              敌方 {arcs.foe.minM.toLocaleString("zh-CN")}~
              {arcs.foe.maxM.toLocaleString("zh-CN")}m
              <span className={`app-a-chip app-a-${arcs.foe.type}`}>
                {DMG_LABEL[arcs.foe.type]}
              </span>
            </span>
            ...
            {battle?.foeChargeOn ? (
              <span
                className="app-bts-repair is-down"
                title="敌方正在突进：够不着你时机动翻倍逼近——进入其射程后仍会维持 2 秒，随后冷却 20 秒"
              >
                <i /> 敌突进中
              </span>
            ) : null}
              ...
              <span className="app-bts-ammo">
                {ammoChips.map((t) => (
                  <span key={t} className={`app-a-chip app-a-${t}`}>
                    {arcs.ammoNames?.[ammoKey(t)] ?? DMG_LABEL[t]}×
                    {arcs.ammo[ammoKey(t)].toLocaleString("zh-CN")}
                  </span>
                ))}
              </span>
            ) : null}
            {repairRt ? (
              ...
              >
                ...
                <span className="app-dim">
                  组件 ×{repairTotal.toLocaleString("zh-CN")}
                </span>
              </span>
            ) : null}
          </div>
          ...
          <div className="app-bts-reloads">
            {arcs.me.map((w, wi) => {
              ...
              const pct = ready
                ...
                : Math.min(
                    ...
                    Math.max(
                      0,
                      ((w.reloadMs - remain) / Math.max(1, w.reloadMs)) * 100,
                    ),
                  );
                ...
                >
                  ...
                  <span className="app-bts-reload-track">
                    <i
                      ...
                      style={{
                        width: `${pct}%`,
                        background: ready ? "#6fd98a" : dotColor,
                      }}
                    />
                  </span>
            ...
            {thruster && arcs && arcs.thrusterBoost > 0 ? (
              ...
              >
                ...
                <span className="app-bts-reload-ms">
                  {thruster.boosting
                    ? `推进 ${Math.max(0.1, Math.ceil(thruster.remainMs / 100) / 10)}s`
                    : `${Math.max(0.1, Math.ceil(thruster.remainMs / 100) / 10)}s`}
                </span>
              </span>
            ) : null}
          </div>
          <div className="app-bts-sliderRow">
            ...
            <span className="app-gold app-bts-desire">
              期望 {sliderToDesire(sliderV).toLocaleString("zh-CN")}m
            </span>
          </div>
