    ...
    for (const fx of arrivals) {
      ...
      if (fx.droneDown) {
        ...
        if (model) {
          ...
          const layDown = layout(dims, foeSizesFor(rowFxTags), visDistRef.current, openM, nearM, meSize, foeRaisesFor(rowFxTags))
      ...
      if (dm) {
        ...
        if (DRONE_STYLE === 'sortie' && !dm.resident) {
          ...
          const st = !prev || now - prev.startAt >= cycleMs + 40 ? { startAt: now, offs: droneRandomOffsets(DRONE_SHOW_MAX, droneOutward) } : prev
