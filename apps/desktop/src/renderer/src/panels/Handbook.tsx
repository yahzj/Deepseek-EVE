...
const GUIDE_ROWS: Array<[string, string]> = [
  ...
  ['精炼', '工业页把矿石炼成矿物（产出倍率由精炼学与高级回收处理提升：基础 100%，最高 160%）；矿物是制造原料，也可卖出。精炼炉可多单位并行：主控亲自运转限 1 台（占主控工作位），每枚 AI 核心各驱动一台——同时启用的 AI 核心总数受 AI 核心上限技能约束（与 AI 副船任务等共用）；原料不锁定，每批到点从货仓+仓库实时扣取。'],
  ...
  ['打捞残骸', '驾驶船高槽装打捞器（无伤害件，升级只减周期）后，在星图「残骸打捞」选星系开捞：单趟作业、满仓自动返航卸货；低安星系打捞作业中可能遇袭（航行与返航途中不会），规则见「航行须知」。'],
  ...
  ['装配', '装备库里的模块可装到高/中/低槽位（布局见船卡；装配受 CPU 约束），卸下自动退回装备库。抗性按 EVE 式乘入合成（上限 90%）；伤害/射速/容量可全额叠加，命中/闪避/抗性/速度同类多装收益递减；「动力」影响弃船避险与跃迁充能。无人机由战斗自动放飞（无需装船）：上限 = 无人机舱（可装「甲板扩展」扩容）+ 余 CPU，战术导控阵列增伤，不消耗、不被击落。'],
  ...
  ['副空间站', '建成的副站并入协会基地网络：市场买卖、精炼与残骸回收、组装机制造、维修补给、换驾驶卸货全部可用——设施与仓库和母港共享（共用同一市场与仓库）；采矿/打捞自动返航、悬赏与扫描的胜利返航都会停到最近已建成的站；手动召回仍回母港。未建成的工地不提供停靠与任何站内功能：人在现场可提交建材，或停靠空间站后一键「前往工地交付」——真实航程前往、到点自动交付并自动返航，途中可随时取消。'],
  ['AI 副船', '练「人工智能专家」+ 买基础 AI 核心，可给闲置舰船指派自动采矿/打捞/掩护巡逻任务——核心效率越高越快；同时启用的 AI 核心总数受 AI 核心上限技能约束（Lv0 无法启用），AI 副船与站内精炼炉/回收炉/制造线共用该上限。'],
  ...
  ['耐久与维修', '战斗会磨损「结构」（=耐久）与装甲，损伤跨场保留，结构归零弃船（货随船失）；护盾损失不保留（战斗中被被动回充、脱战回满）。恢复：空间站付费维修（结构+装甲一并修复），或货仓带「修理组件」野外应急（民用/军用两档，市场可买、工业页可自制；连续出击会自动消耗）。'],
]
...
const RULES_CORE: Array<[string, string]> = [
  ...
  ['连续出击（巡回讨伐）', '空闲时可开：胜利后自动返航回港（去程并入返航），冷却结束自动再出发，往复巡回；货仓装不下缴获 / 耐久低于 50% 且修理组件耗尽 / 战败都会自动暂停。'],
