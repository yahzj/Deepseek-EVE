...
const GUIDE_ROWS: Array<[string, string]> = [
  ...
  ['精炼', '工业页把矿石炼成矿物（产出倍率由精炼学与高级回收处理提升：基础 100%，最高 160%）；矿物是制造原料，也可卖出。精炼炉可多单位并行：主控亲自运转限 1 台（占主控工作位），每枚闲置 AI 核心各驱动一台；原料不锁定，每批到点从货仓+仓库实时扣取。'],
  ...
  ['制造', '市场买蓝图书 → 蓝图书架「学习」后永久可造 → 工业页开工（制造免费，只耗材料与时间，到点自动入库/入坞）。劳动者与精炼炉同一套：主控亲自开线（全局限 1 条、占主控不可离港）或一枚 AI 核心驱动一条线（核心库存即并行上限）。'],
  ['装配', '装备库里的模块可装到高/中/低槽位（布局见船卡；装配受 CPU 约束），卸下自动退回装备库。抗性按 EVE 式乘入合成（上限 90%）；伤害/射速/容量可全额叠加，命中/闪避/抗性/速度同类多装收益递减；「动力」影响弃船避险与跃迁充能。'],
  ...
  ['副空间站', '建成的副站并入协会基地网络：市场买卖、精炼与残骸回收、组装机制造、维修补给、换驾驶卸货全部可用——设施与仓库和母港共享（共用同一市场与仓库）；采矿/打捞自动返航、悬赏与扫描的胜利返航都会停到最近已建成的站；手动召回仍回母港。'],
  ['AI 副船', '练「人工智能专家」+ 买基础 AI 核心，可给闲置舰船指派自动采矿/打捞/掩护巡逻任务（核心效率越高越快）。'],
...
const RULES_CORE: Array<[string, string]> = [
  ...
  ['货仓与出售', '装卸与出售只对当前驾驶船；出售需停靠空间站（母港或已建成副站——共用同一市场与仓库）；副站可卸货入仓库。'],
