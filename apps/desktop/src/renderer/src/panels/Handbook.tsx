...
const GUIDE_GROUPS: GuideGroup[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      ['装配', '装备库里的模块装到高 / 中 / 低槽位（布局见船卡；受 CPU 约束），卸下自动退回装备库。抗性按乘入式合成（上限 90%）；伤害 / 射速 / 容量可全额叠加，命中 / 闪避 / 抗性 / 速度同类多装收益递减；「动力」影响弃船避险与跃迁充能。无人机在装配页「无人机舱」装入清单（舱容 = 无人机舱 + 甲板扩展，与装配共用 CPU），战斗只放飞已装入的；卸下甲板扩展使机舱变小时，超出的无人机会自动退回仓库；战术导控阵列增伤。敌方近防炮会击落机群——战斗内机群可能全部损坏，战后按回收率找回一部分（基础 10%，「无人机回收学」满级 50%），未回收的才自清单永久损失；侦察机靠闪避、攻坚机靠厚甲、哨戒机默认被近防炮放过（其余机型全被打光后就轮到它）。'],
