...
const GUIDE_GROUPS: GuideGroup[] = [
  ...
  },
  {
    title: '残骸与回收',
    rows: [
      ['打捞残骸', '驾驶船高槽装打捞器（无伤害件，升级只减周期）后，在星图「残骸打捞」选星系开捞：自动循环作业，满舱自动返航卸货后自动续捞（勾「本次返航卸货后停止」可做单趟）；低安星系打捞作业中可能遇袭（打捞出航 / 返航的移动段不会），规则见「航行须知」。'],
...
const RULE_SECTS: RuleSect[] = [
  {
    ...
    rows: [
      ...
      ['什么会遇袭', '低安只对「停留与就地作业」动手：矿带采掘中、打捞作业中、扫描期间、掩护巡逻驻留都可能撞见巡逻拦截或海盗伏击；**长途运输途中同样会遇袭**（跑运输的船按"停在出发星系"算）。除此之外的航行与返航途中不会遇袭。AI 副船同样适用。'],
