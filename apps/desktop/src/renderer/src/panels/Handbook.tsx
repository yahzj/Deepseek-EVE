...
const GUIDE_GROUPS: GuideGroup[] = [
  ...
  },
  {
    ...
    rows: [
      ...
      ['装配', '装备库里的模块装到高 / 中 / 低槽位（布局见船卡；受 CPU 约束），卸下自动退回装备库。抗性按乘入式合成（0 基础船 +N%，25% 基础船面板 ≈ N×0.75 + 25%，上限 90%）；伤害 / 射速 / 容量可全额叠加（线性求和，上限只看 CPU），命中 / 闪避 / 抗性 / 速度同类多装收益递减（曲线档第 2 件 ≈87%、第 3 件 ≈57%；缺口档按 1−(1−a)(1−b) 再削剩余缺口）。「动力」影响弃船避险与跃迁充能。中槽「矢量推进器」是周期点火：开场即点火、持续 60 秒，随后冷却 60 秒——点火期间战斗机动显著提升、开火命中略降（失稳代价同样只在点火期生效），冷却期间两者都回到基础值；战斗界面底部显示推进器倒计时。无人机在装配页「无人机舱」装入清单（舱容 = 无人机舱 + 甲板扩展，与装配共用 CPU），战斗只放飞已装入的；卸下甲板扩展使机舱变小时，超出的无人机会自动退回仓库；战术导控阵列增伤。敌方近防炮会击落机群——战斗内机群可能全部损坏，战后按回收率找回一部分（基础 20%，「无人机回收学」满级 50%），**回收名额优先给价值更高的机型**；未回收的才自清单永久损失，战报里会列出这次回收了哪些、净损失哪些；侦察机靠闪避、攻坚机靠厚甲、哨戒机默认被近防炮放过（其余机型全被打光后就轮到它）。'],
  ...
  },
  {
    ...
    rows: [
      ...
      ['回收残骸', '打捞的残骸在工业页开箱拆解：回收卡按敌群列出这批残骸的「保底矿物」，随 星系危险度 × 敌群威胁 上浮；另有概率彩头——同卡的「特色掉落」写出的该敌群主题增幅件（含关底穹顶守卫直出 MK3 武器）与高威胁蓝图碎片（MK3 装备只经碎片解锁）。星图「残骸打捞」页会先标出该星系的这些内容。'],
    ],
  },
  {
    title: '副船与扩张',
