...
const GUIDE_GROUPS: GuideGroup[] = [
  {
    title: '生产循环',
    rows: [
      ['采矿', '出港页「矿带开采」选带出击：原矿进驾驶船货仓，到港自动整仓卸入物品仓库。练「采矿技术 / 采矿护卫舰操作」提产量、缩循环；「自动循环」与 AI 副船让矿机不停转。'],
      ['装卸', '任何舰船进港（停靠母港或已建成副站）都会自动整仓卸入物品仓库（无限容量、不随船）；货仓页也可手动卸货——空闲停靠的非驾驶船同样可以。出售仍只对当前驾驶船开放。'],
      ['精炼', '工业页把原矿炼成原材料（「产出倍率」由精炼学与高级回收处理提升：基础 120%、技能最高 165%——无技能时炼矿净收益约为原料价值两成）；原材料是制造原料，也可卖出。炉位：主控亲自运转限 1 台（占主控工作位），每枚 AI 核心各驱动一台——AI 核心启用数受上限约束，细则见「AI 副船」；原料不锁定，每批到点从货仓 + 仓库实时扣取。'],
      ['制造与蓝图', '市场可购到所有装备与舰船（协会保底艇、异星原型科技除外）的蓝图书：购图到「蓝图书架」学习一次即永久可造，再到工业页组装机按图备料开工，到点自动入库（装备进装备库、舰船进舰船仓库）。图纸价格随产物档位升高、在市场上出现得也稀罕；赶时间可以继续直接买成品现货，两条渠道并存。产线由主控（全局限 1 条、占主控不可离港）或一枚 AI 核心驱动（占用 AI 核心上限，见「AI 副船」）。'],
    ],
  },
  {
    title: '战斗与生存',
    rows: [
      ...
      ['装配', '装备库里的模块装到高 / 中 / 低槽位（布局见船卡；受 CPU 约束），卸下自动退回装备库。抗性按递减方式合成（同一层装得越多越接近上限 90%，基础抗高的船提升越小）；伤害 / 射速 / 容量可以多装全量叠加（只受 CPU 限制），命中 / 闪避 / 抗性 / 速度同类多装收益递减（第 2 件约剩八成七、第 3 件约剩五成七）。「动力」影响弃船避险与跃迁充能。装配 CPU 预算是唯一硬约束（全部位合计 ≤ 船体 CPU；无人机放飞共用同一份预算）——低槽「协处理器」可为预算扩容（本件自身不占 CPU，卸下即收回：卸到超载会被拒绝，避免"装扩容件塞满再卸掉"套利）。中槽「矢量推进器」是周期点火：开场即点火、持续 60 秒，随后冷却 60 秒——点火期间战斗机动显著提升、开火命中略降（失稳代价同样只在点火期生效），冷却期间两者都回到基础值；战斗界面底部显示推进器倒计时。无人机在装配页「无人机舱」装入清单（舱容 = 无人机舱 + 甲板扩展，与装配共用 CPU），战斗只放飞已装入的；卸下甲板扩展使机舱变小时，超出的无人机会自动退回仓库；战术导控阵列增伤。敌方近防炮会击落机群——战斗内机群可能全部损坏，战后按回收率找回一部分（基础 20%，「无人机回收学」满级 50%），回收名额优先给价值更高的机型；未回收的才自清单永久损失，战报里会列出这次回收了哪些、净损失哪些；侦察机靠闪避、攻坚机靠厚甲、哨戒机默认被近防炮放过（其余机型全被打光后就轮到它）。'],
  ...
  },
  {
    ...
    rows: [
      ...
      ['回收残骸', '打捞的残骸在工业页开箱拆解：回收卡按敌群列出这批残骸的「保底原材料」，随 星系危险度 × 敌群威胁 上浮；另有概率特色掉落——同卡的「特色掉落」栏写出的该敌群增幅装备（含关底穹顶守卫直出 MK3 武器）与高威胁蓝图碎片（MK3 装备只经碎片解锁）。星图「残骸打捞」页会先标出该星系的这些内容。'],
    ],
  },
  {
    title: '副船与扩张',
  ...
  },
  {
    title: '市场与世界',
    rows: [
      ['交易', '市场页：常驻供应 / 稀有订单 / 限定奇货三个标签，挂单与市价买卖；卖出成交收贸易税（练贸易技能减免）。市场全程挂单簿撮合，收购价低于供应价；集中买卖会带动价格短时偏离（冲击动量），原矿 / 原材料另受库存池调节。'],
  ...
  },
  {
    title: '虫洞探索',
    rows: [
      ['发现虫洞', '在星图「扫描虫洞」选项卡开始扫描（主控活动，深空工业协会声望 40 解锁，解锁时扫描进度直接给满格）：扫描窗口基准 12 小时，受「信号分析学 / 星图测绘学 / 信号过滤学」与「星际奇遇学」（满级 −20%）缩短；进度满即发现一处虫洞存入库存，最多同时囤 5 处；每处锁定一个敌族、从第 1 层开始探索，进入即消耗该处——也可以派副船自动探索（约 5 分钟，收益约为亲自探索的四成，参与舰受损但不丢船），或直接放弃。'],
      ['编队与质量', '最多带 4 艘船，旗舰过重进不去（会压塌虫洞入口）：带入舰船按级别折算质量（护卫舰 500 / 驱逐舰 1,500 / 巡洋舰 3,500 / 战列舰 7,000），编队总质量越高、可探索的回合数越少——4 艘护卫舰 56 回合，4 艘巡洋舰 32 回合。'],
      ['搜打撤', '洞内按节点推进（舰船信号 / 墓场与遗迹 / 矿脉 / 谜质 / 漂浮信标）：每层要走到下潜点并打赢层末守卫，才能继续深入；撤离自第 2 层起要打一场撤离战，交火中不能撤；船被打沉，该船与它带回的货一起丢。'],
      ['背包与产出', '背包格数按编队合计货仓折算（每 500 m³ = 1 格），打捞到的安全货柜、图纸货柜各占固定格数；洞内只产原矿「虚空母矿」，回港在精炼炉精炼出「虚空晶」；每深入一层收益 ×1.2、威胁 ×1.16——收益涨得比威胁快。'],
    ],
  },
]
...
const RULE_SECTS: RuleSect[] = [
  {
    ...
    rows: [
      ['安全等级', '星图星系标色：越高越安全。安全等级 ≥ 0.5 为高安，0 至 0.5 之间为中安，≤ 0 为低安——只有低安会遇到巡逻拦截与海盗伏击，安全等级越低越危险、越频繁。'],
      ...
      ['结局三档', '击退：缴获少量信用点；受损：敌方火力越强挨得越狠——先扣装甲、装甲吃穿才伤结构（底线 5%，绝不弃船）；被抢：至多 30% 船上货物（无货则抢少量钱包）。'],
      ['受损后先修再判', '遭遇了结后先自动修补：装甲或结构低于 50% 时，就地用修理组件（货舱优先、仓库兜底）补到两者约 60%，或组件用尽；组件不足、或补完后结构仍低于 50%，才收手返港待命——驾驶船停下采掘 / 打捞 / 扫描并即时返航最近已建成站，副船中止任务召回（副船结构低于 50% 需修好才能再派）。'],
      ['迎战也有保险', '低安遭遇点「⚔ 迎战」时，本场结构损失过半即自动脱离（轻损退出，不给缴获也不额外扣损），随后同样按上面的撤退规则返港——不会把船打到弃船。'],
    ],
  },
  {
    title: '重要规则留档',
  ...
  },
  {
    title: '虫洞探索',
    rows: [
      ['进洞与离开', '进洞占主控一个活动：探索期间采矿 / 扫描 / 打捞 / 远征 / 长途运输都开不了，进洞时会自动停掉正在进行的作业；关掉虫洞界面＝临时离开——主控立刻释放、洞内进度原样保存并冻结，回来时要求主控空闲。'],
      ['回合与全损', '到达一个地点耗 1 回合，同一地点每多打一波、每多捡一堆各 +1 回合；回合耗尽只能撤离。船被打沉：该船与它带回的货一起丢失；整队失联＝本趟没有产出。'],
      ['临时空间', '货仓右侧的备用格区（4×8 = 32 格），用来腾位置或暂存待丢的东西：里面有东西时不能进行其他操作，离开背包页前必须放回货仓或丢掉。'],
      ['谜质储存器', '洞内采到的谜质储存器在货仓里占 2×2 格，本趟虫洞内生效、离开即消失；拖进临时空间就会失效。'],
    ],
  },
  {
    title: '目标档案',
    rows: [
      ['寻找人类', '人类已全体失踪——你是一艘前人类时代的舰船 AI。目前没有任何可执行线索，完成方法未知；以这座章鱼宇宙人统治的母港为起点，往未知的前方继续航行，壮大自身规模，应对各种危险，或许终会有所发现。'],
    ],
  },
]
