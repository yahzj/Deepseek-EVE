 ...
 *
 * ⚠ **2026-09-22 起只剩一个消费方**：`ui/ActivityScreen.tsx`（主控活动窗口）。战斗屏按船长令
 * 「界面回滚，战斗界面回滚到全屏显示」回到自己的全屏覆盖层（`.app-battle-screen`）⇒ **不再套本壳**
 * （那一版封存在分支 `archive/window-embed-20260922`，见 `docs/design/archived-window-embed-20260922.md`）。
 * 壳仍然留着：活动窗口就是照它做的，且日后要再窗口化战斗时它还在。
 *
 ...
 * - `open=false` 时本壳返回 `null`（不上屏），但**组件本身可能仍挂载** ⇒ 别把"挂载"与"上屏"当成一回事。
 *   （战斗屏那套"挂载 ≠ 上屏"如今由它自己的 `visible` 入参管，理由同款：慢镜与战报要在结算之后才渲染。）
 *
 * z 序：窗口 110 已无意义（不再有浮层）；详情窗（120/140）与悬浮说明（1000）仍在本块之上。
 *
 * 谁该用它：**"看一个持续过程"的界面**（主控活动）。一次性确认框继续用 `.app-modal` 原族，不要套本壳。
 */
