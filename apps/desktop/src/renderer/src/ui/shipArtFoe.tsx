...
/** 单边：`k` 为向 `side` 一侧鼓出的上限（0 = 直线）；控制点落在**原顶点**上 ⇒ 曲线自然向里收，转角自动圆化。
 *  ⚠ 外移量按**边长自适应**（`min(k, 边长×0.35)`）：小附肢不该鼓成大肚皮，否则读成"折纸"。 */
function edge(a: Pt, b: Pt, k: number, side: number): string {
  if (!k) return line(a, b)
  const dx = b[0] - a[0]
  const dy = b[1] - a[1]
  const len = Math.hypot(dx, dy) || 1
  const nx = -dy / len
  const ny = dx / len
  const kk = Math.min(k, len * 0.35)
  const mx = (a[0] + b[0]) / 2 + nx * kk * side
  const my = (a[1] + b[1]) / 2 + ny * kk * side
  return `Q${mx} ${my} ${b[0]} ${b[1]}`
}

/**
 * **有机圆角轮廓**（C 族专用）：点列按序首尾相接成**一条闭合曲线**——
 * `k` 为各边向"外侧"鼓出的量（`k>0` 时整条边变弧，转角随之自然圆化）。
 *
 * 实现口径（踩过的坑）：曲线**控制点落在原顶点上**（`Q 中点的外移点 → 下个顶点`）。
 * 早先的写法是把每条边两端各内缩 `r`、再在中段画线/弧——**相邻两边之间会漏出 `2r` 的缺口**，
 * 闭合轮廓在 `fill:none` 下就裂成一圈碎线段（2026-09-26 船长「C 组敌人没有封闭外形」正是此症）。
 * ⇒ **轮廓线一律不许留缝**：要圆角就靠这条 Q 曲线，不许靠"内缩 + 补线"。
 *
 * 为什么 C 族单独要它：异形族是**生物体**，直角多边形会读成机械舰；
 * ⚠ 突出口外的部件（獠牙/节肢/骨刺）**一律用闭合楔形**（`poly`/`blob`），不许用开放折线——
 * 开放折线在 `fill:none` 的渲染下就是"没封闭的线"。
 */
function blob(pts: Pt[], k = 4): string {
  const n = pts.length
  let cx = 0
  let cy = 0
  ...
  cy /= n
  const first = pts[0]!
  const out: string[] = [`M${first[0]} ${first[1]}`]
  for (let i = 0; i < n; i++) {
    const a = pts[i]!
    const b = pts[(i + 1) % n]!
    const mx = (a[0] + b[0]) / 2
    const my = (a[1] + b[1]) / 2
    const side = (mx - cx) * (b[1] - a[1]) - (my - cy) * (b[0] - a[0]) >= 0 ? 1 : -1
    out.push(edge(a, b, k, side))
  }
  return `${out.join(' ')} Z`
}
...
// ───────────────────────── C 族 · 异形（5 条）─────────────────────────
// 语言：有机曲线（圆角闭合轮廓 `blob()` + `pod()` 胶囊）、节肢、膜翼、孢子囊；**无舷窗、无喷口**
// （引擎表为空 → 渲染层不画尾焰）。
// ⚠ 两条硬口径（2026-09-26 船长「C 组敌人没有封闭外形」后定）：
//   ① **船体 = 一条闭合轮廓**，内部结构一律收在体内，不许拿开放折线当骨架；
//   ② 突出口外的部件（獠牙/节肢/骨刺/触须）**用闭合楔形 `poly`/`blob`**，不许用开放折线。

/** ⑧ 畸变幼虫（T1 / brawl）：最小有机水滴 —— **体前一根弯獠牙 + 两侧膜翼**，圆钝无节 */
const C_RIFT_LARVA = (
  <>
    <path d={blob([[196, 62], [172, 46], [132, 40], [86, 42], [56, 50], [30, 34], [44, 62], [30, 90], [56, 74], [86, 82], [132, 84], [172, 78]], 5)} />
    <path className="shipart-panel" d={s(`M92 44 Q100 62 92 80`, `M126 41 Q136 62 126 82`, `M158 46 Q166 62 158 78`)} />
    <path className="shipart-panel" d={s(`M66 54 Q92 60 118 62`, `M66 70 Q92 64 118 62`)} />
    <path className="shipart-acc" d={poly([198, 56], [208, 40], [212, 45], [204, 58])} />
    <path className="shipart-accf" d={pod(200, 58, 4, 4)} />
  </>
)
...
const C_SC_LARVA = (
  <>
    <path d={blob([[188, 62], [166, 46], [122, 40], [74, 48], [56, 54], [32, 44], [46, 62], [32, 80], [56, 70], [74, 78], [122, 84], [166, 78]], 6)} />
    <path className="shipart-panel" d={s(`M110 41 Q118 62 110 83`, `M146 43 Q154 62 146 81`)} />
    <path className="shipart-accf" d={s(pod(112, 44, 12, 8), pod(138, 42, 12, 8), pod(162, 48, 10, 7))} />
    <path className="shipart-acc" d={blob([[188, 56], [204, 46], [210, 52], [192, 62]], 2)} />
  </>
)

/** ⑩ 星髓成虫（T2 / brawl）：**三节分节甲壳（节缝收在体内）+ 背脊结晶列 + 三对节肢桨**，有节（区别于幼虫的无节） */
const C_SC_ADULT = (
  <>
    <path d={blob([[190, 62], [170, 44], [140, 40], [116, 38], [78, 46], [60, 56], [36, 44], [44, 62], [36, 80], [60, 68], [78, 88], [116, 92], [140, 90], [170, 80]], 4)} />
    <path className="shipart-panel" d={s(line([142, 41], [142, 89]), line([116, 39], [116, 91]))} />
    <path className="shipart-panel" d={s(`M84 48 Q92 62 84 86`, `M158 46 Q166 62 158, 78`)} />
    <path className="shipart-accf" d={s(pod(94, 46, 10, 7), pod(122, 42, 10, 7), pod(150, 46, 10, 7))} />

    <path className="shipart-acc" d={blob([[190, 56], [206, 46], [212, 52], [194, 62]], 2)} />
  </>
)

/** ⑪ 孢群异虫（T3 / brawl）：**前部巨大瓣状开口口器（两瓣，闭合楔形）+ 背部孢子囊 + 腹部育囊**，体型最圆鼓 */
const C_SPORE_HIVE = (
  <>
    <path d={blob([[188, 62], [166, 50], [140, 44], [98, 40], [62, 50], [46, 62], [62, 74], [98, 84], [140, 86], [166, 78]], 6)} />
    <path d={poly([60, 52], [34, 54], [32, 62], [34, 70], [60, 72])} />
    <path className="shipart-panel" d={s(line([98, 41], [98, 83]), line([140, 45], [140, 85]))} />
    <path className="shipart-accf" d={s(pod(114, 42, 13, 9), pod(140, 40, 13, 9), pod(114, 84, 13, 9), pod(140, 86, 13, 9))} />
    <path className="shipart-panel" d={s(`M68 52 Q76 62 68 74`, `M166 52 Q174 62 166 74`)} />
  </>
)

/** ⑫ 噬口巨兽（T4 / brawl）：**全库最大** —— 一条闭合巨兽轮廓，口部开缝露齿、背部骨刺、尾部两根闭合螯肢 */
const C_MAW = (
  <>
    <path d={blob([[208, 56], [212, 62], [208, 68], [186, 78], [140, 84], [96, 82], [62, 74], [34, 88], [50, 64], [34, 36], [62, 50], [96, 42], [118, 28], [140, 40], [162, 26], [186, 46]], 5)} />
    <path d={s(line([96, 42], [156, 50], [96, 82]), line([156, 50], [96, 62]))} />
    <path className="shipart-panel" d={s(line([140, 41], [140, 83]), line([172, 44], [172, 80]))} />
    <path className="shipart-accf" d={pod(182, 56, 6, 5)} />
    <path className="shipart-accf" d={s(pod(120, 41, 10, 6), pod(146, 40, 10, 6))} />
  </>
)
