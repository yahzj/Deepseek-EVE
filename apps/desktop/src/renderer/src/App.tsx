...
async function applyLayoutAndQuit(): Promise<void> {
   ...
   *   战斗中点过「← 最小化」（原「← 退出战场」）的玩家整场战报都收不到）。挂载条件必须**先于**结算成立、
   *   且只由子组件的收尾回调撤销。
   * - **上屏** `battleOpen`：玩家自己的开关 —— 开战自动开一次、点「← 最小化」关上（战斗照常在后台
  ...
  return (
    <div ref={rootRef} className={`app-root${mobileRot ? ' is-mobile-rot' : ''} is-layout-${layoutKind}`}>
       ...
       * - 上屏由 `visible` 管：玩家点「← 最小化」真的关屏（战斗后台照打），浮动入口随时点回来。
      ...
      {battleMounted ? (
        <BattleScreen
          ...
          onClose={() => {
            // 2026-09-10 修复（船长定位）：最小化战场 = 仅关闭观看界面——战斗后台照常推进、
