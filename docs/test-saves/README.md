...
- 建议测试路径：舰船页换驾驶到演示船 → 装配页看高/中/低三组位 + 合成预览（回避含陀螺
  缺口 11%→24%、命中含索敌、机动速度含加力）；低槽再装第 3 件动能稳定器应**全额叠加**
  （+30%）；中槽再装第 2 件索敌/陀螺应显示「第 N 件衰减」提示（EVE 曲线 ≈87% / 缺口只削
  剩余）；同类抗性/推进/容量件多装均合法（V18.1 取消同类唯一）；去悬赏开战看 ×2 齐射
  合并条目与动能/高爆弹型分开装填。
- V18A 时代旧档（若船队保留）：可照常加载——唯一约束取消不影响已装单件。