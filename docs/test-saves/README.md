...
- 注入门槛：钱包 +5000 万 ISK；**新增满槽演示船「复数装配演示」**（虎鲨级 sh-tigershark：
  高槽 2×动能 MK2 + 1×高爆 MK2、中槽 盾扩 MK2 + 推进 MK2、低槽 甲板 MK2 + 货舱 MK2，
  CPU 144/165，货仓预置蜂鸟 ×20 + 赤鸢 ×10）；装备库补矿枪 MK2 ×2、动能炮 MK2 ×1、
  无人机挂架/导控 MK2 各 1；仓库三型通用弹 ×500 各；全舰耐久回满。
- 建议测试路径：舰船页把驾驶换成「复数装配演示」→ 装配页看高/中/低三组位 + CPU 余量；
  再装第 2 块盾扩应被「同类唯一」拒绝；高槽空位装第 3 门动能炮看 ×3；换装矿枪 ×2 试复数
  采矿；装无人机挂架/导控后去悬赏开战——看战斗多炮 ×N 齐射合并条目与动能/高爆弹型分开
  装填（异弹型炮无弹时停火、同型主炮持续）。