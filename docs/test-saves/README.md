...
### V18 槽位制 + V18.1 支援件 + V18B-1 导弹架（v18）
- 文件：`test-save-v18-*.json`（取最新一份）
- 注入门槛：钱包 +5000 万 ISK；**新增满槽演示船「复数装配演示」**（虎鲨级 sh-tigershark：
  高槽 2×动能 MK2 + 1×导弹架 MK2（留 1 位可加第 3 门 → ×3 齐射）、中槽 索敌阵列 MK2 +
  姿态陀螺 MK2、低槽 动能稳定器 MK2 ×2，CPU 144/165，货仓预置蜂鸟 ×20 + 赤鸢 ×10）；
  装备库补矿枪 MK2 ×2、动能炮 MK2 ×1、稳定器/射速/索敌/陀螺 MK2 各 1、无人机挂架 MK2 ×1；
  仓库三型通用弹 ×500 各；全舰耐久回满。
- 建议测试路径：舰船页换驾驶到演示船 → 装配页看高/中/低三组位 + 合成预览（回避含陀螺
  缺口、命中含索敌、机动速度含加力）；导弹架与动能炮混装开战——爆破导弹无视近盲、
  命中不随距离衰减（与动能炮近距离衰减/近盲对照），exp 弹与动能弹分开装填；低槽再装
  第 3 件动能稳定器应**全额叠加**；中槽再装第 2 件索敌/陀螺应显示「第 N 件衰减」。
- V18B-1 迁移：旧「高爆炮台」已退役，装载/库存件自动 1:1 迁移为同档导弹架（含市场
  目录同步）——旧档（如 test-save-v18 早期版本）加载即迁，无资产丢失。