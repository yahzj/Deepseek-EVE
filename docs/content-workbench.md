...
> ⚠ **表头里的中文说明词已按现行术语统一（2026-09-15 船长「按照新名词改名」）**：
> `kind(ore**原矿**/mineral**原材料**/…/container货柜/matter谜质储存器/aicoreAI 核心)`、
> `空间站收购价**信用点**`、`空间站售价**信用点**`、`奖励**信用点**`（`ISK` 退场）。
> 物品类别那一串**由引擎单点 `ITEM_KIND_LABELS` 派生**（加类别自动跟随，不再手抄）。
> 手上若还有改名前的文件：**导入端认得旧表头**（`content-schema.ts` 的 `HEAD_ALIASES` 会先归一化并提示），
> 但建议重跑一次 `npm run content:export` 取干净文件再编辑 —— 且**任何改名之后都要重导**（工作台不会自动跟）。
