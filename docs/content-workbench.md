...
## 五、Phase B：content:import（回写，实现中）

`npm run content:import skills path\to\skills.csv` → 按 id 字段级回写源 TS（保留注释/分组/
排版最小 diff），逐列类型/枚举/引用/范围校验，收尾自动 `content:check` + typecheck +
打印 `git diff` 摘要（改了几行/哪些字段/哪些 id）。交付后本文件同步更新操作细节。
