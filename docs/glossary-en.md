...
9. **括号限用**（**2026-09-26 船长令**：「**除非非常有必要，否则不要用括号进行额外说明。需要说明时和我沟通。**」）：
   **能不放就不放**；确有必要时**先与船长沟通**再写。规格型数量词（`30 s` · `500 m` · `50 m³`）与档位后缀（`T1` · `MK2`）照留。
   触发实例：挂载件效果句尾原先各带一个装置名括号，英文侧同样要从"名字 ＋ 规格"压成纯规格。
