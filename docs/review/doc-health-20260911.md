...
## 5. 船长裁决（2026-09-11）与落地

| # | 事项 | 船长裁决 | 落地 |
|---|---|---|---|
| 1 | §3 归档候选（7 公告稿 + 3 旧交接件 + 6 早期稿 + `refine-balance-50`） | **甲 · 原地保留，只留状态标注** | ✅ **文件一律不动**；改为逐件加「2026-09-11 状态标注」（22 件：7 份公告稿「使命已完成」、4 份 handover/d2/d3/verify 的工位与结案标注、3 份旧 handoff 的取代标注、`handoff-20260909-dsh-reinstall`「⚠ 仍有效」、`refine-balance-50`「历史锚」、`captain-skill-audit` 稿头、`hull-repair`/`g-exile`/`drone-bay`/`drone-roles`/`industry-copy-trim` 的「已实现」标注） |
| 2 | §2 那 33 份早期稿漂移 | **甲 · 只修高危项 + 挂横幅**（本轮已完成） | ✅ 高危 13 处已逐条修正（见 §1 第二批），48 份早期稿已挂「立稿留档」横幅；**其余漂移项继续留档在 §2**，日后顺手清 |
| 3 | `docs/test-saves/` 冗余 | **甲 · 只删那一对 MD5 完全相同的备份** | ✅ 删除 `user-backup-20260910-233013.json`（与 `-233010` 字节完全相同，MD5 `BDA8EE38…`）；目录 41 → **40** 个文件；README 补了去重说明。**同 case 旧版（drone/redtide）与其余真档备份一律保留** |
| 4 | `docs/roadmap.md` 是否切分 | （裁决同上：不切） | ✅ 未切；只在文件头补了「两套时间序」读法注 |
| 5 | 顺带发现的真问题 | **要处理：点防 PD 六参数补定稿 + 早期挂账并入待决策清单**（富凡晶石价差不处理） | ✅ 点防六参数收口于 `docs/design/drone-losses-20260910.md` **§6.1**（含逐项与代码核对；四处失效引用已改指）；✅ `handoff-20260909-dsh-reinstall.md` 三项挂账并入 `docs/review/pending-decisions-20260910.md` **§十二**（含 roadmap **O6** 公告未发、巡洋 T3 数值终审、渠道工船例外口径） |

**仍未处理（登记在案）**：`ore-veldspar` 收价 `items.ts` **13** vs `marketCatalog.ts` **12** 的不一致（船长本轮未选，属数据侧、需定谁为准）。
