...
## 追补：AI 徽标的悬停显示"AI 正在干哪些活动"（**船长 2026-09-13 定 · 已落码**）

> 船长原话（照抄）：「**活动界面AI图标的鼠标悬浮提示改为显示AI正在干哪些活动。**」

- **改前**：两枚 AI 徽标（粉色「副船 ×N」/ 薄荷「工业 ×N」）的悬停只有一句写死的
  「正在执行采矿 / 打捞 / 掩护巡逻」/「精炼炉 / 回收炉 / 制造线」——玩家看不到**具体在干什么**。
- **改法**：core `ActivityView` 新增 **`aiGroup?: 'ship' | 'industry'`**（只对 `kind === 'ai'` 的条目有值：
  AI 副船任务 = `ship`、站内 AI 精炼/回收炉与制造线 = `industry`；主控亲自干的制造**不是** AI 活动），
  UI 直接按它分组，**不用去猜 id 前缀**。悬停文案改成一条一行：
  「· 谁 · 干什么——在哪 / 什么阶段（还剩多久或百分比）」，最多 8 条、多出的折成「…另有 N 条」，
  末尾照旧给 AI 槽位说明与「点击前往…」。
- **只动悬停文案**：徽标计数、点击去处、渲染结构一律未改；`title` 仍走全站自绘提示
  （限宽 300px + 多行 + 悬停 200ms，见 §6 那条改判），所以 `\n` 会按行渲染。
- **验证**：typecheck 四包 0 错 · core **122 文件 / 1268 用例**全绿（`t1.test.ts` 新增两条断言：
  站内 AI 作业 `aiGroup === 'industry'`、副船任务 `aiGroup === 'ship'` 且 label 含「采矿」）·
  `content:check` ✅ · 桌面 build ✅；**负向验证真红**（把副船那处标成 `industry` ⇒ 该用例真红，还原后 7/7）。
