# 成就系统（**两批均已完成**）— 设计稿（2026-09-20）

> **状态：两批都已完成并合入 main**（第一批 = 徽章框架；**第二批 = 里程碑成就 18 枚**，同日完成）。
...
- ~~**里程碑成就内容**（第二批）：另设一批非任务类成就（如首杀、深入层数、AI 核心累计等）——
  内容与判定另批定，第一批只把框架留好。~~ ⇒ ✅ **第二批已完成（2026-09-20）**：
  18 枚里程碑成就，判据 = 终身计数（`state.firstStats`），家族 = 深渊层深 4 · 层末守卫 2 ·
  稀有残骸额外战利品 5 · AI 核心 4 · 副空间站 2 · 谜质科技满树 1。
  阈值依据**实测**（4 船真档编队 12 趟平均到达 2.90 层 ⇒ 层深阶梯取 2/3/4/5）。
  落码 = `data/src/achievements.ts` 的 `milestoneAchievements()` ＋ `core` 侧
  `AchievementSource` 的 `milestone` 分支 ＋ 4 个新计数键（`rareBoxes` / `whMaxDepth` /
  `whBossClears` / `matterTechMaxed`，其中两个走新增的峰值型写入 `peakFirst`）。
- 不做成就点数 / 等级 / 排行 / 分享。
