...
- 状态：**落码完成 · 待验收**（2026-09-25 · 二号 · d2）——四闸门全绿（typecheck / test **2,349 例** / content:check / l10n:check / build + docs:index），**未推送**
...
| **入账日志**（新） | 夺回 / 全部夺回 / 旗舰击沉三处**里程碑**各留一条日志（新 id `core.weekend.001~003`，中英双语）——起因是这两笔钱原先**只入账不吭声**（`weekendResolveBattle` 的 `note` 有措辞，但两个调用点都把返回值丢了）⇒ 玩家看到钱包/货舱突然多出东西却没有解释。**普通进度推进不记**（面板有进度条与百分比，免得每场刷一条） |
...
### 九之三 · 两封通讯 ＋ 结算面板（同日 · 船长给文案并令「落码吧」）

**船长原话（照抄要点）**：「**每场都发，但是覆盖上一次的**」·「通讯的跳转**显示详细奖励**，**点击后弹出类似虫洞撤离的结算界面**，显示玩家在**各个被入侵星系的贡献**和**旗舰战的输出**，并列出玩家**获得的物品**（包括旗舰战的掉落物）」·「**通讯中不要使用这种备注的样式**」（指正文里的括号备注）·「用全称」（族名）·「你可以进行润色修改」。

| 件 | 落法 |
|---|---|
| 实例通讯（新机制） | `state.commsInstance`（随档可选字段 · 零迁移）：**固定 id**（`msg-weekend-warn` / `msg-weekend-settle`）＋ 整条内容 ⇒ 每场**整条替换**＝"覆盖上一次"；`commsInbox` 把它与表消息**合并渲染**，已读/未读计数/弹窗队列/送达记账全部复用既有那一套；覆盖时**重置已读并重新入弹窗队列**（＝又是一封新信）。投递口 `deliverCommsInstance`（返回是否覆盖了旧的） |
| 两封信 | `core/weekendComms.ts`：预警＝开局（落点处数 ＝ 核心＋外围 · 核心星系名 · **族名走 `pNId`** ⇒ 英文界面才有译名）；结算＝**贡献奖入账那一拍**（正文里的奖励清单＝实发）。文案进 `l10n/table.ts`：`core.weekend.010~013`（主题与正文）· `014`/`015`（有奖/零贡献两版正文）· `020~023`（族名全称：海盗/异形生物/鱿烬亡军/墨潮帮 · Pirate/Alien/Deadarmy/Ink Tide）。**奖励清单结构化随信**（`rewards: {itemId|isk, qty}[]`），界面按语言拼串 ⇒ 数据只有一份、中英各自成句、**引擎里不拼中文** |
| 幂等/自愈 | `weekendSyncComms(state, ctx, now)`：按**场次号**比（存档里那封的 `seq` ≠ 本场 ⇒ 投递/覆盖）⇒ 引擎每拍无脑调一次即可；离线跨过开局/结束点、老档首载都会自动补齐，同场绝不重发 |
| 结算面板（新） | `panels/WeekendSummary.tsx`：骨相全照虫洞撤离结算的 `SettleView`（`app-wh-settle-*` 同上、逐格淡入同节奏），补的只有"各星系贡献"那一列（星系名 · 贡献% · 已夺回/未夺回 · 该处奖励）与 `app-weekend-sum-*` 几条样式；入口 = 结算信跳转按钮（`action: 'weekendSummary'` ⇒ 弹 `.app-modal-*` 族的面板，不跳页） |
| 战果快照（新） | `state.weekendLastResult`（随档 · **每场覆盖**）：场次/族/核心/结束时刻/旗舰结局/占比与档位/**逐星系贡献与奖励**/旗舰输出与血池/**到手合计**（含旗舰掉落）+ 奖励物品 id。写在 `weekendSettleAndGrant` 里 ⇒ 面板与信件读的是**同一份**、且下一场开局清台账后仍留得住 |
| 到手台账（新） | `WeekendEventState.rewardLedger`：夺回（逐星系）· 贡献奖 · 旗舰掉落三处入账时累加 ⇒ **"说的与发的逐值一致"**（清单不另算一遍） |
| 英文与"不要备注样式" | 正文里的奖励**不写成括号备注**，而是清单里的一项；英文同构（`ui.comms.071~073` 出分隔符与信用点单位，物品名走游戏既有物品表） |
| 存档 | `save.ts`：`commsInstance` 逐字段清洗（缺 id/主题/正文一律丢，参数与奖励只留合法项）；`weekendLastResult` 结构对不上就整条丢（宁可不给面板，也不喂半条） |

**验证**：`weekend-wiring-20260925.test.ts` 再增 2 例（**13 例**）——⑫ 预警开局送达（处数/核心名/族名 id/跳星图）· 同场不重发 · 结算在**入账之后**送达（有奖版正文 ＋ `action` ＋ 结构化清单）· 快照与实发逐值一致 · **下一场覆盖后同 id 仍只有一封**；⑬ 零贡献走变体（无奖版正文 ＋ 空清单）· **两封信与快照都随档往返** · 读档后再同步不重发。
**验证读数**：typecheck ✓ · core **2,351 例全绿**（208 文件）· content:check ✓ · l10n:check ✓ · `ui:rot-check` ✓ · `ui:theme-check` ✓ · build ✓ · docs:index ✓。

## 十、归档草案（**等船长一句「可以」**后照此并入；本文件随后删除）

### 十之一 并入 `docs/roadmap.md`

位置：插在 **`- 2026-09-25：**威胁定价式落地…**`** 那条**之上**（本文件倒序，新的在上）。正文：

```markdown
- 2026-09-25：**H 族入侵按新规重做（威胁重定价 · 抽签 · 遇袭 ×0.75 · 旗舰轮 · 引擎接线与结算入账）（二号 · d2 · 已合入 main · 未推送）**——**①船长原话（照抄要点）**：「**我们回到H族上，按照新规对H族进行调整**」＋「**Q1，非旗舰威胁按照90/108/129定，旗舰因为是小队战，之后在讨论。Q2按照敌舰设定的属性比例重定…Q3伏击 ×0.5移除。旗舰卡单独。**」（后改口：「**伏击恢复×0.5，没问题。（伏击并不是袭击舰队）这边的残骸就不冻结了。**」）＋「**能否让入侵遭遇的敌人卡是随机抽取的**」＋「**外围玩家主动出击和被动遇袭都是从骚扰和袭击舰队中抽取。核心区，则是抽取袭击和主力舰队。遇袭的时候遭遇的敌人按强度\*0.75算。**」＋「**血: 3920 →39200**」→「**感觉整体威胁可以上调了**」→「**取170威胁**」→「**BOSS血条15万（约2.5个母舰）来算**」→「**只有不落才会有撤离动画…选甲吧…黑匣先做壳。火力钳制先按照现有的不做改变。**」＋「**那你将工作做完再汇报。活动开关仍是"仅调试模式"，这条不变。**」。**②落地（三批）**：a) **重定价**——H 三张非旗舰卡按定价式（单舰 ×3）落 90/108/129，属性 = 每艘船血与单发**同乘一个 K**（血÷DPS = 舰级自然比）；旗舰卡 170（**小队 ×10**）、母舰舰级血 ×10（3,920 → 39,200）⇒ 全波 94,439 血 / 峰值 357.7 DPS / 达成预算 2,906 = F(170)；**BOSS 池固定 150,000**（取代两条自适应公式），母舰血条 = **池子剩余**（新 `FoeOverride.bossHp/bossShipId`，随档逐拍套用）。b) **抽签与遇袭**——新 `FoeOverride.strengthMul`（真缩放）＋ `foeThreatOfAnomaly`（定价反解）；外围池 {骚扰, 袭击}、核心池 {袭击, 主力}，`hash32` 独立子流**不消费主随机序列**；板面/主动 = 每场每星系定一支，**遇袭每场重抽**且强度 **×0.75**、标签 = 缩放后实测价（76/91/109）。c) **残骸与黑匣**——残骸组 `h-wh` 退役 ⇒ **`h-hi`（高安组）**（旧组从无产出路径 ⇒ 零迁移）；新物品 **`blackbox-h`「墨潮旗舰黑匣」**（`kind:'kit'` · 稀有度 4 · 只收不卖 · 中英双语）＋ 市场收购行第 9 行。**③引擎接线与结算（第二批起）**：悬赏板/星图改用 `weekendBountyCardsOf`（H = 抽到的**独立卡** · 真实 id · 威胁 = 卡面 · 星系覆写 · 奖励 ×1.4）· 远征落盘 `ExpeditionState.foeGalaxyId`（战后归属与残骸注入不再落母港）· **占领区破例补全**（原只破概率侧、暴露收集侧按安全等级丢掉中安/高安 ⇒ 现 `collectExposures(…, includeOccupiedAt)` 两半齐）· **贡献奖入账**（新 `weekendSettleAndGrant` ＋ 引擎每拍在 `weekendTick` 前补发；幂等靠 `prizePaidAtWallMs`；占比按 `endedAtWallMs` 评估）· 夺回奖励（×8 ＋ 2M，全清再 +5M）真入账。**④同批修掉的五处旧账**：`save` 丢 `market.npcBuy[].bm`（布尔门槛标记）· `market` 消化队列残留 `price` · **`weekendEvent` 清洗器漏掉旗舰 BOSS 那 7 个字段**（读档血条回满 / 削血清零 / 同场幂等键丢失）· **遇袭进度两头都反**（打赢记成主动胜利 +10%、打输记成击退 +3%）· **迎战遇袭一分不给 + 文字结算路径没接**（`settleFight` 先清遭遇槽 ⇒ 归属反推落空；现显式传 `WeekendOutcomeHint`）。**⑤验证**：typecheck ✓ · core **2,348 例全绿**（208 文件；新增 `h-reprice` 17 例 · `weekend-wiring` 9 例 · 存档往返 1 例）· content:check ✓ · l10n:check ✓ · build ✓ · docs:index ✓。**⑥工作文档**：`docs/design/h-family-reprice-20260925.md` 已归档（内容并入本条目 ＋ `docs/glossary.md` 周末入侵词条，原文件删除）。⛔ 遗留：**A/C/G 三族仍是占位口径**（派生卡保留原卡 id ⇒ 标签 78/120 与属性不符）· **入侵通讯两封未做**（设计稿只写"两封"、内容待船长定）· **M3 两个新族未开工** · 解除"仅调试可见"待船长令。
```

### 十之二 并入 `docs/glossary.md`（改「周末入侵」那一行；下表是要替换/新增的片段）

| 片段 | 现状 | 归档后 |
|---|---|---|
| 悬赏替换 | 派生卡只覆盖名字/威胁/奖励 · 保留原卡 id | **H 族 = 抽到的独立卡**（真实 id · 威胁 = 卡面 · 星系覆写 · 奖励 = 原卡 ×1.4）；A/C/G 仍原卡派生（占位） |
| 威胁 | 外围 78 / 核心 120 | **H 族按卡面**：骚扰 90 · 袭击 108 · 主力 129 · 旗舰 170（小队战）；A/C/G 占位 78/120 |
| 遇袭 | `p = 60%×(1−进度)` · 中安/高安破例 | 同上，另加：**强度 ×0.75** · 标签 = 缩放后实测价（76/91/109）· 板面/主动每场每星系定一支、**遇袭每场重抽** |
| 进度 | 主动外围 +10% / 核心 +5% · 击退遇袭 +3% · 离线 +1% | 同上（本轮把 +3%/+1% 真正接通：迎战打赢 = +3% · 文字结算击退 = +1% · **战败不动进度**） |
| 旗舰 | 120 威胁 · 4 波 · 4 艘小队战 · 2h 未击毁则章鱼人得手 | **170 威胁 · 血池 150,000（母舰血条 = 池子剩余，单场不死、跨场累计）** · 4 波小队战 · 击沉必掉 **`blackbox-h` ＋ 稀有残骸 ×3** · 2h 未击毁 ⇒ 章鱼人得手（黑匣归零、贡献奖照给） |
| 贡献奖 | 四档（Q5） | 同上，另加：**结束时真入账**（幂等标记 `prizePaidAtWallMs` · 占比按结束时刻评估） |
| 新增登记 | — | **`h-hi`**（墨潮帮残骸 · 高安组）· **`blackbox-h`**（墨潮旗舰黑匣，用途待"改装/特殊装备"批） |

### 十之三 归档三步（船长说「可以」后一次性做完）
...
⚠ 边界一条：**"仅调试可见"的闸门落在"开局"**（`ensureWeekendEvent` 里 `WEEKEND_DEBUG_ONLY && !debugOn ⇒ 不开局`），所以正常模式的新档/老档永远不会有入侵；但**已经带着入侵的 debug 档**（只有船长与二号的档会这样）导进正常模式时，界面照旧显示该场入侵、`weekendTick` 也照旧推进。要不要收紧成"非调试模式一律隐藏/冻结"？请一句话（收紧 = 在 `weekendTick` 与界面取数口加同一道 `weekendDebugOn` 判据）。

## 十一、验机清单（**船长验收照着点一遍**）

前置：设置里开**调试模式**（`debugQuick`）——入侵只在调试模式开局（船长令）。

| # | 看哪里 | 期望 |
|---|---|---|
| 1 | 事件日志底部 | 出现**红色「周末入侵」框**（点击跳星图）；日志一条「入侵开始：本周的入侵舰队已在目标星系出现。」 |
| 2 | 星图 | 被占星系有**红环与旗标**；核心星系与外围共 1 + N 处 |
| 3 | 被占星系的悬赏卡 | 名字 =「**H 族舰队 · 墨潮帮骚扰舰队**」（或「· 墨潮帮袭击舰队」；核心区是「袭击 / 主力」）；**威胁 = 卡面自身**（外围 90 或 108 · 核心 108 或 129，**不再是 78/120**）；奖励 = 该星系原卡 ×1.4 |
| 4 | 在该星系**采矿/打捞** | **高频遇袭**（**高安也掷、且无 5 分钟入场缓冲**）；遭遇卡 ∈ 该区域池；**强度 ×0.75**、标签 = 缩放后实测价（**76 / 91**）；日志写「**入侵遭遇**」 |
| 5 | 遇袭点「迎战」打赢 | 进度 **+3%**（不是 +10%）；打输只受损、进度不动；放着不管 60 秒自动结算 ⇒ 判成击退时 **+1%** |
| 6 | 打该星系的悬赏（主动出击） | 进度 **+10%**；打满 ⇒ 该处**夺回**：钱包 +2M、稀有残骸 ×8（货舱），并留一条日志「**✦ 夺回「<星系>」：夺回奖励已入账 —— 稀有残骸 ×8 ＋ 2,000,000 信用点。**」（全部夺回时换成"全清"那条并含 +5M） |
| 7 | 全部外围夺回后 | 核心星系解锁：打核心悬赏每场 **+5%**（没夺回外围时打核心 = 0，门禁） |
| 8 | 核心条满 | 日志「入侵核心已被打通：旗舰现身。」；面板出现「**挑战旗舰**」 |
| 9 | 挑战旗舰 | 4 波小队战；母舰血条 = **池子剩余**（总池 15 万，跨场累计、**单场打不死**）；章鱼人在你不打时慢慢削 |
| 10 | 把池子打空 | 旗舰**击沉**：**墨潮旗舰黑匣 ×1** ＋ 稀有残骸 ×3（都在货舱）；日志两条——「**✦ 入侵旗舰击沉：母舰血量归零 —— 战利品已入账（旗舰黑匣 ×1 ＋ 稀有残骸 ×3）。**」＋ 结束那条；再等一拍 ⇒ 「**贡献奖已发放：贡献占比 x%，奖励 稀有残骸 ×N ＋ M 信用点**」（0% ⇒ 「本次入侵没有贡献，无贡献奖。」） |
| 11 | 存档/读档 | 中途读档：**母舰血条不回满**（池子随档）、章鱼削血不清零；活动结束后读档：**贡献奖不重发** |
| 12 | 导航「通讯」 | 开局那拍收到**「航线警告：墨潮帮入侵」**（发件人 深空工业协会 · 航线安全），正文写本场落点处数与核心星系；点跳转去星图 |
| 13 | 活动结束时 | 再收一封**「航线通报：墨潮帮入侵已被终结！星域恢复了和平！」**（每场覆盖上一封，收件箱里始终只有这一封）；点「**查看详细奖励**」弹出**入侵结算面板**：清缴贡献/夺回星系/旗舰输出/稀有残骸/信用点/旗舰黑匣 ＋ **各星系贡献明细** ＋ 旗舰战一行 |

⚠ 观感（颜色/动画/排版）请按老规矩你自己看；上面 11 条都是**读数类**（数字与文案），我能自证。
