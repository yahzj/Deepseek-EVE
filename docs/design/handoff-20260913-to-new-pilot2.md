...
1. `git -C d2 merge main`：**一号正在补族池两处**（族池加「无人机」类 + `engine.wormholeDescend` 传 `scanBonus`）。
   他提交后你要**核验并补齐**：① `content:check` 的「无人机归属 / 孤儿」契约（⚠ 无人机是**选择性替换物，只有 C/E 两族有**，
   **不要**按"五族都要有"判空）② 对应用例 ③ 按族清单与设计稿同步。
2. 船长手上还挂着两条待裁：**公告里那处「鱿蜂无人机」是否改名**（仅文案、id 不动）与**上线动作**（删 `unreleased` + 公告待审稿，他答过"先不动"）。

## 6. 挂账（当前 **4 条**）

| # | 事项 | 归属 | 状态 |
|---|---|---|---|
| 1 | 族池加「无人机」类（C 巢卫攻坚 / E 构件哨戒，**10 架/次**，复用 `RARE_BOX_DRONE_UNITS`） | 一号 | 在途 |
| 2 | `engine.wormholeDescend` 传 `wormholeScanBonusOf(ctx, run.fleet)` | 一号 | 在途 |
...
13. **"强增"前先确认字段有消费点**：`lockRangeM`（锁定范围）与 `signatureM`（信号半径）**全引擎零消费**（纯展示），
...
- [ ] `git status` 干净、已 `merge main` 追平、四连全绿（1232 用例基线）
