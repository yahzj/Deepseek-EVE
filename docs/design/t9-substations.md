...
- ⚠️ **交付前置 = 工地现场（2026-09-06 紧急修复，玩家反馈"建设任务资源无法提交"）**：任务完成即返航
  改动后，建成前的副站不再有任何停靠入口（返航只去已建成站/母港），而交付原要求"停靠该站"——首档
  『奠基』本就应先到工地交付才能解锁泊位，形成死锁。修复：交付条件放宽为 `playerAtSite`（停靠该站，
  或野外停留于站点星系——掩护巡逻/作业到场即可，无需停靠；母港特例保留）；任务卡判定/提示与到位
  日志同步改为"现场交付"；core t9 +3 回归例。
- renderer：任务中心建站族卡（提交面板：选料/数量/可用量；工地现场判定）、通讯器浮层（App 待播自动弹出 +
  卡内重看）、活动栏停靠文案、CargoPage（出售仅母港、卸货任意站）、styles。
- 测试：`t9.test.ts` 8 组（交付/档位/建成并入与最近站解析/抵达挂点两态/通讯镜像与已读/存档容错/
  工地现场交付 ×2 回归）。
- 换驾驶在任何空间站可用（舰队=随行船队模型，船长确认 ②）；~~母港独有：市场/制造台~~
  **2026-09-08 起作废**：**已建成副站 = 母港镜像**——市场/精炼炉/残骸回收/组装机等站内功能在母港与已建成副站
  同样可用（设施与仓库全局共享，空间站只是入口；判定 `location.ts isAtHomeLike`）；野外/航行中/修建中工地不算基地。
- **2026-09-09 追加口径**：建站**建材 = 精炼矿物、排除原矿**（`types.ts` 档位材料单注释；
  原"该星系常见产出（含原矿）"的缴交主体口径按此收窄）；「建成前不视为任何站点」的口径不变——
  工地只能在现场交付，不提供泊位/功能（见上方 ⚠️ 交付前置段）。
