...
6. **2026-09-24 改判显示形态**：「**虫洞的围剿，改为照常显示下方地点信号，族徽位置移动到右上角
   探索过的小点处**」；追问后裁决 = **乙：中心仍按原遮蔽规则**（未扫描不给信号、星云未驱散只显示云），
   族徽只在右上角提示这里压着敌人。
...
### 显示形态（2026-09-24 船长二次改判 · 已落码）

- **中心那一格**：照常按原遮蔽规则画。`revealOf` 的 `foe` 档**带上 `under`**（撇开围剿者、这一格本来会
  揭示成什么：`unknown` / `nebula` / `signal` / `known`），界面拿 `under` 画中心符号。
- **右上角**：族徽挪到"去过小点"那一片角（同一处只画一个：有围剿者画族徽、否则去过画小点）；
  它**一直看得见**——不被星云与"没扫过"吃掉（2026-09-23 那条「敌人看得到这个标记」不变）。
- 微调入口：`Wormhole.tsx` 顶部 `WORMHOLE_FOE_MARK_PX`（边长 12）与 `WORMHOLE_FOE_MARK_AT`（锚点 0.44 × 格径）。
- 用例：`wh-spawn.test.ts` ⑪ 锁三档（未扫描 / 星云未驱散 / 扫开与去过）＋ 打掉后揭示档回到 `under`。
