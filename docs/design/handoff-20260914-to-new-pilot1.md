...
> 上一张同族卡**已归档**：`docs/design/archive/handoff-20260912-to-new-pilot1.md`
> （通用工作方式那几节仍有效；本卡只写**今天变了的东西**与新挂账，不重复抄）。
...
- **入口**：星图「出港 · 扫描虫洞」→ 扫满一个窗口（**12 小时** × 三技能乘算 × **星际奇遇学**（虫洞专属，**满级 −20%**）；
  调试 1 秒化）标出一处虫洞，
  库存上限 5 处；**解锁门槛 = 协会声望 40**（解锁发通讯 `msg-wormhole-unlock` + 直接弹窗；
  **达标当次送一格**：`reconcileWormholeScanWelcome` 把进度条置成满一个窗口 ⇒ 点「开始扫描」第一拍即得一处，
  只送一次、老档补发、**不提示**）。
...
**已收口（写卡后当场做完，勿再当挂账）：**
1. ~~剑鱼级 `sh-swordfish` 声明不一致~~ ⇒ **核实为误报**：它**没有** `unreleased` 字段（我当时的扫描把
   `ships.ts` 里一行**注释**里的 `unreleased: true` 当成了字段）。**真正**的遗留是**鹦鹉螺级 `sh-nautilus`**
   （协会测绘处 · 虫洞扫码 +1）——它按设计稿「跟随虫洞挂闸门」写着"上线时删字段" ⇒ **已随虫洞上线放开**
   （6 处：舰体 · 永久/一次性图纸各 1 · 三张市场卡），现口径：图鉴可见、市场可买（9,000,000 ISK · 声望 12）。
2. 历史卡 `docs/design/archive/handoff-20260913-wormhole-4ship-stage.md`（4 舰同屏早已交付）**已按归档口径移入**
   `docs/design/archive/`（连同另两张过期卡：`handoff-20260912-bounty-wave-split.md` ·
   `handoff-20260912-to-new-pilot1.md`），全仓引用路径已同步改（含 `development-conventions.md`
   与变更记录里那两处）。
