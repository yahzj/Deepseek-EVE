...
状态：**已落码 · 四闸门全绿 · 船长实测通过（2026-09-24）· 差推送**（改动**只在本地 commit、未推送**，见下）

> **推送口径（船长 2026-09-24：「不要立刻推送。」＋ 同日晚「所有推送，如果我没强调都是一号负责。」）**：
> 本件开发期间**只做本地 commit，不推 origin**；**推送一律由一号负责**——二号不推、也不代一号推。
> 一号按 §4 推送闸门（与本批公告一起）推一次即可。
> ⇒ **2026-09-24 落码完成 ＋ 船长实测通过**，本地等一号推送。
...
5. 「**船体修理装置（G 族）基础数值上调至15装甲15结构。**」（同日追加 ⇒ **原令写的 5/5 被本令改写**）
...
| **船体修理装置**（G 族） | 每 **5 秒**回 **+15 装甲 +15 结构** × **k**，夹到 `hpMax`（满值不回超）；`k = 该层威胁 ÷ 45`（层 1 = 1.00 ⇒ **15/15** · 层 7 ≈ 1.97 ⇒ ≈30/30 · 层 10 ≈ 2.77 ⇒ ≈42/42）。⚠ 船长 **2026-09-24 同日二次令：基础数值由 5/5 上调至 15/15** |
| **顺序** | 先提闪避（已完成），再挂载（下一轮） |
...
- `k` 的基准 = 层 1 的 45（船长明示「不改动」）⇒ 层 1 也有 ×1.00（即层 1 就有 **15/15** 的修理；
  同日二次令把基数由 5/5 上调到 15/15 ⇒ 整条曲线等比抬到 3 倍：层 1 = 15/15 · 层 7 ≈ 30/30 · 层 10 ≈ 42/42）。
...
7. 用例：`packages/core/tests/` 新增（或并入既有挂载件用例）三条——闪避 +10pp 真进命中判定 ·
   每 5 秒回 15/15（**基数经船长二次令上调，原计划写的是 5/5**）且**层越深回得越多** · 满血不再回。
...
2. **`core/foeMounts.ts`**：`FOE_MOUNT_IDS.gyroStabilizer` / `.hullRepair` ＋ 两条定义（名/英文名/`note` 摘原话）；
   `ResolvedFoeMounts` 加 `foeEvasionBonusAdd` 与 `foeRepairPulse`。⚠ **聚合口径经合并采纳一号那版**：
   **闪避加数取「加和」**（可叠加；两件陀螺仪 = +0.20），其余效果仍是"**后写覆盖先写**"
...
7. **用例**：新增 `packages/core/tests/foe-mounts-20260924.test.ts`（**15 条**）：闪避 +10pp 真进命中判定
   ...
   **双语名对与名字同序对齐** · **摘掉挂件的同形卡 ⇒ 派生威胁/三层血/单发逐值不变**
   （钉住"挂件不参与派生血与火力"）。
   ⚠ **基数改 15/15 时把用例改成"从目录读基数"**（`BASE = FOE_MOUNTS[...].repairPulse`）⇒
   日后再调基数只改目录一处、用例不必重算数字（这次 5/5 → 15/15 就是靠它一次改完）。
...
- **四闸门**：typecheck 四包 0 错 · core **201 文件 / 2271 用例全绿**（含新增 15 条与同步改写的既有 3 条；
  基数改 15/15 后复跑仍全绿）·
...
- **实测结论（船长 2026-09-24）**：**没问题** —— 本件在船长侧验收通过，不再挂"待验收"。
- **追加令（同日 · 落码后）**：船长「**船体修理装置（G 族）基础数值上调至15装甲15结构**」⇒
  只改**目录里的基数**（`FOE_MOUNTS[hullRepair].repairPulse.armor/hull` 5 → **15**）：层 1 = 15/15、
  层 7 ≈ 30/30、层 10 ≈ 42/42；用例改成从目录读基数、`content:check` / 用例复跑全绿。
  ⚠ 这条是**实测之后的加强**，与上面的"实测通过"不冲突（改的是量级，不是机制）。
- **推送**：按页首口径**未推送**，且**推送一律归一号**（船长同日）——二号这边到此收工，不再动本件。
