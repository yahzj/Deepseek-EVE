...
- [x] `npm run test -w @whale/core` 全量全绿（含新增 13 条）
- [x] `npm run content:check` **✅**（新契约：`1 条促销` 逐项通过）
...
- [x] 负向验证：把 `giftWormholes` 改 0 ⇒ 赠送用例如实红；把 `until` 挪到过去 ⇒ 倍率与赠送双双不生效
...
待审稿：`docs/design/announcement-draft-20260916-wormhole-bloom.md`（**待船长审核**；批准后才写进
`packages/data/src/announcements.ts`，驳回则改后重审）。
