...
| 推送闸门 | **本地 main 领先 `origin/main` 126 个提交，全部未推送**。规则：开发中只做本地 commit；该工作**确实完成**（自测全绿 + 船长验收）后，才把**代码与公告一起推一次** |
...
| # | 事项 | 现状 | 你需要做的 |
|---|---|---|---|
| 1 | **公告待批：14 张候选卡分三稿** | `announcement-draft-20260910-pending.md`（11 张：三号的 9 张 + 一号的蓝图速率、稀有残骸解禁）、`announcement-draft-20260910-d2-batches.md`（2 张：五族专属装备、敌方混伤）、`announcement-draft-20260910-drone-losses.md`（1 张） | 等船长**逐条**批复（可/改/删）；批准的按序写入 `announcements.ts` 顶部（现 22 条），与代码一起推 |
| 2 | 派系活跃稀有残骸掉落概率 | `FACTION_RARE_DROP_CHANCE = 0.1` ×1 件——`content:check` 输出仍标「待船长核定」 | 提醒船长定数（或明确"维持 10%"） |
| 3 | 高级箱「主题件兜底」金额是否收口 | 命中率降到 5/8/10% 后，未命中必给主题件：外围有主题池卡每箱可兑现 ≈11.9 万 ISK（全清日板口径每天可达百万级） | 船长未表态；若要拧，旋钮 = 主题件兜底概率或矿物单位（`RARE_BOX_MINERAL_UNITS`） |
