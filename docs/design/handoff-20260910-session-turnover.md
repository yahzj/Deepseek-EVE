...
| 推送闸门 | **已清空（2026-09-11 更新）**：`origin/main` 已追平本地 main（`4a6805f`，本地未推送 **0** 个提交）。规则：开发中只做本地 commit；该工作**确实完成**（自测全绿 + 船长验收）后，才把**代码与公告一起推一次**。**网络提示**：到 GitHub 的直连不稳定（`github.com:443` 常被重置），系统代理 `127.0.0.1:7897` 开着时推送正常；**代理没开就先请船长开**，不要改 git 持久代理配置 |
...
| 5 | 本批改动验收 | **2026-09-11：已推送**——船长「都不用公告，如果没有问题就进行推送」⇒ 公告全部不发（`announcements.ts` 保持 25 条）、代码与文档一次性推送，`origin/main` = `4a6805f`（本地未推送 0） | 转**实机验收**：推完即可玩，问题回传一号/二号 |
