...
- [x] 后续独立项：星图·远征行动列表弹窗化（2026-09-08 本地完成：点星系弹出 行动/扫描 浮层（app-comm 族），星图不再被详情挤压；**待船长实机验收，未推送**）
- [x] 后续微调项：组装机/任务中心 筛选行固定（2026-09-08 本地完成：win-fixed-body——类型/子标签行与说明固定，卡网格/任务区独立内滚；**待验收未推送**）
- [ ] 全量回归：桌面各页 + 手机横屏矮高预算（不裁内容红线）+ ≤1180px 窄桌面


## 星图·远征 自适应收口（2026-09-10 船长反馈：全屏时星图页出现滚动条）

**现象与量化**（网页版构建 + 无头 Chrome 实测布局）：

| 窗口 | 星图实际渲染 | 面板可用高度 | 结果 |
|---|---|---|---|
| 1600×900 | 1007×433 | 547 | 正常 |
| 1920×1080 | 1327×570 | 727 | 正常 |
| 2560×1440 | 1967×844 | 1087 | 正常 |
| 2560×1080 | 1959×841 | 727 | 溢出 188px → 面板内纵向滚动条 |
| 3440×1440 | 2839×1218 | 1087 | 溢出 205px → 面板内纵向滚动条 |

**根因**：星图为 `<svg viewBox="0 0 700 300">` + `width:100%; height:auto`——高度完全由宽度按 700:300 推出。
窗口越宽星图越高，一旦「宽度推出的高度」超过面板可用高度，星图所在面板体（`.wui-panel-body`，`overflow:auto`）
就出现滚动条（页面本身仍是 `overflow:hidden`，属二级窗内滚动）。

**定案（船长 2026-09-10 确认）**：星图等比缩放适配可用框，任何窗口尺寸都不出滚动条。
- 星图·远征面板加 `.app-exp-panel`，其面板体改纵向 flex：状态行/提示行/编辑条 `flex:0 0 auto` 保持自然高度；
- `.app-starmap-wrap` 改列向 flex 且 `min-height:0`（允许收缩）；
- `.app-starmap` 改 `flex:0 1 auto; min-height:200px; box-sizing:border-box`：
  容器够高时尺寸与观感与原先完全一致；装不下时 SVG 按 viewBox 居中等比缩小（`preserveAspectRatio` 默认 xMidYMid meet）；
  保底 200px——窗口被压到极矮时宁可面板内滚动，也不把星图压成一条线；
- 顺带修掉 `width:100%` + 1px 边框叠加造成的 2px 横向溢出（border-box）。

**复测（同一探针，修后）**：1600×900 / 1920×1080 / 2560×1080 / 3440×1440 四档面板体均 `client == scroll`（无滚动条），
星图完整落在可用框内并居中；修前两档的 188px / 205px 溢出归零。