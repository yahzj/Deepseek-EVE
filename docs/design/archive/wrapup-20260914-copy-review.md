...
| 工作区 | `main` 有**一号在途未提交改动**（`holdDropWithGrab` 拖拽偏移链 5 文件：Wormhole.tsx / engine.ts / index.ts / wormholeHold.ts / wormhole-hold.test.ts）——船长已裁定「**一号工作由他自己完成**」，本会话**未动、未提交**，收尾时原样保留。（**补记：本件提交后一号已自行提交 `1d5b1838` 并清掉遗留探针，工作区已干净**） |
...
_工作区剩一号在途未提交 5 文件 ＋ 其遗留探针 `tools/_wh-drag-probe.ts`（均未动，按船长裁定交一号处置；**补记：一号已自行提交 `1d5b1838` 并清掉探针，工作区已干净**）_
