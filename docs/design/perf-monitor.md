...
> 状态：**已交付并归档放置**（2026-09-07 船长定）。工具已入库可用、本机跑分结果已记录；
> 后续是否实施"第二包：战斗渲染隔离"等优化，等玩家在真机用 debug HUD「复制报告」
> 回传快照后再评估。当前结论：JS 主线程开销很小（战斗最重 ≈单核 4%），
> 高占用更可能来自 GPU/合成/绘制或多实例叠加（未测，待真机数据）。
>
> 用法速记：玩家侧 DevTools `localStorage.setItem('whale-idle:debug','1')` → 刷新 →
> 顶栏「⏱ 性能」→「复制报告」；本机 `powershell -File tools/run-autoperf.ps1`。
