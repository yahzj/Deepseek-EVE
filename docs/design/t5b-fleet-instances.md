...
## 追加：舰队卡左侧舰影（2026-09-10 船长定，状态：已实现待验收）
- 需求原文：船长「我的舰队页面，将每艘船的舰船形象放在对应容器的最左侧展示」+「当玩家屏幕宽度不足时隐藏舰船图形」。
- 形态：每张舰队卡左侧一个固定尺寸（132 宽）舰影列，右侧原有信息整体包进 `.app-ship-main`；舰影 = `ShipSprite`
  取该船 defId 的独立矢量形，`engine={false}`（静态展示，不画尾焰，避免与"出航中"语义混淆）。
- 作用域：只给舰队卡加修饰类 `.is-fleet`（`.app-ship-card` 本身不改）——同页「舰船市场」标签页的卡片保持原纵向排布。
- 显隐口径：ShipPage 用 `useLayoutEffect` + `ResizeObserver` 量**列表容器实宽**（`clientWidth`，已扣竖向滚动条），
  `< 舰影 132 + 间距 10 + 信息列下限 560 = 702px` 即整列不渲染。不用窗口宽猜：壳层有 168px 左侧栏 + 面板内边距，
  且手机竖屏会走 `is-mobile-rot` 的 1200 虚拟宽（布局宽与窗口宽不再同尺度）。
  不振荡的依据：卡片高度由右侧信息列决定（舰影更矮），隐藏舰影不会反过来改变容器宽。
- 性能：舰影组件走 `memo`（引擎每 tick 触发整树重渲染，props 恒定即整棵 SVG 子树跳过 diff）；未加 `filter` 辉光
  （列表最多 20+ 条，静态 filter 会拖滚动画质，见约定第十四章第 5 条）。
- 各窗口档位实测口径：桌面窗口（最小 1024）列表 ≈ 810px 起 → 显示；手机竖屏旋转模式布局宽 ≈ 988 → 显示；
  手机原生横屏 / 浏览器窄窗 < 702 → 隐藏。
- 验证：typecheck ×4 + core 658/658 + `build -w @whale/desktop` + content:check 全绿。
