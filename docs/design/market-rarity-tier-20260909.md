...
> 状态：**已确认并落地**（船长 2026-09-09 多轮拍板；2026-09-10 又两笔修订：**tier3 权重 0.25 → 0.15**、
> **12 件 MK3 装备由 2 升 3**，见 §二 末段）。经办：二号。
> 触发：船长观察「稀有订单过多、各种东西混杂」；要求卖单/买单刷新几率与权重受稀有度影响。
> **注意**：§一 是 09-09 的**决策原文留档**（其中第 7 条的「0.25」已被 09-10 修订取代），**现行值一律以 §二 为准**。
...
7. 系数与总量：tier3 权重 **0.25**（**2026-09-10 改为 0.15，见 §二**）、稀有卖单总量**不收敛**（模拟数据审定后船长拍板维持现状张数公式）。
...
- 消费点（仅 rare 渠道，同权重表）：
  - 卖单：slowSupplyDraw rare 加权抽取，行权重 × `rareTierWeight(def, ctx)`
    （tier3 → `balance.market.rareTier3Weight`（**2026-09-10 船长定 0.25 → 0.15**），其余 → 1）；
  - 买单：refreshGoodOrders rare 分支收购概率（每 60s 窗 3% × 建站扩容）同乘该权重。
- 蓝图 ×0.5、暗市闸 ×0.04 等既有权重规则不变；奇货（exotic）掷骰路径未动。
- **2026-09-10 同批两笔（船长定）**：①权重 0.25 → **0.15**；②**12 件 MK3 装备由 2 升 3**
  （护盾增强器三系 / 护盾扩展器 / 装甲镀层三系 / 装甲增厚板 / 矢量推进器 / 无人机甲板扩展 /
  中继天线 / 导控阵列）——它们随"装备价对齐同级武器价"跨过 50 万线，与其余 MK3 装备同级。
  **实测（2880 窗 × 5 种子）**：tier2 57 行 / tier3 **69 行**；tier3 每窗 1.84 张（**占比 13%**，
  改前 10%）、单行间隔 **5.6 小时**（改前 5.7h——权重降 40% 与行数 +21% 相抵）；tier3 蓝图行
  **51.3 小时**（改前 40.9h）；tier2 单行 0.9 小时。**档位自此与价格解耦**（调价不再自动改档）。
  **船长 2026-09-10 复核**：tier3 **每窗总量占比由 10% 升到 13%**（行数 57 → 69 把权重降幅摊薄）已知情，
  裁决**保持 0.15 不再下调**——口径取「逐件更难碰」（原 T3 行更难、新升档的 12 件由 0.9h → 5.6h）。
