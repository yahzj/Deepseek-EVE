...
- **资源价值曲线**：单件资源（1 m³/单位）站内收价严格递进——矿石 13→490（7 种；克洛基石/
  ...
  旧 4 矿带保持 0；
- **精炼经济（2026-09-08 船长两轮定稿）**：玩家口径术语 = **产出倍率**（旧称收率，已全量替换）；
  配方 perOre 按「产出倍率 100%（无技能）时每批产值 ≈ 耗料价值（双方站内收价口径）」标定——
  零技能精炼不吃亏（取整允许实际净率微正 +0.4%~+4.6%）；倍率 = 基础 100% + 精炼学 +8%/级 +
  高级回收处理 +4%/级，**上限 160%**（技能加满，把倍率从 100% 推到 160%，无技能产出与
  「50% 起点」体系等值、技能每级收益减半防工业技能过肥）；满技能净 ≈ +60%~+79% 耗料值；
  配方实值见 §3.1 与 items.ts（护栏 tools/content-check.ts）；产物只指向矿物（气体/冰不出新物种类）；
...
| id | 名称 | 站内收价 | perOre（产出倍率 100% 基准） | 无技能实际净率 | 满技能(160%)净率 |
|---|---|---|---|---|---|
| ore-veldspar | 富凡晶石 | 13 | trit 1.175, pyer 0.34 | +2.6% | +65.4% |
| ore-scorched | 灼烧岩 | 18 | pyer 0.985, mex 0.35 | +2.2% | +64.4% |
| ore-hemorphite | 希莫非特 | 55 | noc 0.53, trit 1.445 | +2.2% | +64.4% |
| ore-glowstone | 辉云岩 | 150 | iso 2.64, trit 2.15 | +2.3% | +64.0% |
| ore-sunshard | 曦棱晶 | 115 | iso 2.075, pyer 0.77 | +3.5% | +65.9% |
| ore-voidshard | 玄晶 | 340 | star 1.185, dark 0.095, noc 0.24 | +2.7% | +70.4% |
| ore-nebulite | 星幽矿 | 490 | dark 0.5, star 0.685 | +4.6% | +69.4% |

> 2026-09-08 精炼配平刷新（两轮）：旧表（比值 0.95~1.93 @旧"100%收率"，希莫非特按历史略亏
> 保留）作废；比价基准 = 资源站内收价（baseSellPriceIsk，市场均衡价 ≈ catalog 基价、微差不参与
> 配平）；perOre 为「倍率 100% 下每 1 单位产出」（配方较旧表按 ×~1.1~1.8 上调后 ÷2 等价迁移）；
> 净率为按表定批量与引擎 floor 的实际结算；完整记录见 docs/design/refine-balance-50.md。

**气体（4）**（kind=gas，1 m³/单位，可采集可精炼）

| id | 名称 | 站内收价 | perOre（产出倍率 100% 基准） | 无技能实际净率 | 满技能(160%)净率 |
|---|---|---|---|---|---|
| gas-neon | 氖云气 | 85 | iso 1.43, trit 0.92 | +0.5% | +61.3% |
| gas-phosphor | 磷光霾 | 330 | iso 2.29, star 0.81, mex 0.63 | +0.5% | +59.7% |
| gas-ionstorm | 离子风暴云 | 230 | star 0.77, dark 0.065, iso 0.315 | +1.2% | +66.4% |
| gas-aurora | 极光云 | 330 | star 0.735, dark 0.25 | +1.7% | +78.7% |

**冰矿（3）**（kind=ice，1 m³/单位，可采集可精炼）

| id | 名称 | 站内收价 | perOre（产出倍率 100% 基准） | 无技能实际净率 | 满技能(160%)净率 |
|---|---|---|---|---|---|
| ice-frost | 蓝霜冰 | 150 | iso 2.475, mex 0.73 | +0.4% | +60.2% |
| ice-marrow | 寒髓冰 | 230 | star 0.66, iso 1.21, mex 0.44 | +0.7% | +62.6% |
| ice-darkstar | 暗星冰 | 360 | dark 0.295, star 0.55, iso 0.915 | +0.9% | +68.6% |
...
- **甲壳重装线（+3）**：陆龟级重装艇 7000/13/24/0.50·330k、玳瑁级重装巡舰 12000/13/22/0.46·760k、
  玄武级重装战列舰 19000/14/26/0.40·2.2M（exotic，声望 9）
